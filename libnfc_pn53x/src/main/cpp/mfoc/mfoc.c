...
bool mf_init(mfreader *r) {
    // Connect to the first NFC devices
    nfc_init(&context);
    if (context == NULL) {
...
int
mf_enhanced_auth(int e_sector, int a_sector, mftag t, mfreader r, denonce *d, pKeys *pk, char mode,
                 bool dumpKeysA) {
    ...
    if (mode == 'r') {
            ...
            if (valid_nonce(NtProbe, NtEnc, Ks1, d->parity)) {
                ...
                while ((revstate->odd != 0x0) || (revstate->even != 0x0)) {
                    ...
                    if (((kcount % MEM_CHUNK) == 0) || (kcount >= pk->size)) {
                        ...
                        pk->possibleKeys = (uint64_t *) realloc((void *) pk->possibleKeys,
                                                                pk->size * sizeof(uint64_t));
        ...
        if (kcount != 0) {
            pk->size = --kcount;
            if ((pk->possibleKeys = (uint64_t *) realloc((void *) pk->possibleKeys,
                                                         pk->size * sizeof(uint64_t))) == NULL) {
                ERR("Memory allocation error for pk->possibleKeys");
                //exit(EXIT_FAILURE);
                return -1;
            }
        ...
        for (j = 0; j < (t.num_sectors); ++j) {
            ...
            if ((dumpKeysA && !t.sectors[j].foundKeyA) || (!dumpKeysA && !t.sectors[j].foundKeyB)) {
                ...
                for (k = 0; k < probes; ++k) {
                    ...
                    //TODO 此处调用破解函数进行嵌套漏洞密钥流获取!!!
                    int authresult = mf_enhanced_auth(e_sector, 0, t, r, &d, pk, 'd',
                                                      dumpKeysA); // AUTH + Get Distances mode
                    if (authresult == -99999) {
                    ...
                    printf("start recover 2byte key stream!\n");
                    printf("Sector: %d, type %c, probe %d, distance %d ", j,
                           (dumpKeysA ? 'A' : 'B'), k, d.median);
                    // Configure devices to the previous state
                    mf_configure(r.pdi);
                    mf_anticollision(t, r);
                    ...
                    //我们有可能是潜在密钥的32b密钥流(4字节，然后我们可以碰撞初剩下的16bit，也就是两字节)!
                    for (n = 0; n < sets; n++) {
                        // AUTH + Recovery key mode (for a_sector), repeat 5 times
                        //验证 + 恢复密钥模式，重复5次!
                        int authresult = mf_enhanced_auth(e_sector, t.sectors[j].trailer, t, r, &d,
                                                          pk, 'r',
                                                          dumpKeysA);
                        if (authresult == -1) {
                            return authresult;
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    //得到native函数的定义类
    jclass clazz = (*jniEnv)->FindClass(jniEnv, "cn/rrg/natives/MfocTools");
    if (clazz == NULL) {
        return -1;
    }
