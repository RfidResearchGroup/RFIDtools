...
// Prototypes
// struct usb_device *dev, usb_dev_handle *udev don't need!
bool pn53x_usb_get_usb_device_name(char *buffer,
                                   size_t len);

int pn53x_usb_init(nfc_device *pnd);

static int
pn53x_usb_bulk_read(struct pn53x_usb_data *data, uint8_t abtRx[], const size_t szRx,
                    const int timeout) {
    return c_read(abtRx, szRx, timeout);
}

static int
pn53x_usb_bulk_write(struct pn53x_usb_data *data, uint8_t abtTx[], const size_t szTx,
                     const int timeout) {
...
const struct pn53x_usb_supported_device pn53x_usb_supported_devices[] = {
        {"NXP_PN531",   NXP_PN531,   "Philips / PN531"},
        {"NXP_PN533",   NXP_PN533,   "NXP / PN533"},
        {"SCM_SCL3711", SCM_SCL3711, "SCM Micro / SCL3711-NFC&RW"},
        {"SCM_SCL3712", SCM_SCL3712, "SCM Micro / SCL3712-NFC&RW"},
        {"SONY_PN531",  SONY_PN531,  "Sony / PN531"},
        {"ASK_LOGO",    ASK_LOGO,    "ASK / LoGO"},
        {"SONY_RCS360", SONY_RCS360, "Sony / FeliCa S360 [PaSoRi]"}
};
...
#define MAXSZXRAMUSBDESC 61
    ...
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_INFO, "%s",
            "Fixing USB descriptors corruption");
...
pn53x_usb_get_device_model(const nfc_connstring connstring) {
    if (connstring == NULL) return UNKNOWN;
    for (size_t n = 0;
         n < sizeof(pn53x_usb_supported_devices) / sizeof(struct pn53x_usb_supported_device); n++) {
        if (strcmp(connstring, pn53x_usb_supported_devices[n].connstring)) {
...
static size_t
pn53x_usb_scan(const nfc_context *context, nfc_connstring connstrings[],
...
pn53x_usb_open(const nfc_context *context, const nfc_connstring connstring) {
    ...
    // init model!
    DRIVER_DATA(pnd)->model = pn53x_usb_get_device_model(connstring);
    ...
    switch (DRIVER_DATA(pnd)->model) {
        ...
        case UNKNOWN:
            CHIP_DATA(
                    pnd)->timer_correction = 0;   // TODO: allow user to know if timed functions are available
            break;
    }
...
pn53x_usb_send(nfc_device *pnd, const uint8_t *pbtData, const size_t szData, const int timeout) {
    uint8_t abtFrame[PN53X_USB_BUFFER_LEN] = {0x00, 0x00,
    ...
    if (pn53x_check_ack_frame(pnd, abtRxBuf, res) == 0) {
        ...
        if ((res = pn53x_usb_bulk_write(DRIVER_DATA(pnd), (uint8_t *) pn53x_nack_frame,
                                        sizeof(pn53x_nack_frame), timeout)) < 0) {
            pnd->last_error = res;
            // try to interrupt current device state
            pn53x_usb_ack(pnd);
            return pnd->last_error;
...
pn53x_usb_receive(nfc_device *pnd, uint8_t *pbtData, const size_t szDataLen, const int timeout) {
    ...
    if (0 != (memcmp(abtRxBuf, pn53x_preamble, 3))) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s",
                "Frame preamble+start code mismatch");
    ...
    if ((0x01 == abtRxBuf[offset]) && (0xff == abtRxBuf[offset + 1])) {
        // Error frame
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s",
                "Application level error detected");
        pnd->last_error = NFC_EIO;
        return pnd->last_error;
...
pn53x_usb_ack(nfc_device *pnd) {
    return pn53x_usb_bulk_write(DRIVER_DATA(pnd), (uint8_t *) pn53x_ack_frame,
                                sizeof(pn53x_ack_frame), 1000);
}
    ...
    switch (DRIVER_DATA(pnd)->model) {
        case ASK_LOGO:
            if (NP_ACTIVATE_FIELD == property) {
                ...
                if ((res = pn53x_write_register(pnd, PN53X_SFR_P3, _BV(P31) | _BV(P34),
                                                bEnable ? _BV(P34) : _BV(P31))) < 0)
                    return NFC_ECHIP;
            }
            break;
        case SCM_SCL3711:
...
static int
pn53x_usb_get_supported_modulation(nfc_device *pnd, const nfc_mode mode,
