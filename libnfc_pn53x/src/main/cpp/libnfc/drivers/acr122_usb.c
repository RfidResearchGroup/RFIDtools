...
struct acr122_usb_data {
    //TODO 由上层USB接口，IN端点和OUT端点,底层只做SEND RECV FLUSH CLOSE
    //usb_dev_handle *pudh;
...
static int acr122_usb_send_apdu(nfc_device *pnd,
                                const uint8_t ins, const uint8_t p1, const uint8_t p2,
                                const uint8_t *const data, size_t data_len, const uint8_t le,
                                uint8_t *out, const size_t out_size);

static int
acr122_usb_bulk_read(struct acr122_usb_data *data, uint8_t abtRx[], const size_t szRx,
                     const int timeout) {
    //int res = usb_bulk_read(data->pudh, data->uiEndPointIn, (char *) abtRx, szRx, timeout);
    /*int res = c_read((char *) abtRx, szRx);
    if (res > 0) {
    ...
    return c_read(abtRx, szRx, timeout);
}

static int
acr122_usb_bulk_write(struct acr122_usb_data *data, uint8_t abtTx[], const size_t szTx,
                      const int timeout) {
    //LOG_HEX(NFC_LOG_GROUP_COM, "TX", abtTx, szTx);
    //int res = usb_bulk_write(data->pudh, data->uiEndPointOut, (char *) abtTx, szTx, timeout);
    /*int res = c_write((char *) abtTx, szTx);
    if (res > 0) {
    ...
    return c_write(abtTx, szTx, timeout);
}
...
acr122_usb_scan(const nfc_context *context, nfc_connstring connstrings[],
    ...
    for (bus = usb_get_busses(); bus; bus = bus->next) {
        ...
        for (dev = bus->devices; dev; dev = dev->next, uiBusIndex++) {
            for (size_t n = 0; n < sizeof(acr122_usb_supported_devices) /
                                   sizeof(struct acr122_usb_supported_device); n++) {
                if ((acr122_usb_supported_devices[n].vendor_id == dev->descriptor.idVendor) &&
                    ...
                    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                            "devices found: Bus %s Device %s Name %s", bus->dirname, dev->filename,
                            acr122_usb_supported_devices[n].name);
                    //usb_close(udev);
                    c_close();
                    snprintf(connstrings[device_found], sizeof(nfc_connstring), "%s:%s:%s",
    ...
    snprintf(connstrings[device_found], sizeof(nfc_connstring), "%s:%s:%s", ACR122_USB_DRIVER_NAME,
             "usb", "dxl");
    device_found++;
    return device_found;
}
...
/*static bool
acr122_usb_get_usb_device_name(struct usb_device *dev, usb_dev_handle *udev, char *buffer,
                               size_t len) {
    *buffer = '\0';
    ...
    *//*if (dev->descriptor.iManufacturer || dev->descriptor.iProduct) {
        if (udev) {
            ...
            usb_get_string_simple(udev, dev->descriptor.iProduct, buffer + strlen(buffer),
                                  len - strlen(buffer));
        }
    }
...
acr122_usb_open(const nfc_context *context, const nfc_connstring connstring) {
    ...
    // 获取名字，不需要（我们定义）acr122_usb_get_usb_device_name(dev, data.pudh, pnd->name, sizeof(pnd->name));
...
static int
acr122_build_frame_from_apdu(nfc_device *pnd, const uint8_t ins, const uint8_t p1, const uint8_t p2,
                             const uint8_t *data, const size_t data_len, const uint8_t le) {
    if (data_len > sizeof(DRIVER_DATA(pnd)->apdu_frame.apdu_payload))
        return NFC_EINVARG;
    if ((data == NULL) && (data_len != 0))
        return NFC_EINVARG;

    DRIVER_DATA(pnd)->apdu_frame.ccid_header.dwLength = htole32(
            data_len + sizeof(struct apdu_header));
...
acr122_build_frame_from_tama(nfc_device *pnd, const uint8_t *tama, const size_t tama_len) {
    ...
    DRIVER_DATA(pnd)->tama_frame.ccid_header.dwLength = htole32(
            tama_len + sizeof(struct apdu_header) + 1);
...
acr122_usb_send(nfc_device *pnd, const uint8_t *pbtData, const size_t szData, const int timeout) {
    ...
    if ((res = acr122_usb_bulk_write(DRIVER_DATA(pnd),
                                     (unsigned char *) &(DRIVER_DATA(pnd)->tama_frame), res,
                                     timeout)) < 0) {
        pnd->last_error = res;
        return pnd->last_error;
    }
...
acr122_usb_receive(nfc_device *pnd, uint8_t *pbtData, const size_t szDataLen, const int timeout) {
    ...
    if (res < 12) {
        ...
        // try to interrupt current devices state
        acr122_usb_ack(pnd);
    ...
    if (!((len > 1) && (abtRxBuf[10] == 0xd5))) { // In case we didn't get an immediate answer:
        ...
        if (abtRxBuf[10] != SW1_More_Data_Available) {
            if ((abtRxBuf[10] == SW1_Warning_with_NV_changed) &&
                ...
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s",
                        "PN532 has detected an error at the maps level");
            } else if ((abtRxBuf[10] == SW1_Warning_with_NV_changed) && (abtRxBuf[11] == 0x00)) {
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "%s",
                        "PN532 didn't reply");
            } else {
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                        "Unexpected Status Word (SW1: %02x SW2: %02x)", abtRxBuf[10], abtRxBuf[11]);
            }
            pnd->last_error = NFC_EIO;
            return pnd->last_error;
        }
        res = acr122_usb_send_apdu(pnd, APDU_GetAdditionnalData, 0x00, 0x00, NULL, 0, abtRxBuf[11],
    ...
    if (len > szDataLen) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                "Unable to read data: buffer too small. (szDataLen: %"
                PRIuPTR
                ", len: %"
                PRIuPTR
                ")",
...
acr122_usb_ack(nfc_device *pnd) {
    ...
    uint8_t acr122_ack_frame[] = {
            GetFirmwareVersion}; // We can't write a PN532's ACK frame, so we use a normal command to cancel current command
...
acr122_usb_send_apdu(nfc_device *pnd,
    ...
    if ((res = acr122_usb_bulk_write(DRIVER_DATA(pnd),
                                     (unsigned char *) &(DRIVER_DATA(pnd)->apdu_frame), frame_len,
                                     1000)) < 0)
        return res;
...
acr122_usb_init(nfc_device *pnd) {
    ...
    uint8_t acr122u_get_led_state_frame[] = {
            ...
            0xAA, // P2: LED state control
            0x04, // Lc
            0x00, 0x00, 0x00, 0x00, // Blinking duration control
    };

    log_put(LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s", "ACR122 Get LED state");
    if ((res = acr122_usb_bulk_write(DRIVER_DATA (pnd), (uint8_t *) acr122u_get_led_state_frame,
                                     sizeof(acr122u_get_led_state_frame), 1000)) < 0)
        return res;
    ...
    if ((res = acr122_usb_bulk_write(DRIVER_DATA(pnd), ccid_frame, sizeof(struct ccid_header),
                                     1000)) < 0)
        return res;
    if ((res = acr122_usb_bulk_read(DRIVER_DATA(pnd), abtRxBuf, sizeof(abtRxBuf), 1000)) < 0)
        return res;
