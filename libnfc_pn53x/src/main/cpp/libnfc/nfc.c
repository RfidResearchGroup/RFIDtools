...
nfc_register_driver(const struct nfc_driver *ndr) {
    if (!ndr)
        return NFC_EINVARG;
    ndr = (struct nfc_driver *) malloc(
            sizeof(struct nfc_driver));
    if (!ndr)
        return NFC_ESOFT;
    return NFC_SUCCESS;
}
...
nfc_close(nfc_device *pnd) {
    if (pnd) {
        // Close, clean up and release the devices
...
nfc_list_devices(nfc_context *context, nfc_connstring connstrings[], const size_t connstrings_len) {
    ...
    for (uint32_t i = 0; i < context->user_defined_device_count; i++) {
        if (context->user_defined_devices[i].optional) {
            // let's make sure the devices exists
            nfc_device *pnd = NULL;
            ...
            if (pnd) {
                nfc_close(pnd);
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "User devices %s found\n",
                        context->user_defined_devices[i].name);
                strcpy((char *) (connstrings + device_found),
                       context->user_defined_devices[i].connstring);
                device_found++;
...
nfc_initiator_init(nfc_device *pnd) {
    ...
    if ((res = nfc_device_set_property_bool(pnd, NP_ACTIVATE_FIELD, true)) < 0)
        return res;
    // Let the devices try forever to find a target/tag
...
nfc_initiator_list_passive_targets(nfc_device *pnd,
    ...
    while (nfc_initiator_select_passive_target(pnd, nm, pbtInitData, szInitDataLen, &nt) > 0) {
        ...
        if ((nm.nmt == NMT_FELICA) || (nm.nmt == NMT_JEWEL) || (nm.nmt == NMT_BARCODE) ||
            (nm.nmt == NMT_ISO14443BI) || (nm.nmt == NMT_ISO14443B2SR) ||
...
nfc_initiator_poll_dep_target(struct nfc_device *pnd,
    ...
    while (remaining_time > 0) {
        if ((res = nfc_initiator_select_dep_target(pnd, ndm, nbr, pndiInitiator, pnt, period)) <
            0) {
            if (res != NFC_ETIMEOUT) {
                result = res;
                goto end;
 ...
 */
int
nfc_initiator_transceive_bytes(nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx,
                               uint8_t *pbtRx,
 ...
 */
int
nfc_target_send_bits(nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits,
                     const uint8_t *pbtTxPar) {
 ...
 */
int
nfc_device_get_supported_modulation(nfc_device *pnd, const nfc_mode mode,
 ...
 */
int
nfc_device_get_supported_baud_rate(nfc_device *pnd, const nfc_modulation_type nmt,
 ...
 */
int
nfc_device_get_supported_baud_rate_target_mode(nfc_device *pnd, const nfc_modulation_type nmt,
                                               const nfc_baud_rate **const supported_br) {
