...
// A struct used to write sample-configs over USB
typedef struct {
    int8_t decimation;
...
// Operation not supported by devices    client/pm3: probably only on pm3 once client becomes universal
...
/* Set if this devices understands the extend start flash command */
#define DEVICE_INFO_FLAG_UNDERSTANDS_START_FLASH     (1<<4)

/* Set if this devices understands the chip info command */
