...
static int usage_lf_cmdread(void) {
    PrintAndLogEx(NORMAL,
                  "Usage: lf cmdread d <delay period> z <zero period> o <one period> c <cmdbytes>");
...
static int usage_lf_config(void) {
    PrintAndLogEx(NORMAL,
                  "Usage: lf config [h] [L | H | q <divisor> | f <freq>] [b <bps>] [d <decim>] [a 0|1]");
    PrintAndLogEx(NORMAL, "Options:");
    PrintAndLogEx(NORMAL, "       h                 This help");
    PrintAndLogEx(NORMAL, "       L                 Low frequency (125 kHz)");
    PrintAndLogEx(NORMAL, "       H                 High frequency (134 kHz)");
    PrintAndLogEx(NORMAL,
                  "       q <divisor>       Manually set freq divisor. %d -> 134 kHz, %d -> 125 kHz",
                  LF_DIVISOR_134, LF_DIVISOR_125);
    PrintAndLogEx(NORMAL, "       f <freq>          Manually set frequency in kHz");
    PrintAndLogEx(NORMAL,
                  "       b <bps>           Sets resolution of bits per sample. Default (max): 8");
    PrintAndLogEx(NORMAL,
                  "       d <decimate>      Sets decimation. A value of N saves only 1 in N samples. Default: 1");
    PrintAndLogEx(NORMAL,
                  "       a [0|1]           Averaging - if set, will average the stored sample value when decimating. Default: 1");
    PrintAndLogEx(NORMAL,
                  "       t <threshold>     Sets trigger threshold. 0 means no threshold (range: 0-128)");
    PrintAndLogEx(NORMAL,
                  "       s <samplestoskip> Sets a number of samples to skip before capture. Default: 0");
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL, "      lf config              - shows current config");
    PrintAndLogEx(NORMAL, "      lf config b 8 L        - samples at 125 kHz, 8bps.");
    PrintAndLogEx(NORMAL,
                  "      lf config H b 4 d 3    - samples at 134 kHz, averages three samples into one, stored with ");
...
static int usage_lf_simfsk(void) {
    PrintAndLogEx(NORMAL,
                  "Usage: lf simfsk [h] [c <clock>] [H <fcHigh>] [L <fcLow>] [d <hexdata>]");
    ...
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL,
                  "       lf simfsk c 40 H 8 L 5 d 010203      -  FSK1  rf/40  data 010203");
    PrintAndLogEx(NORMAL,
                  "       lf simfsk c 40 H 5 L 8 d 010203      -  FSK1a rf/40  data 010203");
    PrintAndLogEx(NORMAL,
                  "       lf simfsk c 64 H 10 L 8 d 010203     -  FSK2  rf/64  data 010203");
    PrintAndLogEx(NORMAL,
                  "       lf simfsk c 64 H 8 L 10 d 010203     -  FSK2a rf/64  data 010203");
    PrintAndLogEx(NORMAL, "");
    return PM3_SUCCESS;
}
...
static int usage_lf_simpsk(void) {
    PrintAndLogEx(NORMAL,
                  "Usage: lf simpsk [1|2|3] [c <clock>] [i] [r <carrier>] [d <raw hex to sim>]");
    PrintAndLogEx(NORMAL, "Options:");
    PrintAndLogEx(NORMAL, "       h              This help");
    PrintAndLogEx(NORMAL,
                  "       c <clock>      Manually set clock - can autodetect if using DemodBuffer");
...
static int usage_lf_find(void) {
    ...
    PrintAndLogEx(NORMAL, "       h             This help");
    PrintAndLogEx(NORMAL,
                  "       <0|1>         Use data from Graphbuffer, if not set, try reading data from tag.");
    PrintAndLogEx(NORMAL,
                  "       u             Search for Unknown tags, if not set, reads only known tags.");
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL,
                  "      lf search     = try reading data from tag & search for known tags");
    PrintAndLogEx(NORMAL,
                  "      lf search 1   = use data from GraphBuffer & search for known tags");
    PrintAndLogEx(NORMAL,
                  "      lf search u   = try reading data from tag & search for known and unknown tags");
    PrintAndLogEx(NORMAL,
                  "      lf search 1 u = use data from GraphBuffer & search for known and unknown tags");
    return PM3_SUCCESS;
}
...
static int usage_lf_tune(void) {
    ...
    PrintAndLogEx(NORMAL, "       q <divisor>   - Frequency divisor. %d -> 134 kHz, %d -> 125 kHz",
                  LF_DIVISOR_134, LF_DIVISOR_125);
...
static int CmdLFTune(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "Measuring LF antenna at %.2f kHz, click button or press Enter to exit",
                  LF_DIV2FREQ(divisor));
...
int CmdLFfskSim(const char *Cmd) {
    ...
    if (size > (PM3_CMD_DATA_SIZE - sizeof(lf_fsksim_t))) {
        PrintAndLogEx(WARNING,
                      "DemodBuffer too long for current implementation - length: %zu - max: %zu",
...
// DXL, The lf search plus version!
int CmdLFSearchPlus(const char *Cmd) {
    int retval = PM3_SUCCESS;
    // we have some default divisor values
    ...
    // 31 -> 375.00
    // 23 -> 500.00
    int default_divisor[] = {95, 88, 47, 31, 23};
    ...
    // Iteration defaults
    for (int i = 0; i < sizeof(default_divisor) / sizeof(int); ++i) {
        if (kbd_enter_pressed()) {
            PrintAndLogEx(NORMAL, "%s", "Keyboard pressed Done.");
            recovery_divisor();
            return retval;
        }
        // Try to change config!
        config.divisor = (int16_t) default_divisor[i];
        if (lf_config(&config) == PM3_SUCCESS) {
    ...
    // Iterate all values(from 19 to 255)
    for (int i = 19; i <= 255; ++i) {
        ...
        for (int j = 0; j < sizeof(default_divisor) / sizeof(int); ++j) {
            if (i == default_divisor[i]) {
                continue;
            }
        }
        config.divisor = (int16_t) i;
        if (lf_config(&config) == PM3_SUCCESS) {
            retval = CmdLFfind(Cmd);
            if (retval == PM3_SUCCESS) {
                break;
...
//    if (demodTI() == PM3_SUCCESS)              { PrintAndLogEx(SUCCESS, "\nValid " _GREEN_("Texas Instrument ID") "found!"); goto out;}
    ...
    if (testRaw == 'u') {
        ...
        if (ASKDemod_ext("0 0 0", true, false, 1, &st) == PM3_SUCCESS) {
            ...
            PrintAndLogEx(NORMAL,
                          "if it does not look right it could instead be ASK/Biphase - try " _YELLOW_(
                                  "'data rawdemod ab'"));
            goto out;
        }
...
static command_t CommandTable[] = {
        ...
        {"search",    CmdLFfind,        AlwaysAvailable,
                                                         "Read and Search for valid known tag (in offline mode it you can load first then search)"},
        {"search++",  CmdLFSearchPlus,  IfPm3Lf,         "You can automatically iterate the divisor and execute the LF search command, and the iteration will end after the card is successfully searched."},
