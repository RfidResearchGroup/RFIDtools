...
static int CmdEMVExec(const char *Cmd) {
    ...
    if (!AIDlen) {
        ...
        // check search and select maps id
        TLVPrintAIDlistFromSelectTLV(tlvSelect);
        EMVSelectApplication(tlvSelect, AID, &AIDlen);
    }
    ...
    // check if we found EMV maps on card
    if (!AIDlen) {
    ...
    if (GetCardPSVendor(AID, AIDlen) == CV_VISA && (TrType == TT_VSDC || TrType == TT_CDA)) {
        ...
        // needs to write AC2 command (res == ARQC)
...
static command_t CommandTable[] =  {
    ...
    {"eload",       CmdEmvELoad,                    IfPm3Iso14443,   "load EMV tag into devices"},
