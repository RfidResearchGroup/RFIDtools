...
static bool dl_it(uint8_t *dest, uint32_t bytes, PacketResponseNG *response, size_t ms_timeout,
                  bool show_warning, uint32_t rec_cmd);
...
void
SendCommandBL(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2, void *data, size_t len) {
    SendCommandOLD(cmd, arg0, arg1, arg2, data, len);
}

void
SendCommandOLD(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2, void *data, size_t len) {
    ...
    **/
    ...
    // tell communication thread that a new command can be write
...
static void SendCommandNG_internal(uint16_t cmd, uint8_t *data, size_t len, bool ng) {
    ...
    PacketCommandNGPostamble *tx_post = (PacketCommandNGPostamble *) ((uint8_t *) &txBufferNG +
    ...
    **/
    ...
    if ((conn.send_via_fpc_usart && conn.send_with_crc_on_fpc) ||
        ...
        compute_crc(CRC_14443_A, (uint8_t *) &txBufferNG, sizeof(PacketCommandNGPreamble) + len,
                    &first, &second);
...
void
SendCommandMIX(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2, void *data, size_t len) {
    uint64_t arg[3] = {arg0, arg1, arg2};
    if (len > PM3_CMD_DATA_SIZE_MIX) {
        PrintAndLogEx(WARNING, "Sending %zu bytes of payload is too much for MIX frames, abort",
                      len);
        return;
    }
...
*uart_communication(void *targ) {
    ...
    while (connection->run) {
        ...
        if ((res == PM3_SUCCESS) && (rxlen == sizeof(PacketResponseNGPreamble))) {
            ...
            if (rx.magic == RESPONSENG_PREAMBLE_MAGIC) { // New style NG reply
                if (length > PM3_CMD_DATA_SIZE) {
                    PrintAndLogEx(WARNING, "Received packet frame with incompatible length: 0x%04x",
                                  length);
                    error = true;
                }
                    ...
                    if ((res != PM3_SUCCESS) || (rxlen != length)) {
                        ...
                        if (rx.ng) {      // Received a valid NG frame
                            ...
                            if (length < sizeof(arg)) {
                                PrintAndLogEx(WARNING,
                                              "Received MIX packet frame with incompatible length: 0x%04x",
                                              length);
                                error = true;
                            }
                ...
                if (!error) {                        // Check CRC, accept MAGIC as placeholder
                    ...
                    if (rx.crc != RESPONSENG_POSTAMBLE_MAGIC) {
                        ...
                        compute_crc(CRC_14443_A, (uint8_t *) &rx_raw,
                                    sizeof(PacketResponseNGPreamble) + length, &first, &second);
                ...
                res = uart_receive(sp, ((uint8_t *) &rx_old) + sizeof(PacketResponseNGPreamble),
            ...
            if (rxlen > 0) {
                PrintAndLogEx(WARNING, "Received packet frame preamble too short: %d/%zu", rxlen,
                              sizeof(PacketResponseNGPreamble));
                error = true;
            }
            if (res == PM3_ENOTTY) {
                LOGD("commfailed on 471");
                commfailed = true;
            }
        }
        ...
        if (txBuffer_pending) {
            ...
            // main thread doesn't know write failed...
...
bool OpenProxmark(void *port, bool wait_for_port, int timeout, bool flash_mode, uint32_t speed) {
    ...
    if (!wait_for_port) {
        PrintAndLogEx(INFO, "Using UART port  %s", portname);
    ...
    if (sp == INVALID_SERIAL_PORT) {
        ...
        PrintAndLogEx(WARNING, "\n" _RED_("ERROR:") "serial port " _YELLOW_(
                "%s") " is claimed by another process", portname);
        sp = NULL;
        return false;
    } else {
...
int TestProxmark(void) {
    ...
    if ((resp.length != sizeof(pm3_capabilities)) ||
 ...
 */
bool WaitForResponseTimeoutW(uint32_t cmd, PacketResponseNG *response, size_t ms_timeout,
                             bool show_warning) {
...
* @return true if command was returned, otherwise false
*/
bool GetFromDevice(DeviceMemType_t memtype, uint8_t *dest, uint32_t bytes, uint32_t start_index,
                   uint8_t *data, uint32_t datalen, PacketResponseNG *response, size_t ms_timeout,
                   bool show_warning) {
        ...
        if (getReply(response)) {
            ...
            if (response->cmd == rec_cmd) {
                ...
                if (offset + copy_bytes > bytes) {
                    PrintAndLogEx(FAILED,
                                  "ERROR: Out of bounds when downloading from device,  offset %u | len %u | total len %u > buf_size %u",
