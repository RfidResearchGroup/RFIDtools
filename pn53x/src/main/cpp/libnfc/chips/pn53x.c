...
const nfc_baud_rate pn533_iso14443a_supported_baud_rates[] = {NBR_847, NBR_424, NBR_212, NBR_106,
                                                              0};
...
const nfc_baud_rate pn533_iso14443b_supported_baud_rates[] = {NBR_847, NBR_424, NBR_212, NBR_106,
                                                              0};
const nfc_modulation_type pn53x_supported_modulation_as_target[] = {NMT_ISO14443A, NMT_FELICA,
                                                                    NMT_DEP, 0};
...
pn53x_init(struct nfc_device *pnd) {
    ...
    if (!CHIP_DATA(pnd)->supported_modulation_as_initiator) {
        CHIP_DATA(pnd)->supported_modulation_as_initiator = malloc(
                sizeof(nfc_modulation_type) * (NMT_DEP + 1));
        if (!CHIP_DATA(pnd)->supported_modulation_as_initiator)
            return NFC_ESOFT;
        int nbSupportedModulation = 0;
        if ((pnd->btSupportByte & SUPPORT_ISO14443A)) {
            CHIP_DATA(
                    pnd)->supported_modulation_as_initiator[nbSupportedModulation] = NMT_ISO14443A;
        ...
        if (pnd->btSupportByte & SUPPORT_ISO14443B) {
            CHIP_DATA(
                    pnd)->supported_modulation_as_initiator[nbSupportedModulation] = NMT_ISO14443B;
            nbSupportedModulation++;
            CHIP_DATA(
                    pnd)->supported_modulation_as_initiator[nbSupportedModulation] = NMT_ISO14443BI;
            nbSupportedModulation++;
            CHIP_DATA(
                    pnd)->supported_modulation_as_initiator[nbSupportedModulation] = NMT_ISO14443B2SR;
            nbSupportedModulation++;
            CHIP_DATA(
                    pnd)->supported_modulation_as_initiator[nbSupportedModulation] = NMT_ISO14443B2CT;
            nbSupportedModulation++;
        }
    ...
    if (!CHIP_DATA(pnd)->supported_modulation_as_target) {
        CHIP_DATA(
                pnd)->supported_modulation_as_target = (nfc_modulation_type *) pn53x_supported_modulation_as_target;
    }
...
pn53x_reset_settings(struct nfc_device *pnd) {
    ...
    if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_BitFraming, SYMBOL_TX_LAST_BITS, 0x00)) <
        0) {
        return res;
    }
...
int
pn53x_transceive(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx,
    ...
    if (timeout > 0) {
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "Invalid timeout value: %d",
                timeout);
    }
    ...
    // Call the write/receice callback functions of the current driver
    ...
    if ((CHIP_DATA(pnd)->type == PN532) &&
        ...
        CHIP_DATA(
                pnd)->power_mode = NORMAL; // When TgInitAsTarget reply that means an external RF have waken up the chip
    }
...
pn53x_set_parameters(struct nfc_device *pnd, const uint8_t ui8Parameter, const bool bEnable) {
    uint8_t ui8Value = (bEnable) ? (CHIP_DATA(pnd)->ui8Parameters | ui8Parameter) : (
...
pn53x_set_tx_bits(struct nfc_device *pnd, const uint8_t ui8Bits) {
    ...
    if (CHIP_DATA(pnd)->ui8TxBits != ui8Bits) {
        ...
        if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_BitFraming, SYMBOL_TX_LAST_BITS,
                                        ui8Bits)) < 0)
            return res;
...
int
pn53x_unwrap_frame(const uint8_t *pbtFrame, const size_t szFrameBits, uint8_t *pbtRx,
                   uint8_t *pbtRxPar) {
    uint8_t btFrame;
    uint8_t btData;
    uint8_t uiBitPos;
...
int
pn53x_decode_target_data(const uint8_t *pbtRawData, size_t szRawData, pn53x_type type,
        ...
        case NMT_ISO14443A:
            ...
            if (szRawData > (pnti->nai.szUidLen + 5)) {
                pnti->nai.szAtsLen = ((*(pbtRawData++)) -
                                      1);     // In pbtRawData, ATS Length byte is counted in ATS Frame.
...
static int
pn53x_WriteRegister(struct nfc_device *pnd, const uint16_t ui16RegisterAddress,
                    const uint8_t ui8Value) {
    uint8_t abtCmd[] = {WriteRegister, ui16RegisterAddress >> 8, ui16RegisterAddress & 0xff,
                        ui8Value};
    PNREG_TRACE(ui16RegisterAddress);
...
int
pn53x_write_register(struct nfc_device *pnd, const uint16_t ui16RegisterAddress,
    ...
    if ((ui16RegisterAddress < PN53X_CACHE_REGISTER_MIN_ADDRESS) ||
        ...
        if (ui8SymbolMask != 0xff) {
            ...
            if ((res = pn53x_read_register(pnd, ui16RegisterAddress, &ui8CurrentValue)) < 0)
                return res;
            uint8_t ui8NewValue = ((ui8Value & ui8SymbolMask) |
        ...
        CHIP_DATA(pnd)->wb_data[internal_address] = (CHIP_DATA(pnd)->wb_data[internal_address] &
                                                     CHIP_DATA(pnd)->wb_mask[internal_address] &
                                                     (~ui8SymbolMask)) | (ui8Value & ui8SymbolMask);
        CHIP_DATA(pnd)->wb_mask[internal_address] =
                CHIP_DATA(pnd)->wb_mask[internal_address] | ui8SymbolMask;
...
pn53x_writeback_register(struct nfc_device *pnd) {
    ...
    if (BUFFER_SIZE(abtReadRegisterCmd) > 1) {
        ...
        if ((res = pn53x_transceive(pnd, abtReadRegisterCmd, BUFFER_SIZE(abtReadRegisterCmd),
                                    abtRes, szRes, -1)) < 0) {
            return res;
        }
        size_t i = 0;
        ...
        for (size_t n = 0; n < PN53X_CACHE_REGISTER_SIZE; n++) {
            if ((CHIP_DATA(pnd)->wb_mask[n]) && (CHIP_DATA(pnd)->wb_mask[n] != 0xff)) {
                CHIP_DATA(pnd)->wb_data[n] = (
                        (CHIP_DATA(pnd)->wb_data[n] & CHIP_DATA(pnd)->wb_mask[n]) |
    ...
    if (BUFFER_SIZE(abtWriteRegisterCmd) > 1) {
        ...
        if ((res = pn53x_transceive(pnd, abtWriteRegisterCmd, BUFFER_SIZE(abtWriteRegisterCmd),
                                    NULL, 0, -1)) < 0) {
            return res;
        }
    }
...
pn53x_decode_firmware_version(struct nfc_device *pnd) {
    ...
    switch (CHIP_DATA(pnd)->type) {
        case PN531:
            snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text),
                     "PN531 v%d.%d", abtFw[0], abtFw[1]);
            pnd->btSupportByte = SUPPORT_ISO14443A | SUPPORT_ISO18092;
            break;
        case PN532:
            snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text),
                     "PN532 v%d.%d", abtFw[1], abtFw[2]);
            pnd->btSupportByte = abtFw[3];
            break;
        case PN533:
        case RCS360:
            snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text),
                     "PN533 v%d.%d", abtFw[1], abtFw[2]);
            pnd->btSupportByte = abtFw[3];
            break;
        case PN53X:
            // Could not happend
...
pn53x_set_property_int(struct nfc_device *pnd, const nfc_property property, const int value) {
    switch (property) {
        ...
        case NP_TIMEOUT_ATR:
            ...
            return pn53x_RFConfiguration__Various_timings(pnd, pn53x_int_to_timeout(
                    CHIP_DATA(pnd)->timeout_atr), pn53x_int_to_timeout(
                    CHIP_DATA(pnd)->timeout_communication));
        case NP_TIMEOUT_COM:
...
pn53x_set_property_bool(struct nfc_device *pnd, const nfc_property property, const bool bEnable) {
    ...
    switch (property) {
        case NP_HANDLE_CRC:
            ...
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_TxMode, SYMBOL_TX_CRC_ENABLE,
                                            btValue)) < 0)
                return res;
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_RxMode, SYMBOL_RX_CRC_ENABLE,
                                            btValue)) < 0)
                return res;
            pnd->bCrc = bEnable;
            return NFC_SUCCESS;
        ...
        case NP_HANDLE_PARITY:
            ...
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_ManualRCV, SYMBOL_PARITY_DISABLE,
                                            btValue)) < 0)
                return res;
            pnd->bPar = bEnable;
            return NFC_SUCCESS;
        ...
        case NP_INFINITE_SELECT:
            ...
            return pn53x_RFConfiguration__MaxRetries(pnd,
                                                     ...
                                                     (bEnable) ? 0xff
                                                               : 0x02         // MxRtyPassiveActivation, default: 0xff (0x00 leads to problems with PN531)
            );
        ...
        case NP_FORCE_ISO14443_A:
            ...
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_TxMode, SYMBOL_TX_FRAMING, 0x00)) <
                0) {
                return res;
            }
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_RxMode, SYMBOL_RX_FRAMING, 0x00)) <
                0) {
                return res;
            }
        ...
        case NP_FORCE_ISO14443_B:
            ...
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_TxMode, SYMBOL_TX_FRAMING, 0x03)) <
                0) {
                return res;
            }
        ...
        case NP_FORCE_SPEED_106:
            ...
            if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_TxMode, SYMBOL_TX_SPEED, 0x00)) <
                0) {
                return res;
            }
...
pn53x_initiator_select_passive_target_ext(struct nfc_device *pnd,
    ...
    if (nm.nmt == NMT_ISO14443BI || nm.nmt == NMT_ISO14443B2SR || nm.nmt == NMT_ISO14443B2CT) {
        if (CHIP_DATA(pnd)->type == RCS360) {
            // TODO add support for RC-S360, at the moment it refuses to write raw frames without a first select
        ...
        do {
            if (nm.nmt == NMT_ISO14443B2SR) {
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtInitiate, szInitiateLen, abtRx,
                                                            sizeof(abtRx), timeout)) < 0) {
                    if ((res == NFC_ERFTRANS) &&
                        (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                        continue;
                    } else
                        return res;
                }
                abtSelect[1] = abtRx[0];
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtSelect, sizeof(abtSelect),
                                                            abtRx, sizeof(abtRx), timeout)) < 0) {
                    return res;
                }
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt),
                                                            abtTargetsData + 2,
                                                            sizeof(abtTargetsData) - 2, timeout)) <
                    0) {
                    if ((res == NFC_ERFTRANS) &&
                        (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                        continue;
            ...
            if ((res = pn53x_initiator_transceive_bytes(pnd, pbtInitData, szInitData,
                                                        abtTargetsData, sizeof(abtTargetsData),
                                                        timeout)) < 0) {
                if ((res == NFC_ERFTRANS) &&
                    (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                    continue;
            ...
            if (nm.nmt == NMT_ISO14443B2CT) {
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtRead, sizeof(abtRead),
                                                            abtTargetsData + 4,
                                                            sizeof(abtTargetsData) - 4, timeout)) <
                    0) {
                    return res;
                }
            ...
            nttmp.nm = nm;
            if ((res = pn53x_decode_target_data(abtTargetsData, szTargetsData, CHIP_DATA(pnd)->type,
                                                nm.nmt, &(nttmp.nti))) < 0) {
                return res;
            }
            if (nm.nmt == NMT_ISO14443BI) {
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtAttrib, sizeof(abtAttrib), NULL,
                                                            0, timeout)) < 0) {
                    return res;
                }
                szTargetsData = (size_t) res;
        ...
        do {
            ...
            if ((res = nfc_initiator_transceive_bits(pnd, NULL, 0, NULL, abtRx, sizeof(abtRx),
                                                     abtRxPar)) < 0) {
        ...
        if ((res = pn53x_InListPassiveTarget(pnd, pm, 1, pbtInitData, szInitData, abtTargetsData,
                                             &szTargetsData, timeout)) <= 0)
            return res;
        ...
        nttmp.nm = nm;
        if ((res = pn53x_decode_target_data(abtTargetsData + 1, szTargetsData - 1,
...
pn53x_initiator_poll_target(struct nfc_device *pnd,
    ...
    if (CHIP_DATA(pnd)->type == PN532) {
        ...
        if ((res = pn53x_InAutoPoll(pnd, apttTargetTypes, szTargetTypes, uiPollNr, uiPeriod,
                                    ntTargets, 0)) < 0)
            return res;
        switch (res) {
            case 0:
        ...
        do {
            for (size_t p = 0; p < uiPollNr; p++) {
                for (size_t n = 0; n < szModulations; n++) {
                    ...
                    if ((res = pn53x_initiator_select_passive_target_ext(pnd, pnmModulations[n],
                                                                         pbtInitiatorData,
                                                                         szInitiatorData, pnt,
                                                                         timeout_ms)) < 0) {
...
pn53x_initiator_select_dep_target(struct nfc_device *pnd,
    ...
    const uint8_t abtPassiveInitiatorData[] = {0x00, 0xff, 0xff, 0x00,
                                               0x0f}; // Only for 212/424 kpbs: First 4 bytes shall be set like this according to NFCIP-1, last byte is TSN (Time Slot Number)
    ...
    int res;
    if (pndiInitiator) {
        res = pn53x_InJumpForDEP(pnd, ndm, nbr, pbtPassiveInitiatorData, pndiInitiator->abtNFCID3,
                                 pndiInitiator->abtGB, pndiInitiator->szGB, pnt, timeout);
    } else {
        res = pn53x_InJumpForDEP(pnd, ndm, nbr, pbtPassiveInitiatorData, NULL, NULL, 0, pnt,
                                 timeout);
    }
    if (res > 0) {
        if (pn53x_current_target_new(pnd, pnt) == NULL) {
...
int
pn53x_initiator_transceive_bytes(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx,
                                 uint8_t *pbtRx,
    ...
    // We can not just write bytes without parity if while the PN53X expects we handled them
    if (!pnd->bPar) {
        pnd->last_error = NFC_EINVARG;
        return pnd->last_error;
    }
    ...
    uint8_t abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
    if ((res = pn53x_transceive(pnd, abtCmd, szTx + szExtraTxLen, abtRx, sizeof(abtRx), timeout)) <
        0) {
        pnd->last_error = res;
        return pnd->last_error;
    }
    ...
    if (pbtRx != NULL) {
        if (szRxLen > szRx) {
            log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                    "Buffer size is too short: %" PRIuPTR " available(s), %" PRIuPTR " needed",
                    szRx, szRxLen);
            return NFC_EOVFLOW;
        }
        // Copy the received bytes
...
// prescaler = 10 => precision: ~1.5us  timer saturates at  ~100ms
    ...
    // Initialize timer
    pn53x_write_register(pnd, PN53X_REG_CIU_TMode, 0xFF, SYMBOL_TAUTO |
                                                         ((CHIP_DATA(pnd)->timer_prescaler >> 8) &
                                                          SYMBOL_TPRESCALERHI));
    pn53x_write_register(pnd, PN53X_REG_CIU_TPrescaler, 0xFF,
                         (CHIP_DATA(pnd)->timer_prescaler & SYMBOL_TPRESCALERLO));
...
static uint32_t __pn53x_get_timer(struct nfc_device *pnd, const uint8_t last_cmd_byte) {
    ...
    uint8_t abtRes[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
    size_t szRes = sizeof(abtRes);
    // Let's write the previously constructed ReadRegister command
    ...
    if (counter == 0) {
        ...
        // Correction depending on devices design
...
int
pn53x_initiator_transceive_bits_timed(struct nfc_device *pnd, const uint8_t *pbtTx,
    ...
    // Let's write the previously constructed WriteRegister command
    if ((res = pn53x_transceive(pnd, abtWriteRegisterCmd, BUFFER_SIZE(abtWriteRegisterCmd), NULL, 0,
                                -1)) < 0) {
        return res;
    }
...
int
pn53x_initiator_transceive_bytes_timed(struct nfc_device *pnd, const uint8_t *pbtTx,
                                       const size_t szTx, uint8_t *pbtRx, const size_t szRx,
                                       uint32_t *cycles) {
    uint16_t i;
    uint8_t sz = 0;
    int res = 0;

    // We can not just write bytes without parity while the PN53X expects we handled them
    if (!pnd->bPar) {
        pnd->last_error = NFC_EINVARG;
        return pnd->last_error;
    }
    ...
    if (pnd->bCrc) {
        ...
        if ((txmode & SYMBOL_TX_FRAMING) == 0x00)
            ...
            log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                    "Unsupported framing type %02X, cannot adjust CRC cycles",
...
static int pn53x_Diagnose06(struct nfc_device *pnd) {
    ...
    while (failures < 2) {
        if ((ret = pn53x_transceive(pnd, abtCmd, sizeof(abtCmd), abtRx, sizeof(abtRx), 1000)) !=
            1) {
...
static int pn53x_ISO14443A_4_is_present(struct nfc_device *pnd) {
    ...
    if (CHIP_DATA(pnd)->type == PN533) {
        ...
        int failures = 0;
        while (failures < 2) {
            if ((ret = nfc_initiator_transceive_bytes(pnd, abtCmd, sizeof(abtCmd), NULL, 0, 300)) <
                1) {
                if ((ret == NFC_ERFTRANS) &&
                    (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Timeout
                    ret = NFC_ETGRELEASED;
                    break;
                } else { // Other errors can appear when card is tired-off, let's try again
                    failures++;
...
static int pn53x_ISO14443A_Barcode_is_present(struct nfc_device *pnd) {
    int ret;
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
            "target_is_present(): Ping Barcode");
    ...
    while (failures < 3) {
        ...
        uint8_t abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
        uint8_t abtRxPar[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
        if ((ret = nfc_initiator_transceive_bits(pnd, NULL, 0, NULL, abtRx, sizeof(abtRx),
                                                 abtRxPar)) < 1) {
            failures++;
        } else {
            nfc_device_set_property_bool(pnd, NP_HANDLE_CRC, true);
...
static int pn53x_ISO14443A_MFUL_is_present(struct nfc_device *pnd) {
    ...
    if (CHIP_DATA(pnd)->type == PN533) {
        ...
        int failures = 0;
        while (failures < 2) {
            if ((ret = nfc_initiator_transceive_bytes(pnd, abtCmd, sizeof(abtCmd), NULL, 0, -1)) <
                1) {
                if ((ret == NFC_ERFTRANS) &&
                    (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Timeout
                    return NFC_ETGRELEASED;
...
static int pn53x_ISO14443A_MFC_is_present(struct nfc_device *pnd) {
    ...
    if ((CHIP_DATA(pnd)->type == PN533) &&
        (CHIP_DATA(pnd)->current_target->nti.nai.btSak != 0x09)) {
        ...
        if ((ret = pn53x_initiator_select_passive_target_ext(pnd,
                                                             CHIP_DATA(pnd)->current_target->nm,
                                                             pbtInitiatorData, szInitiatorData,
                                                             NULL, 300)) == 1) {
            ret = NFC_SUCCESS;
...
static int pn53x_DEP_is_present(struct nfc_device *pnd) {
    ...
    if ((CHIP_DATA(pnd)->type == PN531) || (CHIP_DATA(pnd)->type == PN532) ||
        (CHIP_DATA(pnd)->type == PN533))
        ret = pn53x_Diagnose06(pnd);
    else
        ret = NFC_EDEVNOTSUPP;
...
static int pn53x_ISO14443B_4_is_present(struct nfc_device *pnd) {
    ...
    if (CHIP_DATA(pnd)->type ==
        PN533) { // Not supported on PN532 even if the doc is same as for PN533
        ret = pn53x_Diagnose06(pnd);
    } else {
...
static int pn53x_ISO14443B_SR_is_present(struct nfc_device *pnd) {
    int ret;
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
            "target_is_present(): Ping B2 ST SRx");
...
static int pn53x_ISO14443B_CT_is_present(struct nfc_device *pnd) {
    int ret;
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
            "target_is_present(): Ping B2 ASK CTx");
...
pn53x_initiator_target_is_present(struct nfc_device *pnd, const nfc_target *pnt) {
    ...
    if (CHIP_DATA(pnd)->current_target == NULL) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
                "target_is_present(): no saved target");
    ...
    if ((pnt != NULL) && (!pn53x_current_target_is(pnd, pnt))) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
                "target_is_present(): another target");
    ...
    switch (CHIP_DATA(pnd)->current_target->nm.nmt) {
        case NMT_ISO14443A:
            ...
            }/* else if(CHIP_DATA(pnd)->current_target->nti.nai.btSak & 0x28) {
                ret = pn53x_ISO14443A_MFC_is_present(pnd);
            } */else {
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s",
...
int
pn53x_target_init(struct nfc_device *pnd, nfc_target *pnt, uint8_t *pbtRx, const size_t szRxLen,
                  int timeout) {
    pn53x_reset_settings(pnd);
    ...
    switch (pnt->nm.nmt) {
        case NMT_ISO14443A: {
            ...
            // Historical Bytes
            pbtTkt = iso14443a_locate_historical_bytes(pnt->nti.nai.abtAts, pnt->nti.nai.szAtsLen,
                                                       &szTkt);
        }
            break;
    ...
    while (!targetActivated) {
        ...
        if ((res = pn53x_TgInitAsTarget(pnd, ptm, pbtMifareParams, pbtTkt, szTkt, pbtFeliCaParams,
                                        pbtNFCID3t, pbtGBt, szGBt, pbtRx, szRxLen, &btActivatedMode,
                                        timeout)) < 0) {
            if (res == NFC_ETIMEOUT) {
                pn53x_idle(pnd);
            }
        ...
        if (pnt->nm.nmt == nm.nmt) { // Actual activation have the right modulation type
            if ((pnt->nm.nbr == NBR_UNDEFINED) ||
                (pnt->nm.nbr == nm.nbr)) { // Have the right baud rate (or undefined)
...
int
pn53x_target_receive_bits(struct nfc_device *pnd, uint8_t *pbtRx, const size_t szRxLen,
                          uint8_t *pbtRxPar) {
    size_t szRxBits = 0;
...
int
pn53x_target_receive_bytes(struct nfc_device *pnd, uint8_t *pbtRx, const size_t szRxLen,
                           int timeout) {
    uint8_t abtCmd[1];
...
int
pn53x_target_send_bits(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits,
    ...
    // Try to write the bits to the reader
...
int
pn53x_target_send_bytes(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx,
                        int timeout) {
    uint8_t abtCmd[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
    int res = 0;
...
static struct sErrorMessage {
    ...
    const char *pcErrorMsg;
        ...
        {EDEPINVSTATE, "Invalid DEP State"},  // DEP Protocol: Invalid devices state, the system is in a state which does not allow the operation
...
int
pn53x_RFConfiguration__Various_timings(struct nfc_device *pnd, const uint8_t fATR_RES_Timeout,
...
int
pn53x_RFConfiguration__MaxRetries(struct nfc_device *pnd, const uint8_t MxRtyATR,
                                  const uint8_t MxRtyPSL, const uint8_t MxRtyPassiveActivation) {
...
pn53x_InListPassiveTarget(struct nfc_device *pnd,
    ...
    int res = 0;
    if ((res = pn53x_transceive(pnd, abtCmd, 3 + szInitiatorData, pbtTargetsData, *pszTargetsData,
                                timeout)) < 0) {
        return res;
    }
...
pn53x_InDeselect(struct nfc_device *pnd, const uint8_t ui8Target) {
    if (CHIP_DATA(pnd)->type == RCS360) {
        ...
        int res = 0;
        if ((res = pn53x_transceive(pnd, abtCmdGetStatus, sizeof(abtCmdGetStatus), abtStatus,
                                    szStatus, -1)) < 0) {
            return res;
        }
        szStatus = (size_t) res;
    ...
    if (res < 0) {
        ...
        if (szTargetFound > 0) {
            ...
            if (abtRx[0] > 1) {
                ...
                // AutoPollTargetData length
                ln = *(pbt++);
                pn53x_decode_target_data(pbt, ln, CHIP_DATA(pnd)->type, pntTargets[1].nm.nmt,
                                         &(pntTargets[1].nti));
            }
        }
    }
...
pn53x_InJumpForDEP(struct nfc_device *pnd,
    ...
    switch (nbr) {
        case NBR_106:
            ...
            if (pbtPassiveInitiatorData && (ndm ==
                                            NDM_PASSIVE)) {        /* can't have passive initiator data when using active mode */
                abtCmd[3] |= 0x01;
    ...
    size_t szRx = sizeof(abtRx);
    int res = 0;
    // Try to find a target, call the transceive callback function of the current devices
...
pn53x_TgInitAsTarget(struct nfc_device *pnd, pn53x_target_mode ptm,
    ...
    uint8_t abtCmd[39 + 47 + 48] = {
            TgInitAsTarget}; // Worst case: 39-byte base, 47 bytes max. for General Bytes, 48 bytes max. for Historical Bytes
    ...
    if ((CHIP_DATA(pnd)->type != PN531) &&
        (CHIP_DATA(pnd)->type != RCS360)) { // PN531 does not handle Historical Bytes
...
int
pn53x_check_error_frame(struct nfc_device *pnd, const uint8_t *pbtRxFrame,
                        const size_t szRxFrameLen) {
 ...
 */
int
pn53x_build_frame(uint8_t *pbtFrame, size_t *pszFrame, const uint8_t *pbtData,
                  const size_t szData) {
    if (szData <= PN53x_NORMAL_FRAME__DATA_MAX_LEN) {
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                "We can't write more than %d bytes in a raw (requested: %" PRIdPTR ")",
...
int
pn53x_get_supported_baud_rate(nfc_device *pnd, const nfc_mode mode, const nfc_modulation_type nmt,
...
pn53x_get_information_about(nfc_device *pnd, char **pbuf) {
    ...
    *pbuf = malloc(buflen);
    ...
    for (int i = 0; nmt[i]; i++) {
        if ((res = snprintf(buf, buflen, "%s%s (", (i == 0) ? "" : ", ",
        ...
        for (int j = 0; nbr[j]; j++) {
            if ((res = snprintf(buf, buflen, "%s%s", (j == 0) ? "" : ", ",
...
pn53x_data_new(struct nfc_device *pnd, const struct pn53x_io *io) {
    ...
    // Set power mode to normal, if your devices starts in LowVBat (ie. PN532
    // Com) the driver layer have to correctly set it.
