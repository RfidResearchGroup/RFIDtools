...
nfc_context_new(void) {
...
#else
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "log_level is set to %d",
            res->log_level);
#endif
    ...
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%d devices(s) defined by user",
            res->user_defined_device_count);
    for (uint32_t i = 0; i < res->user_defined_device_count; i++) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                "  #%d name: \"%s\", connstring: \"%s\"", i, res->user_defined_devices[i].name,
...
void
prepare_initiator_data(const nfc_modulation nm, uint8_t **ppbtInitiatorData,
