...
static int l_WaitForResponseTimeout(lua_State *L) {
    ...
    if (WaitForResponseTimeout(cmd, &response, ms_timeout)) {
        ...
        //signal error by returning Nil, errorstring
        lua_pushnil(L);
        lua_pushstring(L, "No response from the devices");
        return 2;
    }
}
...
static int l_mfDarkside(lua_State *L) {
    ...
    switch (n) {
        case 2: {
            ...
            if (size != 2)
                return returnToLuaWithError(L, "Wrong size of keytype, got %d bytes, expected 1",
                                            (int) size);
        ...
        case 1: {
            ...
            if (size != 2)
                return returnToLuaWithError(L, "Wrong size of blockno, got %d bytes, expected 2",
                                            (int) size);
...
static int l_hardnested(lua_State *L) {
    ...
    if (size != 2)
        return returnToLuaWithError(L, "Wrong size of trgBlockNo, got %d bytes, expected 2",
                                    (int) size);

    const char *p_trg_keytype = luaL_checklstring(L, 5, &size);
    if (size != 1)
        return returnToLuaWithError(L, "Wrong size of trgKeyType, got %d bytes, expected 1",
                                    (int) size);
    ...
    if (size != 1)
        return returnToLuaWithError(L, "Wrong size of nonce_file_read, got %d bytes, expected 1",
                                    (int) size);

    const char *p_nonce_file_write = luaL_checklstring(L, 8, &size);
    if (size != 1)
        return returnToLuaWithError(L, "Wrong size of nonce_file_write, got %d bytes, expected 1",
                                    (int) size);
    ...
    uint64_t foundkey = 0;
    int retval = mfnestedhard(blockNo, keyType, key, trgBlockNo, trgKeyType,
                              haveTarget ? trgkey : NULL, nonce_file_read, nonce_file_write, slow,
                              tests, &foundkey, filename);
    DropField();
...
int setLuaPath(lua_State *L, const char *path) {
    ...
    int requiredLength =
            strlen(cur_path) + strlen(path) + 10; //A few bytes too many, whatever we can afford it
...
//		{"keygen.algoC",				l_keygen_algoC},
    ...
    char libraries_path[strlen(get_my_executable_directory()) + strlen(LUA_LIBRARIES_DIRECTORY) +
                        strlen(LUA_LIBRARIES_WILDCARD) + 1];
