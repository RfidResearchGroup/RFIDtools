...
// write a raw topaz command, returns the length of the response (0 in case of error)
...
// calculate CRC bytes and write topaz command, returns the length of the response (0 in case of error)
...
int CmdHFTopazReader(const char *Cmd) {
	...
	if (status == -1) {
		if (verbose) PrintAndLogEx(WARNING, "Error: couldn't read ATQA");
		return -1;
	}
	...
	// ToDo: CRC check
	PrintAndLogEx(NORMAL, "HR0  : %02x (%sa Topaz tag (%scapable of carrying a NDEF message), %s memory maps)", rid_response[0],
