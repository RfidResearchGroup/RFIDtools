...
int usage_lf_em410x_demod(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "     clock               -  set clock as integer, optional, if not set, autodetect.");
    ...
    PrintAndLogEx(NORMAL,
                  "           lf em 410x_demod        = demod an EM410x Tag ID from GraphBuffer");
    PrintAndLogEx(NORMAL,
                  "           lf em 410x_demod 32     = demod an EM410x Tag ID from GraphBuffer using a clock of RF/32");
    PrintAndLogEx(NORMAL,
                  "           lf em 410x_demod 32 1   = demod an EM410x Tag ID from GraphBuffer using a clock of RF/32 and inverting data");
    PrintAndLogEx(NORMAL,
                  "           lf em 410x_demod 1      = demod an EM410x Tag ID from GraphBuffer while inverting data");
    PrintAndLogEx(NORMAL,
                  "           lf em 410x_demod 64 1 0 = demod an EM410x Tag ID from GraphBuffer using a clock of RF/64 and inverting data and allowing 0 demod errors");
    return 0;
}
...
int usage_lf_em410x_write(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "       <clock>   - 16|32|40|64, optional, set R/F clock rate, defaults to 64");
    PrintAndLogEx(NORMAL, "Examples:");
...
int usage_lf_em410x_ws(void) {
    PrintAndLogEx(NORMAL,
                  "Watch 'nd Spoof, activates reader, waits until a EM410x tag gets presented then it starts simulating the found UID");
    PrintAndLogEx(NORMAL, "");
...
int usage_lf_em410x_brute(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "       d (2000)      - pause delay in milliseconds between UIDs simulation, default 1000 ms (optional)");
...
void printEM410x(uint32_t hi, uint64_t id) {
    ...
    if (hi) {
        ...
        PrintAndLogEx(NORMAL, "DEZ 5.5        : %05" PRIu64 ".%05" PRIu64, (id >> 16LL) & 0xFFFF,
                      (id & 0xFFFF));
        PrintAndLogEx(NORMAL, "DEZ 3.5A       : %03" PRIu64 ".%05" PRIu64, (id >> 32ll),
                      (id & 0xFFFF));
        PrintAndLogEx(NORMAL, "DEZ 3.5B       : %03" PRIu64 ".%05" PRIu64, (id & 0xFF000000) >> 24,
                      (id & 0xFFFF));
        PrintAndLogEx(NORMAL, "DEZ 3.5C       : %03" PRIu64 ".%05" PRIu64, (id & 0xFF0000) >> 16,
                      (id & 0xFFFF));
        PrintAndLogEx(NORMAL, "DEZ 14/IK2     : %014" PRIu64, id);
        PrintAndLogEx(NORMAL, "DEZ 15/IK3     : %015" PRIu64, id2lo);
        PrintAndLogEx(NORMAL,
                      "DEZ 20/ZK      : %02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64 "%02" PRIu64,
        ...
        PrintAndLogEx(NORMAL, "}\nOther          : %05" PRIu64 "_%03" PRIu64 "_%08" PRIu64,
                      (id & 0xFFFF), ((id >> 16LL) & 0xFF), (id & 0xFFFFFF));
        ...
        PrintAndLogEx(NORMAL, "Pattern Sebury : %d %d %d  [0x%X 0x%X 0x%X]", sebury1, sebury2,
                      sebury3, sebury1, sebury2, sebury3);
    }
}
...
// this read loops on devices side.
...
int CmdEM410xSim(const char *Cmd) {
    ...
    PrintAndLogEx(NORMAL, "Starting simulating UID %02X%02X%02X%02X%02X  clock: %d", uid[0], uid[1],
                  uid[2], uid[3], uid[4], clock);
...
int CmdEM410xBrute(const char *Cmd) {
    ...
    for (uint32_t c = 0; c < uidcnt; ++c) {
        ...
        PrintAndLogEx(NORMAL, "Bruteforce %d / %d: simulating UID  %s, clock %d", c + 1, uidcnt,
                      testuid, clock);
...
int CmdEM410xWrite(const char *Cmd) {
    ...
    if ((clock != 16) && (clock != 32) && (clock != 64) && (clock != 40)) {
        PrintAndLogEx(WARNING,
                      "Error! Clock rate %d not valid. Supported clock rates are 16, 32, 40 and 64.\n",
                      clock);
        return 0;
    }

    if (card == 1) {
        PrintAndLogEx(NORMAL, "Writing %s tag with UID 0x%010" PRIx64 " (clock rate: %d)", "T55x7",
                      id, clock);
...
bool EMwordparitytest(uint8_t *bits) {
    ...
    // col parity check
    uint8_t c1 = bytebits_to_byte(bits, 8) ^bytebits_to_byte(bits + 9, 8) ^
                 bytebits_to_byte(bits + 18, 8) ^bytebits_to_byte(bits + 27, 8);
...
int EM4x50Read(const char *Cmd, bool verbose) {
    ...
    if (!clk) {
        ...
        if (!clk) {
            if (verbose || g_debugMode)
                PrintAndLogEx(WARNING, "Error: EM4x50 - didn't find a clock");
            return 0;
        }
    } else tol = clk / 8;
...
// download samples from devices and copy to Graphbuffer
...
int CmdEM4x05Write(const char *Cmd) {
    ...
    if (!WaitForResponseTimeout(CMD_ACK, &resp, 2000)) {
        PrintAndLogEx(WARNING, "Error occurred, devices did not respond during write operation.");
        return -1;
    }
...
void printEM4x05config(uint32_t wordData) {
    ...
    PrintAndLogEx(NORMAL,
                  " LastWordR:  %02u | Address of last word for default read - meaning %u blocks are output",
                  LWR, numblks);
    PrintAndLogEx(NORMAL, " ReadLogin:   %u | Read Login is %s", readLogin,
                  readLogin ? "Required" : "Not Required");
    PrintAndLogEx(NORMAL, "   ReadHKL:   %u | Read Housekeeping Words Login is %s", readHKL,
                  readHKL ? "Required" : "Not Required");
    PrintAndLogEx(NORMAL, "WriteLogin:   %u | Write Login is %s", writeLogin,
                  writeLogin ? "Required" : "Not Required");
    PrintAndLogEx(NORMAL, "  WriteHKL:   %u | Write Housekeeping Words Login is %s", writeHKL,
                  writeHKL ? "Required" : "Not Required");
    PrintAndLogEx(NORMAL, "    R.A.W.:   %u | Read After Write is %s", raw, raw ? "On" : "Off");
    PrintAndLogEx(NORMAL, "   Disable:   %u | Disable Command is %s", disable,
                  disable ? "Accepted" : "Not Accepted");
    PrintAndLogEx(NORMAL, "    R.T.F.:   %u | Reader Talk First is %s", rtf,
                  rtf ? "Enabled" : "Disabled");
    PrintAndLogEx(NORMAL, "    Pigeon:   %u | Pigeon Mode is %s\n", pigeon,
                  pigeon ? "Enabled" : "Disabled");
}
...
void printEM4x05info(uint32_t block0, uint32_t serial) {
    ...
    PrintAndLogEx(NORMAL, " Cust Code: %03u | %s", custCode,
                  (custCode == 0x200) ? "Default" : "Unknown");
    if (serial != 0)
        PrintAndLogEx(NORMAL, "\n  Serial #: %08X\n", serial);
}
...
void printEM4x05ProtectionBits(uint32_t word) {
    for (uint8_t i = 0; i < 15; i++) {
        PrintAndLogEx(NORMAL, "      Word:  %02u | %s", i,
                      (((1 << i) & word) || i < 2) ? "Is Write Locked" : "Is Not Write Locked");
        if (i == 14)
            PrintAndLogEx(NORMAL, "      Word:  %02u | %s", i + 1,
                          (((1 << i) & word) || i < 2) ? "Is Write Locked" : "Is Not Write Locked");
    }
}
