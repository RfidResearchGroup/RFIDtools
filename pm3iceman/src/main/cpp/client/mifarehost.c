...
int mfDarkside(uint8_t blockno, uint8_t key_type, uint64_t *key) {
    ...
    PrintAndLogEx(NORMAL,
                  "press pm3-button on the proxmark3 devices to abort both proxmark3 and client.");
    PrintAndLogEx(NORMAL,
                  "--------------------------------------------------------------------------------\n");
...
// Sends chunks of keys to devices.
// 0 == ok all keys found
// 1 == 
// 2 == Time-out, aborting
int mfCheckKeys_fast(uint8_t sectorsCnt, uint8_t firstChunk, uint8_t lastChunk, uint8_t strategy,
    ...
    // write keychunk
    UsbCommand c = {CMD_MIFARE_CHKKEYS_FAST,
                    {(sectorsCnt | (firstChunk << 8) | (lastChunk << 12)), strategy, size}};
    ...
    PrintAndLogEx(NORMAL, "\n[-] Chunk: %.1fs | found %u/%u keys (%u)", (float) (t2 / 1000.0),
                  curr_keys, (sectorsCnt << 1), size);
...
#define CANDIDATE_SIZE 0xFFFF * 6
    ...
    for (i = 0, counter = 1; i < CANDIDATE_SIZE; i += KEYBLOCK_SIZE, ++counter) {
        ...
        // progress
        if (counter % 20 == 0)
            PrintAndLogEx(SUCCESS, "tried : %s.. \t %u keys", sprint_hex(candidates + i, 6),
                          counter * KEYS_IN_BLOCK);
    }
    return found;
}
...
int mfnested(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType,
    ...
    *(uint64_t *) p4 = -1;
    ...
    for (int i = 0; i < keycnt; i += max_keys) {
        ...
        if (!mfCheckKeys(statelists[0].blockNo, statelists[0].keyType, false, size, keyBlock,
                         &key64)) {
            ...
            PrintAndLogEx(SUCCESS,
                          "target block:%3u key type: %c  -- found valid key [%012" PRIx64 "]",
    ...
    while (true) {
        ...
        if (WaitForResponseTimeout(CMD_ACK, &resp, 500)) {
            ...
            switch (ok) {
                ...
                case 2 :
                    PrintAndLogEx(SUCCESS, "always leak NACK detected");
                    return 3;
                case 1 :
                    PrintAndLogEx(SUCCESS, "NACK bug detected");
                    return 1;
                case 0 :
                    PrintAndLogEx(SUCCESS, "No NACK bug detected");
                    return 2;
                default :
                    PrintAndLogEx(WARNING, "errorcode from devices [%i]", ok);
                    return 0;
            }
            break;
        }
...
void detect_classic_magic(void) {
    ...
    switch (isGeneration) {
        case 1:
            PrintAndLogEx(SUCCESS, "Answers to magic commands (GEN 1a): YES");
            break;
        case 2:
            PrintAndLogEx(SUCCESS, "Answers to magic commands (GEN 1b): YES");
            break;
            //case 4: PrintAndLogEx(SUCCESS, "Answers to magic commands (GEN 2): YES"); break;
        default:
            PrintAndLogEx(INFO, "Answers to magic commands: NO");
            break;
    }
}