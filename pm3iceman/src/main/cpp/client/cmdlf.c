...
int usage_lf_config(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "       d <decim>     Sets decimation. A value of N saves only 1 in N samples. Default: 1");
    PrintAndLogEx(NORMAL,
...
/* write a LF command before reading */
...
#define FPGA_HF 2
    ...
    //can write only 512 bits at a time (1 byte sent per bit...)
...
int CmdLFfskSim(const char *Cmd) {
    ...
    if (size > USB_CMD_DATA_SIZE) {
        PrintAndLogEx(NORMAL,
                      "DemodBuffer too long for current implementation - length: %d - max: %d",
...
int CmdLFfind(const char *Cmd) {
    ...
    // only run these tests if devices is online
    if (isOnline) {
...
static command_t CommandTable[] = {
        ...
        {"cmdread",   CmdLFCommandRead, 0, "<off period> <'0' period> <'1' period> <command> ['h' 134] \n\t\t-- Modulate LF reader field to write command before read (all periods in microseconds)"},
