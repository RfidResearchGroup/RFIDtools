...
uint32_t ul_ev1_pwdgenA(uint8_t *uid) {
    ...
    uint32_t xortable[] = {
            0x4f2711c1, 0x07D7BB83, 0x9636EF07, 0xB5F4460E, 0xF271141C, 0x7D7BB038, 0x636EF871,
            0x5F4468E3,
            0x271149C7, 0xD7BB0B8F, 0x36EF8F1E, 0xF446863D, 0x7114947A, 0x7BB0B0F5, 0x6EF8F9EB,
            0x44686BD7,
            0x11494fAF, 0xBB0B075F, 0xEF8F96BE, 0x4686B57C, 0x1494F2F9, 0xB0B07DF3, 0xF8F963E6,
            0x686B5FCC,
            0x494F2799, 0x0B07D733, 0x8F963667, 0x86B5F4CE, 0x94F2719C, 0xB07D7B38, 0xF9636E70,
            0x6B5F44E0
    };
...
int ul_ev1_pwdgen_selftest() {
    ...
    PrintAndLogEx(NORMAL, "UID | %s | %08X | %s", sprint_hex(uid1, 7), pwd1,
                  (pwd1 == 0x8432EB17) ? "OK" : "->8432EB17<-");

    uint8_t uid2[] = {0x04, 0x1f, 0x98, 0xea, 0x1e, 0x3e, 0x81};
    uint32_t pwd2 = ul_ev1_pwdgenB(uid2);
    PrintAndLogEx(NORMAL, "UID | %s | %08X | %s", sprint_hex(uid2, 7), pwd2,
                  (pwd2 == 0x5fd37eca) ? "OK" : "->5fd37eca<--");

    uint8_t uid3[] = {0x04, 0x62, 0xB6, 0x8A, 0xB4, 0x42, 0x80};
    uint32_t pwd3 = ul_ev1_pwdgenC(uid3);
    PrintAndLogEx(NORMAL, "UID | %s | %08X | %s", sprint_hex(uid3, 7), pwd3,
                  (pwd3 == 0x5a349515) ? "OK" : "->5a349515<--");

    uint8_t uid4[] = {0x04, 0xC5, 0xDF, 0x4A, 0x6D, 0x51, 0x80};
    uint32_t pwd4 = ul_ev1_pwdgenD(uid4);
    PrintAndLogEx(NORMAL, "UID | %s | %08X | %s", sprint_hex(uid4, 7), pwd4,
                  (pwd4 == 0x72B1EC61) ? "OK" : "->72B1EC61<--");
    return 0;
}
...
static int
ul_send_cmd_raw(uint8_t *cmd, uint8_t cmdlen, uint8_t *response, uint16_t responseLength) {
    UsbCommand c = {CMD_READER_ISO_14443a,
                    {ISO14A_RAW | ISO14A_NO_DISCONNECT | ISO14A_APPEND_CRC | ISO14A_NO_RATS, cmdlen,
                     0}};
...
static int
ul_auth_select(iso14a_card_select_t *card, TagTypeUL_t tagtype, bool hasAuthKey, uint8_t *authkey,
               uint8_t *pack, uint8_t packSize) {
...
//  or
//  write 300000 + crc (read with extra byte(s))
...
static int ul_fudan_check(void) {
    ...
    UsbCommand c = {CMD_READER_ISO_14443a,
                    {ISO14A_RAW | ISO14A_NO_DISCONNECT | ISO14A_NO_RATS, 4, 0}};
...
static int ndef_print_CC(uint8_t *data) {
    ...
    PrintAndLogEx(NORMAL, "  %02X : version %d.%d supported by tag", data[1], (data[1] & 0xF0) >> 4,
                  data[1] & 0x0F);
    ...
    PrintAndLogEx(NORMAL, "  %02X : %s / %s", data[3],
                  ...
                  (data[3] & 0x0F) == 0 ? "Write access granted without any security" :
                  (data[3] & 0x0F) == 0x0F ? "No write access granted at all" : "(RFU)");
    return 0;
}
...
int ul_print_type(uint32_t tagtype, uint8_t spaces) {
    ...
    if (tagtype & UL)
        PrintAndLogEx(NORMAL, "%sTYPE : MIFARE Ultralight (MF0ICU1) %s", spacer,
                      (tagtype & MAGIC) ? "<magic>" : "");
    else if (tagtype & UL_C)
        PrintAndLogEx(NORMAL, "%sTYPE : MIFARE Ultralight C (MF0ULC) %s", spacer,
                      (tagtype & MAGIC) ? "<magic>" : "");
        ...
        PrintAndLogEx(NORMAL, "%sTYPE : FUDAN Ultralight Compatible (or other compatible) %s",
                      spacer, (tagtype & MAGIC) ? "<magic>" : "");
    else
...
static int ulc_print_3deskey(uint8_t *data) {
    PrintAndLogEx(NORMAL, "         deskey1 [44/0x2C] : %s [s]", sprint_hex(data, 4),
                  sprint_ascii(data, 4));
    PrintAndLogEx(NORMAL, "         deskey1 [45/0x2D] : %s [s]", sprint_hex(data + 4, 4),
                  sprint_ascii(data + 4, 4));
    PrintAndLogEx(NORMAL, "         deskey2 [46/0x2E] : %s [s]", sprint_hex(data + 8, 4),
                  sprint_ascii(data + 8, 4));
    PrintAndLogEx(NORMAL, "         deskey2 [47/0x2F] : %s [s]", sprint_hex(data + 12, 4),
                  sprint_ascii(data + 12, 4));
...
static int ulc_print_configuration(uint8_t *data) {
    ...
    PrintAndLogEx(NORMAL, " Higher Lockbits [40/0x28] : %s - %s", sprint_hex(data, 4),
                  sprint_bin(data, 2));
    PrintAndLogEx(NORMAL, "         Counter [41/0x29] : %s - %s", sprint_hex(data + 4, 4),
                  sprint_bin(data + 4, 2));

    bool validAuth = (data[8] >= 0x03 && data[8] <= 0x30);
    if (validAuth)
        PrintAndLogEx(NORMAL,
                      "           Auth0 [42/0x2A] : %s page %d/0x%02X and above need authentication",
        ...
        if (data[8] == 0) {
            ...
            PrintAndLogEx(NORMAL, "           Auth0 [42/0x2A] : %s auth byte is out-of-range",
                          sprint_hex(data + 8, 4));
        }
    }
...
static int ulev1_print_configuration(uint32_t tagtype, uint8_t *data, uint8_t startPage) {
    ...
    if ((tagtype & (NTAG_213_F | NTAG_216_F))) {
        ...
        PrintAndLogEx(NORMAL, "                    - SLEEP mode %s",
        ...
        if (tagtype & NTAG_213_F) {
            switch (mirror_conf) {
                case 1: {
                    PrintAndLogEx(NORMAL, "         mirror start block %02X | byte pos %02X - %s",
                                  data[2], mirror_byte,
    ...
    PrintAndLogEx(NORMAL, "                    - strong modulation mode %s",
                  (strg_mod_en) ? "enabled" : "disabled");

    if (data[3] < 0xff)
        PrintAndLogEx(NORMAL, "                    - page %d and above need authentication",
                      data[3]);
    else
        PrintAndLogEx(NORMAL, "                    - pages don't need authentication");

    PrintAndLogEx(NORMAL, "  cfg1 [%u/0x%02X] : %s", startPage + 1, startPage + 1,
                  sprint_hex(data + 4, 4));
    if (authlim == 0)
        PrintAndLogEx(NORMAL, "                    - Unlimited password attempts");
    else
        PrintAndLogEx(NORMAL, "                    - Max number of password attempts is %d",
                      authlim);

    PrintAndLogEx(NORMAL, "                    - NFC counter %s",
                  (nfc_cnf_en) ? "enabled" : "disabled");
    PrintAndLogEx(NORMAL, "                    - NFC counter %s",
                  (nfc_cnf_prot_pwd) ? "not protected" : "password protection enabled");

    PrintAndLogEx(NORMAL, "                    - user configuration %s",
                  cfglck ? "permanently locked" : "writeable");
    PrintAndLogEx(NORMAL, "                    - %s access is protected with password",
                  prot ? "read and write" : "write");
    PrintAndLogEx(NORMAL, "                    - %02X, Virtual Card Type Identifier is %s default",
                  vctid, (vctid == 0x05) ? "" : "not");
    PrintAndLogEx(NORMAL, "  PWD  [%u/0x%02X] : %s- (cannot be read)", startPage + 2, startPage + 2,
                  sprint_hex(data + 8, 4));
    PrintAndLogEx(NORMAL, "  PACK [%u/0x%02X] : %s      - (cannot be read)", startPage + 3,
                  startPage + 3, sprint_hex(data + 12, 2));
    PrintAndLogEx(NORMAL, "  RFU  [%u/0x%02X] :       %s- (cannot be read)", startPage + 3,
                  startPage + 3, sprint_hex(data + 14, 2));
    return 0;
}
...
static int ulev1_print_counters() {
    ...
    for (uint8_t i = 0; i < 3; ++i) {
        ...
        if (len == 3) {
            ...
            PrintAndLogEx(NORMAL, "                    - %02X tearing %s", tear[0],
                          (tear[0] == 0xBD) ? "Ok" : "failure");
        }
    }
    return len;
}
...
static int ulev1_print_signature(uint8_t *data, uint8_t len) {
    ...
    PrintAndLogEx(NORMAL, "IC signature public key value : %s",
                  sprint_hex(public_ecda_key, PUBLIC_ECDA_KEYLEN));
...
static int ulev1_print_version(uint8_t *data) {
    ...
    PrintAndLogEx(NORMAL, " Product subtype : %02X, %s", data[3],
                  (data[3] == 1) ? "17 pF" : "50pF");
    ...
    PrintAndLogEx(NORMAL, "   Protocol type : %02X %s", data[7],
...
uint32_t GetHF14AMfU_Type(void) {
    ...
    if (card.uid[0] != 0x05) {
        ...
        switch (nib) {
            ...
            case 2:
                tagtype = (MY_D_NFC);
                break; // or SLE 66RxxP ... up to 512 pages of 8 user bytes... (or in devices mode FF pages of 4 bytes)
            case 3:
...
int CmdHF14AMfUInfo(const char *Cmd) {
    ...
    if ((tagtype & (UL_EV1_48 | UL_EV1_128 | UL_EV1))) {
        if (ulev1_print_counters() != 3) {
            // failed - re-select
            if (!ul_auth_select(&card, tagtype, hasAuthKey, authkeyptr, pack, sizeof(pack)))
                return -1;
        }
    }
    ...
    // Read signature
    if ((tagtype &
         (UL_EV1_48 | UL_EV1_128 | UL_EV1 | UL_NANO_40 | NTAG_213 | NTAG_213_F | NTAG_215 |
          NTAG_216 | NTAG_216_F | NTAG_I2C_1K | NTAG_I2C_2K | NTAG_I2C_1K_PLUS |
          NTAG_I2C_2K_PLUS))) {
    ...
    if ((tagtype &
        ...
        if (!authlim && !hasAuthKey) {
            ...
            if (len > -1) {
                PrintAndLogEx(NORMAL, "Found a default password: %s || Pack: %02X %02X",
            ...
            for (uint8_t i = 0; i < KEYS_PWD_COUNT; ++i) {
                ...
                if (len > -1) {
                    ...
                    if (!ul_auth_select(&card, tagtype, hasAuthKey, authkeyptr, pack,
                                        sizeof(pack)))
                        return -1;
                }
            }
...
int CmdHF14AMfUWrBl(const char *Cmd) {
    ...
    if (blockNo <= 3)
        PrintAndLogEx(NORMAL, "Special Block: %0d (0x%02X) [ %s]", blockNo, blockNo,
                      sprint_hex(blockdata, 4));
    else
        PrintAndLogEx(NORMAL, "Block: %0d (0x%02X) [ %s]", blockNo, blockNo,
                      sprint_hex(blockdata, 4));
...
int usage_hf_mfu_dump(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "Usage:  hf mfu dump k <key> l f <filename w/o .bin> p <page#> q <#pages>");
    PrintAndLogEx(NORMAL, "  Options :");
    PrintAndLogEx(NORMAL,
                  "  k <key> : (optional) key for authentication [UL-C 16bytes, EV1/NTAG 4bytes]");
    PrintAndLogEx(NORMAL, "  l       : (optional) swap entered key's endianness");
...
int usage_hf_mfu_restore(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "  e       : (optional) enable special write version/signature -MAGIC NTAG 21* ONLY-");
    PrintAndLogEx(NORMAL,
                  "  r       : (optional) use the password found in dumpfile to configure tag. requires 'e' parameter to work");
...
int usage_hf_mfu_ucauth(void) {
    ...
    PrintAndLogEx(NORMAL, "      3 : devices key");
...
void printMFUdumpEx(mfu_dump_t *card, uint16_t pages, uint8_t startpage) {
    ...
    PrintAndLogEx(NORMAL, "\nDataType  | Data                    | Ascii");
    PrintAndLogEx(NORMAL, "----------+-------------------------+---------");
    PrintAndLogEx(NORMAL, "Version   | %s| %s", sprint_hex(card->version, sizeof(card->version)),
                  sprint_ascii(card->version, sizeof(card->version)));
    PrintAndLogEx(NORMAL, "TBD       | %-24s| %s", sprint_hex(card->tbo, sizeof(card->tbo)),
                  sprint_ascii(card->tbo, sizeof(card->tbo)));
    PrintAndLogEx(NORMAL, "Tearing   | %-24s| %s", sprint_hex(card->tearing, sizeof(card->tearing)),
                  sprint_ascii(card->tearing, sizeof(card->tearing)));
    PrintAndLogEx(NORMAL, "Pack      | %-24s| %s", sprint_hex(card->pack, sizeof(card->pack)),
                  sprint_ascii(card->pack, sizeof(card->pack)));
    PrintAndLogEx(NORMAL, "TBD       | %-24s| %s", sprint_hex(card->tbo1, sizeof(card->tbo1)),
                  sprint_ascii(card->tbo1, sizeof(card->tbo1)));
    PrintAndLogEx(NORMAL, "Signature1| %s| %s", sprint_hex(card->signature, 16),
                  sprint_ascii(card->signature, 16));
    PrintAndLogEx(NORMAL, "Signature2| %s| %s", sprint_hex(card->signature + 16, 16),
                  sprint_ascii(card->signature + 16, 16));
    PrintAndLogEx(NORMAL, "-------------------------------------------------------------");
    ...
    for (uint8_t i = 0; i < pages; ++i) {
        if (i < 3) {
            PrintAndLogEx(NORMAL, "%3d/0x%02X | %s|   | %s", i + startpage, i + startpage,
                          sprint_hex(data + i * 4, 4), sprint_ascii(data + i * 4, 4));
            continue;
        }
        switch (i) {
            case 3:
        ...
        PrintAndLogEx(NORMAL, "%3d/0x%02X | %s| %d | %s", i + startpage, i + startpage,
                      sprint_hex(data + i * 4, 4), lckbit, sprint_ascii(data + i * 4, 4));
    }
    PrintAndLogEx(NORMAL, "---------------------------------");
}
...
int CmdHF14AMfUDump(const char *Cmd) {
    ...
    PrintAndLogEx(SUCCESS, "Dumped %d pages, wrote %d bytes to %s",
                  pages + (DUMP_PREFIX_LENGTH / 4), pages * 4 + DUMP_PREFIX_LENGTH, filename);
...
int CmdHF14AMfuGenDiverseKeys(const char *Cmd) {
    ...
    uint8_t masterkey[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
                           0xcc, 0xdd, 0xee, 0xff};
