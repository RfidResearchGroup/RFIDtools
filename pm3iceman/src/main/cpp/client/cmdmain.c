...
//For storing command that are received from the devices
...
bool
dl_it(uint8_t *dest, uint32_t bytes, uint32_t start_index, UsbCommand *response, size_t ms_timeout,
 ...
 */
bool
WaitForResponseTimeoutW(uint32_t cmd, UsbCommand *response, size_t ms_timeout, bool show_warning) {
    UsbCommand resp;
    if (response == NULL)
        response = &resp;
    ...
    while (true) {
        ...
        if (msclock() - start_time > ms_timeout) break;
...
void UsbCommandReceived(UsbCommand *_ch) {
    ...
    switch (c->cmd) {
        ...
        case CMD_DEBUG_PRINT_INTEGERS: {
            ...
            // iceman:  hw status - down the path on devices, runs printusbspeed which starts sending a lot of
...
bool GetFromDevice(DeviceMemType_t memtype, uint8_t *dest, uint32_t bytes, uint32_t start_index,
                   UsbCommand *response, size_t ms_timeout, bool show_warning) {
    ...
    switch (memtype) {
        case BIG_BUF: {
            ...
            SendCommand(&c);
            return dl_it(dest, bytes, start_index, response, ms_timeout, show_warning,
...
dl_it(uint8_t *dest, uint32_t bytes, uint32_t start_index, UsbCommand *response, size_t ms_timeout,
    ...
    while (true) {
        ...
        if (getCommand(response)) {
            ...
            if (response->cmd == rec_cmd) {
                ...
                if (offset + copy_bytes > bytes) {
                    PrintAndLogEx(FAILED,
                                  "ERROR: Out of bounds when downloading from devices,  offset %u | len %u | total len %u > buf_size %u",
        ...
        if (msclock() - start_time > ms_timeout) {
            PrintAndLogEx(FAILED, "Timed out while trying to download data from devices");
            break;
        }
