...
int usage_legic_rdmem(void) {
    ...
    PrintAndLogEx(NORMAL, "      h             : this help");
    PrintAndLogEx(NORMAL,
                  "      <offset>      : (hex) offset in data array to start download from");
    PrintAndLogEx(NORMAL, "      <length>      : (hex) number of bytes to read");
    PrintAndLogEx(NORMAL,
                  "      <IV>          : (hex) (optional) Initialization vector to use. Must be odd and 7bits max");
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL,
                  "      hf legic rdmem 0 16        - reads from byte[0] 0x16 bytes(system header)");
    PrintAndLogEx(NORMAL,
                  "      hf legic rdmem 0 4 55      - reads from byte[0] 0x4 bytes with IV 0x55");
...
int usage_legic_restore(void) {
    ...
    PrintAndLogEx(NORMAL, "      h             : this help");
    PrintAndLogEx(NORMAL,
                  "      i <filename>  : filename w/o '.bin' to restore bytes on to card from");
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(NORMAL, "Examples:");
    ...
    if (bIsSegmented) {
        ...
        for (segmentNum = 1; segmentNum < 128; segmentNum++) {
            ...
            if (hasWRP) {
                ...
                if (wrp_len == 8)
                    PrintAndLogEx(NORMAL, "Card ID: %2X%02X%02X", data[i - 4] ^ crc,
                                  data[i - 3] ^ crc, data[i - 2] ^ crc);
            }

            PrintAndLogEx(NORMAL, "Remaining segment payload:  (I %d | K %d | Remain LEN %d)", i, k,
        ...
        bool hasWRP = (wrp > wrc);
        int wrp_len = (wrp - wrc);
        int remain_seg_payload_len = (1024 - 22 -
                                      wrp);    // Any chance to get physical card size here!?
        ...
        if (hasWRP) {
            ...
            // does this one work? (Answer: Only if KGH/BGH is used with BCD encoded card number! So maybe this will show just garbage...)
            if (wrp_len == 8)
                PrintAndLogEx(NORMAL, "Card ID: %2X%02X%02X", data[i - 4], data[i - 3],
                              data[i - 2]);
        }

        PrintAndLogEx(NORMAL, "Remaining segment payload:  (I %d | Remain LEN %d)", i,
                      remain_seg_payload_len);
        PrintAndLogEx(NORMAL, "\nrow  | data");
        PrintAndLogEx(NORMAL, "-----+------------------------------------------------");
        print_hex_break(data + i, remain_seg_payload_len, 16);
...
int legic_read_mem(uint32_t offset, uint32_t len, uint32_t iv, uint8_t *out, uint16_t *outlen) {
    ...
    *outlen = resp.arg[1];
    ...
    if (!GetFromDevice(BIG_BUF_EML, out, *outlen, 0, NULL, 2500, false)) {
        PrintAndLogEx(WARNING, "Fail, transfer from devices time-out");
        return 4;
    }
    return 0;
}
...
int CmdLegicRestore(const char *Cmd) {
    ...
    if (filesize != numofbytes) {
        PrintAndLogEx(WARNING, "Fail, filesize and cardsize is not equal. [%u != %u]", filesize,
                      numofbytes);
        free(data);
        fclose(f);
        return 4;
