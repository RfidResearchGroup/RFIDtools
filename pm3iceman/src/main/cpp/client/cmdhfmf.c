...
int usage_hf14_mf1ksim(void) {
    ...
    PrintAndLogEx(NORMAL, "      h    this help");
    PrintAndLogEx(NORMAL,
                  "      u    (Optional) UID 4,7 or 10bytes. If not specified, the UID 4b from emulator memory will be used");
    PrintAndLogEx(NORMAL,
    ...
    PrintAndLogEx(NORMAL,
                  "      x    (Optional) Crack, performs the 'reader attack', nr/ar attack against a reader");
...
int usage_hf14_hardnested(void) {
    ...
    PrintAndLogEx(NORMAL, "      h         this help");
    PrintAndLogEx(NORMAL,
                  "      w         acquire nonces and UID, and write them to binary file with default name hf-mf-<UID>-nonces.bin");
    PrintAndLogEx(NORMAL,
                  "      s         slower acquisition (required by some non standard cards)");
    PrintAndLogEx(NORMAL,
                  "      r         read hf-mf-<UID>-nonces.bin if tag present, otherwise read nonces.bin, then start attack");
    PrintAndLogEx(NORMAL,
                  "      u <UID>   read/write hf-mf-<UID>-nonces.bin instead of default name");
...
int usage_hf14_chk_fast(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "Usage:  hf mf fchk [h] <card memory> [t|d] [<key (12 hex symbols)>] [<dic (*.dic)>]");
    PrintAndLogEx(NORMAL, "Options:");
    PrintAndLogEx(NORMAL, "      h    this help");
    PrintAndLogEx(NORMAL,
                  "      <cardmem> all sectors based on card memory, other values than below defaults to 1k");
    PrintAndLogEx(NORMAL, "      			 0 - MINI(320 bytes)");
    ...
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL,
                  "      hf mf fchk 1 1234567890ab keys.dic    -- target 1K using key 1234567890ab, using dictionary file");
    PrintAndLogEx(NORMAL,
                  "      hf mf fchk 1 t                        -- target 1K, write to emulator memory");
    PrintAndLogEx(NORMAL,
                  "      hf mf fchk 1 d                        -- target 1K, write to file");
    return 0;
}
...
int usage_hf14_restore(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "  [card memory]: 0 = 320 bytes (Mifare Mini), 1 = 1K (default), 2 = 2K, 4 = 4K");
    PrintAndLogEx(NORMAL,
                  "  u <UID>      : uid, try to restore from hf-mf-<UID>-key.bin and hf-mf-<UID>-data.bin");
    PrintAndLogEx(NORMAL, "  k <name>     : key filename, specific the full filename of key file");
    PrintAndLogEx(NORMAL,
                  "  f <name>     : data filename, specific the full filename of data file");
    PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL,
                  "         hf mf restore                            -- read the UID from tag first, then restore from hf-mf-<UID>-key.bin and and hf-mf-<UID>-data.bin");
    PrintAndLogEx(NORMAL,
                  "         hf mf restore 1 u 12345678               -- restore from hf-mf-12345678-key.bin and hf-mf-12345678-data.bin");
    PrintAndLogEx(NORMAL,
                  "         hf mf restore 1 u 12345678 k dumpkey.bin -- restore from dumpkey.bin and hf-mf-12345678-data.bin");
    PrintAndLogEx(NORMAL,
                  "         hf mf restore 4                          -- read the UID from tag with 4K memory first, then restore from hf-mf-<UID>-key.bin and and hf-mf-<UID>-data.bin");
    return 0;
}
...
int usage_hf14_decryptbytes(void) {
    ...
    PrintAndLogEx(NORMAL,
                  "      <data>       encrypted data, taken directly after at_enc and forward");
    PrintAndLogEx(NORMAL, "Examples:");
...
char *GenerateAbsoluteFilePath(const char *fileName, const char *absolutePath) {
    ...
    char *buf = malloc(
            sizeof(char) *
            (strlen(fileName) + strlen(absolutePath)) + 1);
    //进行生成绝对路径!!!
    sprintf(buf, "%s%s", absolutePath, fileName);
    return buf;
}
...
int CmdHF14AMfWrBl(const char *Cmd) {
    ...
    PrintAndLogEx(NORMAL, "--block no:%d, key type:%c, key:%s", blockNo, keyType ? 'B' : 'A',
                  sprint_hex(key, 6));
...
int CmdHF14AMfRdSc(const char *Cmd) {
    ...
    PrintAndLogEx(NORMAL, "--sector no:%d key type:%c key:%s ", sectorNo, keyType ? 'B' : 'A',
                  sprint_hex(key, 6));
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
        ...
        if (isOK) {
            ...
            PrintAndLogEx(NORMAL, "trailer: %s",
                          sprint_hex(data + (sectorNo < 32 ? 3 : 15) * 16, 16));
        }
    } else {
        PrintAndLogEx(WARNING, "Command execute timeout");
    }
...
int CmdHF14AMfDump(const char *Cmd) {
        ...
        for (blockNo = 0; isOK && blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
            ...
            if (received) {
                ...
                if (isOK) {
                    ...
                    PrintAndLogEx(SUCCESS, "successfully read block %2d of sector %2d.", blockNo,
                                  sectorNo);
                } else {
                    PrintAndLogEx(FAILED, "could not read block %2d of sector %2d", blockNo,
                                  sectorNo);
                    break;
                }
            } else {
                isOK = false;
                PrintAndLogEx(WARNING,
                              "command execute timeout when trying to read block %2d of sector %2d.",
                              blockNo, sectorNo);
                break;
            }
        }
    ...
    if (isOK) {
        ...
        fclose(fout);
        PrintAndLogEx(SUCCESS, "dumped %d blocks (%d bytes) to file %s", numblocks, 16 * numblocks,
                      dataFilename);
    }
    return 0;
}
    ...
    for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
        for (blockNo = 0; blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
            ...
            PrintAndLogEx(NORMAL, "Writing to block %3d: %s",
                          FirstBlockOfSector(sectorNo) + blockNo, sprint_hex(bldata, 16));
...
int CmdHF14AMfNested(const char *Cmd) {
    ...
    if (res) {
        PrintAndLogEx(WARNING, "key is wrong. Can't authenticate to block:%3d key type:%c", blockNo,
                      keyType ? 'B' : 'A');
        return 3;
    }
    ...
    if (cmdp == 'o') {
        ...
        res = mfCheckKeys_fast(SectorsCnt, true, true, 1, MIFARE_DEFAULTKEYS_SIZE + 1, keyBlock,
                               e_sector);
                ...
                for (trgKeyType = 0; trgKeyType < 2; ++trgKeyType) {
                    ...
                    switch (isOK) {
                        ...
                        case -5 :
                            ...
                            res = mfCheckKeys_fast(SectorsCnt, true, true, 2, 1, keyBlock,
                                                   e_sector);
                            continue;
        ...
        if (createDumpFile) {
            ...
            if (fptr == NULL)
                return 1;
            //TODO 此处我们需要进行路径拼接，生成存放于sdcard目录的绝对路径!
            char *tmp = GenerateAbsoluteFilePath(fptr, "/sdcard/NfcTools/keyFile/");
            //释放之前的再重新赋值!
            free(fptr);
            fptr = tmp;
...
int CmdHF14AMfNestedHard(const char *Cmd) {
    ...
    switch (tolower(param_getchar(Cmd, cmdp))) {
        ...
        case 'r':
            ...
            if (fptr == NULL)
                strncpy(filename, NONCESBIN, FILE_PATH_SIZE);
            else
    ...
    if (!know_target_key) {
        ...
        if (res) {
            PrintAndLogEx(NORMAL, "Key is wrong. Can't authenticate to block:%3d key type:%c",
                          blockNo, keyType ? 'B' : 'A');
            return 3;
        }
    }
...
int CmdHF14AMfChk_fast(const char *Cmd) {
    ...
    for (i = 1; param_getchar(Cmd, i); i++) {
        ...
        if (clen == 12) {
            ...
            while (fgets(buf, sizeof(buf), f)) {
                ...
                if (!isxdigit(buf[0])) {
                    PrintAndLogEx(FAILED, "File content error. '%s' must include 12 HEX symbols",
                                  buf);
                    continue;
                }
    ...
    for (uint8_t strategy = 1; strategy < 3; strategy++) {
        ...
        for (uint32_t i = 0; i < keycnt; i += chunksize) {
            ...
            int res = mfCheckKeys_fast(sectorsCnt, firstChunk, lastChunk, strategy, size,
                                       keyBlock + (i * 6), e_sector);
    ...
    if (createDumpFile) {
        ...
        fclose(fkeys);
        PrintAndLogEx(NORMAL,
                      "Found keys have been dumped to file %s. 0xffffffffffff has been inserted for unknown keys.",
                      filename);
    }
...
int CmdHF14AMfChk(const char *Cmd) {
    ...
    uint64_t t1 = msclock();
    // check keys.
    uint8_t trgKeyType = keyType < 2 ? keyType : 0;
    for (; trgKeyType < 2; (keyType == 2) ? (++trgKeyType) : (trgKeyType = 2)) {
        int b = blockNo;
        for (int i = 0; i < SectorsCnt; ++i) {
...
void readerAttack(nonces_t data, bool setEmulatorMem, bool verbose) {
    ...
    if (success) {
        ...
        if (setEmulatorMem) {
            ...
            PrintAndLogEx(NORMAL, "Setting Emulator Memory Block %02d: [%s]", (sector * 4) + 3,
...
int CmdHF14AMf1kSim(const char *Cmd) {
    ...
    PrintAndLogEx(NORMAL, " uid:%s, numreads:%d, flags:%d (0x%02x) ",
                  (uidlen == 0) ? "N/A" : sprint_hex(uid, uidlen >> 1), exitAfterNReads, flags,
                  flags);
    ...
    if (flags & FLAG_INTERACTIVE) {
        PrintAndLogEx(NORMAL, "Press pm3-button or write another cmd to abort simulation");
...
int CmdHF14AMfSniff(const char *Cmd) {
    ...
    PrintAndLogEx(NORMAL,
                  "Press the key on the proxmark3 devices to abort both proxmark3 and client.\n");
    ...
    while (true) {
        ...
        if (res == 2) {                                // received all data, start displaying
            ...
            while (bufPtr - buf < blockLen) {
                ...
                // x = uid,  a = atqa, c = sak
                if ((len == 17) && (bufPtr[0] == 0xff) && (bufPtr[1] == 0xff) &&
                    (bufPtr[15] == 0xff) && (bufPtr[16] == 0xff)) {
                    ...
                    PrintAndLogEx(NORMAL, "%03d| %s |%s", num, isTag ? "TAG" : "RDR",
...
void printKeyTable(uint8_t sectorscnt, sector_t *e_sector) {
    ...
    for (uint8_t i = 0; i < sectorscnt; ++i) {
        ...
        PrintAndLogEx(NORMAL, "|%03d|  %s  | %d |  %s  | %d |", i, strA, e_sector[i].foundKey[0],
...
int CmdHF14AMfELoad(const char *Cmd) {
    ...
    while (!feof(f)) {
        ...
        if (strlen(buf) < blockWidth) {
            if (strlen(buf) && feof(f))
                break;
            PrintAndLogEx(WARNING, "File content error. Block data must include %d HEX symbols",
                          blockWidth);
            fclose(f);
            return 2;
        }
    ...
    if (blockWidth == 8) {
        if ((blockNum != numBlocks)) {
            PrintAndLogEx(FAILED,
                          "Warning, Ultralight/Ntag file content, Loaded %d blocks into emulator memory",
                          blockNum);
            return 0;
        }
    } else {
        if ((blockNum != numBlocks)) {
            PrintAndLogEx(FAILED,
                          "Error, file content, Only loaded %d blocks, must be %d blocks into emulator memory",
                          blockNum, numBlocks);
            return 4;
        }
    }
...
int CmdHF14AMfCLoad(const char *Cmd) {
    ...
    if (fillFromEmulator) {
        for (blockNum = 0; blockNum < 16 * 4; blockNum += 1) {
            ...
            if (blockNum == 0)
                flags = MAGIC_INIT +
                        MAGIC_WUPC;                // switch on field and write magic sequence
            if (blockNum == 1) flags = 0;                                    // just write
            if (blockNum == 16 * 4 - 1)
                flags = MAGIC_HALT + MAGIC_OFF;        // Done. Magic Halt and switch off field.
    ...
    while (!feof(f)) {
        ...
        if (blockNum == 1) flags = 0;                                    // just write
        if (blockNum == 16 * 4 - 1)
            flags = MAGIC_HALT + MAGIC_OFF;        // Done. Switch off field.
...
int CmdHF14AMfice(const char *Cmd) {
    ...
    do {
        ...
        flags |= slow ? 0x0002 : 0;
        UsbCommand c = {CMD_MIFARE_ACQUIRE_NONCES,
    ...
    UsbCommand c = {CMD_MIFARE_ACQUIRE_NONCES,
                    {blockNo + keyType * 0x100, trgBlockNo + trgKeyType * 0x100, 4}};
    clearCommandBuffer();
    SendCommand(&c);
    return 0;
}
