...
int usage_flashmem_read(void){
	PrintAndLogEx(NORMAL, "Read flash memory on devices");
...
int usage_flashmem_load(void){
	PrintAndLogEx(NORMAL, "Loads binary file into flash memory on devices");
...
int usage_flashmem_save(void){
	PrintAndLogEx(NORMAL, "Saves flash memory on devices into the file");
...
int usage_flashmem_wipe(void){
	...
	PrintAndLogEx(NORMAL, "Wipe flash memory on devices, which fills memory with 0xFF\n");
