...
int usage_t55xx_wakup(){
	...
	PrintAndLogEx(NORMAL, "This commands write the Answer-On-Request command and leaves the readerfield ON afterwards.");
	PrintAndLogEx(NORMAL, "Options:");
	PrintAndLogEx(NORMAL, "     h 			- this help");
    PrintAndLogEx(NORMAL, "     p <password>	- password 4bytes (8 hex symbols)");
    PrintAndLogEx(NORMAL, "");
	PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(NORMAL, "      lf t55xx wakeup p 11223344  - write wakeup password");
	return 0;
}
...
static int SanityOfflineCheck( bool useGraphBuffer ){
	if ( !useGraphBuffer && offline) {
		PrintAndLogEx(NORMAL, "Your proxmark3 devices is offline. Specify [1] to use graphbuffer data instead");
		return 0;
	}
	return 1;
}
...
int CmdT55xxWriteBlock(const char *Cmd) {
	...
	if ( !WaitForResponseTimeout(CMD_ACK, &resp, 1500 )){
		PrintAndLogEx(WARNING, "Error occurred, devices did not ACK write operation. (May be due to old firmware)");
		return 0;
	}
	return 1;
}
	...
	if (cmdp == 'i' || cmdp == 'I') {
		...
		for (uint16_t c = 0; c < keycnt; ++c ) {
			...
			if ( !AquireData(T55x7_PAGE0, T55x7_CONFIGURATION_BLOCK, true, testpwd)) {
				PrintAndLogEx(NORMAL, "Aquireing data from devices failed. Quitting");
				free(keyBlock);
				return 0;
			}
...
int tryOnePassword(uint32_t password) {
	...
	if (!AquireData(T55x7_PAGE0, T55x7_CONFIGURATION_BLOCK, true, password)) {
		PrintAndLogEx(NORMAL, "Acquire data from devices failed. Quitting");
		return -1;
	}
