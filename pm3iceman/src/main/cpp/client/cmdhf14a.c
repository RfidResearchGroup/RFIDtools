...
int usage_hf_14a_raw(void) {
	...
	PrintAndLogEx(NORMAL, "       -p    leave the signal field ON after read");
	PrintAndLogEx(NORMAL, "       -a    active signal field ON without select");
	PrintAndLogEx(NORMAL, "       -s    active signal field ON with select");
	PrintAndLogEx(NORMAL, "       -b    number of bits to write. Useful for write partial byte");
	PrintAndLogEx(NORMAL, "       -t    timeout in ms");
	PrintAndLogEx(NORMAL, "       -T    use Topaz protocol to write command");
...
int usage_hf_14a_apdu(void) {
	...
	PrintAndLogEx(NORMAL, "       -k    leave the signal field ON after read response");
