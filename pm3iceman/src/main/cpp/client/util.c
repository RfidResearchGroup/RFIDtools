...
int ukbhit(void) {
    ...
    //try read a char!
    int c = getchar();
    //if not -1，have keyboard input!
    bool ret = c != EOF;
    //我们需要进行读取回滚!c
    if (ret) ungetc(c, stdin);
    //恢复堵塞!
    flags &= ~O_NONBLOCK;
    if (fcntl(STDIN_FILENO, F_SETFL, flags) < 0) {
        perror("fcntl");
...
char *sprint_bin_break(const uint8_t *data, const size_t len, const uint8_t breaks) {
    ...
    if (breaks > 0 && len % breaks != 0)
        rowlen = (len + (len / breaks) > MAX_BIN_BREAK_LENGTH) ? MAX_BIN_BREAK_LENGTH : len + (len /
                                                                                               breaks);
