...
//-----------------------------------------------------------------------------
// Prepare iClass reader command to write to FPGA
//-----------------------------------------------------------------------------
...
void ReaderIClass(uint8_t arg0) {
	...
	bool flagReadAIA = arg0 & FLAG_ICLASS_READER_AIA;					// flag to read block5, maps issuer area
		...
		for ( uint16_t block=0; block < cardsize; block++) {
			...
			if (sendCmdGetResponseWithRetries(read, sizeof(read), resp, 10, 5)) {
				...
				if (stored_data_length + 8 > USB_CMD_DATA_SIZE) {
					//Time to write this off and start afresh
					cmd_send(CMD_ACK,
