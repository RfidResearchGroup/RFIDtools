...
// write an incomplete dummy response in order to trigger the card's authentication failure timeout
#ifndef CHK_TIMEOUT
# define CHK_TIMEOUT() { \
...
uint8_t chkKey( struct chk_t *c ) {
	...
	while( i < 5 ) {
		...
		// if successfull auth, write HALT
		// if ( !res ) 
	...
	if ( strategy == 1 ) {
		...
		for (uint8_t s = 0; s < sectorcnt; ++s) {
			...
			for (uint8_t i = 0; i < keyCount; ++i) {
				...
				// Allow button press / usb cmd to interrupt devices
...
OUT:	
	...
	// All keys found, write to client, or last keychunk from client
...
void MifareCSetBlock(uint32_t arg0, uint32_t arg1, uint8_t *datain){
	...
	while (true) {
		...
		if ((mifare_sendcmd_short(NULL, 0, ISO14443A_CMD_WRITEBLOCK, blockNo, receivedAnswer, receivedAnswerPar, NULL) != 1) || (receivedAnswer[0] != 0x0a)) {
			if (MF_DBGLEVEL >= MF_DBG_ERROR)	Dbprintf("write block write command error");
			errormsg = 4;
			break;
		}
		...
		if ((ReaderReceive(receivedAnswer, receivedAnswerPar) != 1) || (receivedAnswer[0] != 0x0a)) {
			if (MF_DBGLEVEL >= MF_DBG_ERROR)	Dbprintf("write block write data error");
			errormsg = 0;
			break;
		}	
...
void MifareCGetBlock(uint32_t arg0, uint32_t arg1, uint8_t *datain){
	...
	while (true) {
		...
		if ((mifare_sendcmd_short(NULL, 0, ISO14443A_CMD_READBLOCK, blockNo, receivedAnswer, receivedAnswerPar, NULL) != 18)) {
			if (MF_DBGLEVEL >= MF_DBG_ERROR) Dbprintf("read block write command error");
			errormsg = 0;
			break;
		}
	...
	// if MAGIC_DATAIN, the data stays on devices side.
...
/*
	...
	*/
OUT:;
	// removed the if,  since some magic tags misbehavies and write an answer to it.
	mifare_classic_halt_ex(NULL);
