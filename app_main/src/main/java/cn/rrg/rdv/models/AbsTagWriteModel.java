...
public abstract class AbsTagWriteModel
    ...
    private boolean writeBlock(boolean writeZero, MifareAdapter tag, int block, byte[] key, byte[] data, boolean useKeyB) {
        ...
        boolean ret = false;
        boolean auth;
        int sector = MfDataUtils.blockToSector(block);
        if (useKeyB) {
    ...
    public void writeBlock(boolean writeZero, int block, M1KeyBean keys, String data, WriterCallback callback) {
        ...
        //判断给出的密钥是否可以用来验证这个块
        if (MfDataUtils.blockToSector(block) != keys.getSector()) {
    ...
    private void writeSector(boolean writeZero, boolean order, M1Bean data, M1KeyBean keys, WriterCallback callback) {
        ...
        //进一步验证数据和卡片类型的映射!
        if (datas.length != MfDataUtils.getBlockCountInSector(data.getSector())) {
            //数据长度跟卡片对应扇区的块数量不匹配!
        ...
        if (order) {
            ...
            int startBlobk = MfDataUtils.sectorToBlock(sector);
            for (int i = 0, j = startBlobk; i < datas.length; ++i, ++j) {
                ...
                if (!retA && !retB) {
                    //TODO 用户反馈说写0块时如果不是特殊卡失败了很糟心，不想看见，因此隐藏
            ...
            int sector = data.getSector();
            int lastBlock = MfDataUtils.sectorToBlock(sector) + MfDataUtils.getBlockCountInSector(sector) - 1;
            for (int i = datas.length - 1, j = lastBlock; i >= 0; --i, --j) {
                ...
                if (MfDataUtils.isTrailerBlock(j)) {
                    //写入了尾部块后应当重新截取密钥!
                    if (retA) {
    ...
    private void writeSector(MifareAdapter tag, M1Bean data, WriterCallback callback) {
        ...
        //获得当前扇区最后一个块
        int lastBlock = MfDataUtils.sectorToBlock(_sector) + MfDataUtils.getBlockCountInSector(_sector) - 1;
