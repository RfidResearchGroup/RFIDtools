...
void SimulateIso14443bTag(void)
{
	...
	for(;;) {
		...
		if ( (len == sizeof(cmd1) && memcmp(receivedCmd, cmd1, len) == 0)
			...
			respLen	= 0;
			respCode = resp1Code;
			//don't crash at new command just wait and see if reader will write other new cmds.
			//break;
		}
 ...
 * recv        bool when true wait for data from tag and write to client
 * powerfield  bool leave the field on when true
 * data        buffer with byte to write
 *
 * @Output
 * none
 *
