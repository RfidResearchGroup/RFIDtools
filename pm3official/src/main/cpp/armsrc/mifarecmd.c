...
void MifareCWipe(uint32_t arg0, uint32_t arg1, uint32_t arg2, uint8_t *datain){
	...
	while (true){
		...
		if (needFill){
			...
			// write blocks command
			for (int blockNo = 0; blockNo < numBlocks; blockNo++) {
				if ((mifare_sendcmd_short(NULL, 0, 0xA0, blockNo, receivedAnswer, receivedAnswerPar, NULL) != 1) || (receivedAnswer[0] != 0x0a)) {
					if (MF_DBGLEVEL >= 1)	Dbprintf("write block write command error");
					break;
				};
				...
				if ((ReaderReceive(receivedAnswer, receivedAnswerPar) != 1) || (receivedAnswer[0] != 0x0a)) {
					if (MF_DBGLEVEL >= 1)	Dbprintf("write block write data error");
					break;
				};
			}
		...
		if (workFlags & 0x01) {
			if(!iso14443a_select_card(uid, NULL, &cuid, true, 0, true)) {
				...
				if(mifare_classic_halt(NULL, cuid)) {
					if (MF_DBGLEVEL > 2)	Dbprintf("Halt error");
					// Continue, some magic tags misbehavies and write an answer to it.
					// break;
				};
		};
...
void MifareCGetBlock(uint32_t arg0, uint32_t arg1, uint32_t arg2, uint8_t *datain){
	...
	while (true) {
		...
		if ((mifare_sendcmd_short(NULL, 0, 0x30, blockNo, receivedAnswer, receivedAnswerPar, NULL) != 18)) {
			if (MF_DBGLEVEL >= 1)	Dbprintf("read block write command error");
			break;
		};
...
void MifareCIdent(){
	...
	// From iceman1001: removed the if,  since some magic tags misbehavies and write an answer to it.
