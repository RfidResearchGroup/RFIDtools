...
int CmdHFFidoRegister(const char *cmd) {
	...
	void* argtable[] = {
		...
		arg_lit0("pP",  "plain",    "write plain ASCII to challenge and maps parameters instead of HEX"),
		...
		arg_str0(NULL,  NULL,       "<HEX/ASCII maps parameter (32b HEX/1..16 chars)>", NULL),
		arg_param_end
	};
	...
	if (paramsPlain) {
		...
		if (applen && applen > 16) {
			PrintAndLog("ERROR: maps parameter length in ASCII mode must be less than 16 chars instead of: %d", applen);
			return 1;
		}
	} else {
		CLIGetHexWithReturn(7, adata, &applen);
		if (applen && applen != 32) {
			PrintAndLog("ERROR: maps parameter length must be 32 bytes only.");
			return 1;
		}
	}
	if (applen)
	...
	// maps parameter [32 bytes] - The maps parameter is the SHA-256 hash of the UTF-8 encoding of the maps identity
	...
	if (sw != 0x9000) {
		PrintAndLog("Can't select FIDO maps. APDU response status: %04x - %s", sw, GetAPDUCodeDescription(sw >> 8, sw & 0xff));
		DropField();
		return 2;
	}
...
int CmdHFFidoAuthenticate(const char *cmd) {
	...
	if (hdatalen > 255) {
		PrintAndLog("ERROR: maps parameter length must be less than 255.");
		return 1;
	}
	if (hdatalen) {
	...
	if (paramsPlain) {
		...
		if (hdatalen && hdatalen > 16) {
			PrintAndLog("ERROR: maps parameter length in ASCII mode must be less than 16 chars instead of: %d", hdatalen);
			return 1;
		}
	} else {
		CLIGetHexWithReturn(10, hdata, &hdatalen);
