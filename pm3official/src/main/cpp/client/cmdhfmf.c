...
int CmdHF14AMifare(const char *Cmd) {
    ...
    switch (isOK) {
        case -1 :
            PrintAndLog("Button pressed. Aborted.");
            return 1;
        case -2 :
            PrintAndLog(
                    "Card is not vulnerable to Darkside attack (doesn't write NACK on authentication requests).");
            return 1;
        case -3 :
            PrintAndLog(
...
int CmdHF14AMfNestedHard(const char *Cmd) {
    ...
    if (ctmp != 'R' && ctmp != 'r' && ctmp != 'T' && ctmp != 't' && strlen(Cmd) < 20) {
        ...
        PrintAndLog(
                "      iX: set type of SIMD instructions. Without this flag programs autodetect it.");
    ...
    PrintAndLog(
            "--target block no:%3d, target key type:%c, known target key: 0x%02x%02x%02x%02x%02x%02x%s, file action: %s, Slow: %s, Tests: %d ",
            trgBlockNo,
            trgKeyType ? 'B' : 'A',
    ...
    int16_t isOK = mfnestedhard(blockNo, keyType, key, trgBlockNo, trgKeyType,
                                know_target_key ? trgkey : NULL, nonce_file_read, nonce_file_write,
                                slow, tests);
...
int CmdHF14AMfWrBl(const char *Cmd) {
    ...
    PrintAndLog("--block no:%d, key type:%c, key:%s", blockNo, keyType ? 'B' : 'A',
                sprint_hex(key, 6));
...
int CmdHF14AMfRdBl(const char *Cmd) {
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
        ...
        if (mfIsSectorTrailer(blockNo) && (data[6] || data[7] || data[8])) {
            ...
            for (int i = 0; i < 4; i++) {
                PrintAndLogEx(NORMAL, "Access block %d%s: %s", bln,
                              ((blinc > 1) && (i < 3) ? "+" : ""),
...
int CmdHF14AMfRdSc(const char *Cmd) {
    ...
    PrintAndLog("--sector no:%d key type:%c key:%s ", sectorNo, keyType ? 'B' : 'A',
                sprint_hex(key, 6));
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
        ...
        if (isOK) {
            ...
            for (i = 0; i < 4; i++) {
                PrintAndLogEx(NORMAL, "Access block %d%s: %s", bln,
                              ((blinc > 1) && (i < 3) ? "+" : ""), mfGetAccessConditionsDesc(i,
                                                                                             &(data +
                                                                                               (sectorNo <
                                                                                                32
                                                                                                ? 3
            ...
            PrintAndLogEx(NORMAL, "UserData: %s",
                          sprint_hex_inrow(&(data + (sectorNo < 32 ? 3 : 15) * 16)[9], 1));
        }
    } else {
        PrintAndLog("Command execute timeout");
    }
...
int CmdHF14AMfDump(const char *Cmd) {
    ...
    if (strlen(Cmd) > 3 || cmdp == 'h' || cmdp == 'H') {
        ...
        PrintAndLog(
                "  k: Always try using both Key A and Key B for each sector, even if access bits would prohibit it");
    ...
    if ((fin = fopen("/sdcard/NfcTools/keyFile/dumpkeys.bin", "rb")) == NULL) {
    ...
    for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
        for (tries = 0; tries < 3; tries++) {
            UsbCommand c = {CMD_MIFARE_READBL,
                            {FirstBlockOfSector(sectorNo) + NumBlocksPerSector(sectorNo) - 1, 0,
                             0}};
            ...
            if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
                ...
                uint8_t *data = resp.d.asBytes;
                if (isOK) {
                    rights[sectorNo][0] = ((data[7] & 0x10) >> 2) | ((data[8] & 0x1) << 1) |
                                          ((data[8] & 0x10) >> 4); // C1C2C3 for data area 0
                    rights[sectorNo][1] = ((data[7] & 0x20) >> 3) | ((data[8] & 0x2) << 0) |
                                          ((data[8] & 0x20) >> 5); // C1C2C3 for data area 1
                    rights[sectorNo][2] = ((data[7] & 0x40) >> 4) | ((data[8] & 0x4) >> 1) |
                                          ((data[8] & 0x40) >> 6); // C1C2C3 for data area 2
                    rights[sectorNo][3] = ((data[7] & 0x80) >> 5) | ((data[8] & 0x8) >> 2) |
                                          ((data[8] & 0x80) >> 7); // C1C2C3 for sector trailer
                    break;
                } else if (tries == 2) { // on last try set defaults
                    PrintAndLog(
                            "Could not get access rights for sector %2d. Trying with defaults...",
                            sectorNo);
                ...
                PrintAndLog(
                        "Command execute timeout when trying to read access rights for sector %2d. Trying with defaults...",
                        sectorNo);
                rights[sectorNo][0] = rights[sectorNo][1] = rights[sectorNo][2] = 0x00;
                rights[sectorNo][3] = 0x01;
            }
        ...
        for (blockNo = 0; isOK && blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
            ...
            for (tries = 0; tries < 3; tries++) {
                if (blockNo == NumBlocksPerSector(sectorNo) -
                    ...
                    UsbCommand c = {CMD_MIFARE_READBL,
                                    {FirstBlockOfSector(sectorNo) + blockNo, 0, 0}};
                    memcpy(c.d.asBytes, keys[0][sectorNo], 6);
                    SendCommand(&c);
                    ...
                    for (int k = 0; k <= 1; k++) {
                        UsbCommand c = {CMD_MIFARE_READBL,
                                        {FirstBlockOfSector(sectorNo) + blockNo, 1, 0}};
                    ...
                    if ((rights[sectorNo][data_area] == 0x03) ||
                        ...
                        SendCommand(&c);
                        received = WaitForResponseTimeout(CMD_ACK, &resp, 1500);
                    } else if (rights[sectorNo][data_area] ==
                               0x07) {                                        // no key would work
                        PrintAndLog("Access rights do not allow reading of sector %2d block %3d",
                                    sectorNo, blockNo);
                        if (nullMissingKeys) {
            ...
            if (received) {
                ...
                uint8_t *data = resp.d.asBytes;
                if (blockNo ==
                    NumBlocksPerSector(sectorNo) - 1) {        // sector trailer. Fill in the keys.
                ...
                isOK = false;
                PrintAndLog("Command execute timeout when trying to read block %2d of sector %2d.",
                            blockNo, sectorNo);
                break;
            }
        }
    ...
    if (isOK) {
        if ((fout = fopen("/sdcard/NfcTools/dumpFile/dumpdata.bin", "wb")) == NULL) {
            PrintAndLog("Could not create file name dumpdata.bin");
            return 1;
        }
        uint16_t numblocks =
                FirstBlockOfSector(numSectors - 1) + NumBlocksPerSector(numSectors - 1);
...
int CmdHF14AMfRestore(const char *Cmd) {
    ...
    if ((fkeys = fopen("/sdcard/NfcTools/keyFile/dumpkeys.bin", "rb")) == NULL) {
        PrintAndLog("Could not find file dumpkeys.bin");
        return 1;
    }
    ...
    if ((fdump = fopen("/sdcard/NfcTools/dumpFile/dumpdata.bin", "rb")) == NULL) {
    ...
    for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
        for (blockNo = 0; blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
            UsbCommand c = {CMD_MIFARE_WRITEBL,
                            {FirstBlockOfSector(sectorNo) + blockNo, keyType, 0}};
            memcpy(c.d.asBytes, key, 6);
            ...
            PrintAndLog("Writing to block %3d: %s", FirstBlockOfSector(sectorNo) + blockNo,
...
static void
parseParamTDS(const char *Cmd, const uint8_t indx, bool *paramT, bool *paramD, uint8_t *timeout) {
    char ctmp3[4] = {0};
...
int CmdHF14AMfNested(const char *Cmd) {
    ...
    if (param_getchar(Cmd, 1) == '*') {
        ...
        PrintAndLog("--nested. sectors:%2d, block no:*, eml:%c, dmp=%c checktimeout=%d us",
                    SectorsCnt, transferToEml ? 'y' : 'n', createDumpFile ? 'y' : 'n',
                    ((int) btimeout14a * 10000) / 106);
    } else {
        ...
        if (res) {
            PrintAndLog("Can't authenticate to block:%3d key type:%c key:%s", blockNo,
        ...
        PrintAndLog(
                "--nested. sectors:%2d, block no:%3d, key type:%c, eml:%c, dmp=%c checktimeout=%d us",
                SectorsCnt, blockNo, keyType ? 'B' : 'A', transferToEml ? 'y' : 'n',
                createDumpFile ? 'y' : 'n', ((int) btimeout14a * 10000) / 106);
    }

    // one-sector nested
    if (cmdp == 'o') { // ------------------------------------  one sector working
        PrintAndLog("--target block no:%3d, target key type:%c ", trgBlockNo,
                    trgKeyType ? 'B' : 'A');
        ...
        for (int defaultKeyCounter = 0;
             defaultKeyCounter < MifareDefaultKeysSize; defaultKeyCounter++) {
            num_to_bytes(MifareDefaultKeys[defaultKeyCounter], 6,
                         (uint8_t *) (keyBlock + defaultKeyCounter * 6));
        }
        ...
        if (autosearchKey) {
            ...
            PrintAndLog("--auto key. block no:%3d, key type:%c key:%s", blockNo,
                        keyType ? 'B' : 'A', sprint_hex(key, 6));
        }
        ...
        for (i = 0; i < NESTED_SECTOR_RETRY; i++) {
            for (uint8_t sectorNo = 0; sectorNo < SectorsCnt; sectorNo++) {
                for (trgKeyType = 0; trgKeyType < 2; trgKeyType++) {
                    ...
                    PrintAndLog("-----------------------------------------------");
                    int16_t isOK = mfnested(blockNo, keyType, key, FirstBlockOfSector(sectorNo),
        ...
        PrintAndLog("Time in nested: %1.3f (%1.3f sec per key)",
                    ((float) (msclock() - msclock1)) / 1000.0,
                    ((float) (msclock() - msclock1)) / iterations / 1000.0);
        ...
        for (i = 0; i < SectorsCnt; i++) {
            PrintAndLog("|%03d|  %012" PRIx64 "  | %d |  %012" PRIx64 "  | %d |", i,
                        e_sector[i].Key[0], e_sector[i].foundKey[0], e_sector[i].Key[1],
        ...
        // Create dump file
        if (createDumpFile) {
            if ((fkeys = fopen("/sdcard/NfcTools/keyFile/dumpkeys.bin", "wb")) == NULL) {
...
int CmdHF14AMfChk(const char *Cmd) {
    ...
    PrintAndLog(
            "--chk keys. sectors:%2d, block no:%3d, key type:%c, eml:%c, dmp=%c checktimeout=%d us",
            SectorsCnt, blockNo, keyType ? 'B' : 'A', transferToEml ? 'y' : 'n',
            createDumpFile ? 'y' : 'n', ((int) btimeout14a * 10000) / 106);
    ...
    for (i = param3InUse; param_getchar(Cmd, 2 + i); i++) {
        if (!param_gethex(Cmd, 2 + i, keyBlock + 6 * keycnt, 12)) {
            ...
            PrintAndLog("chk key[%2d] %02x%02x%02x%02x%02x%02x", keycnt,
                        ...
                        (keyBlock + 6 * keycnt)[2],
                        (keyBlock + 6 * keycnt)[3], (keyBlock + 6 * keycnt)[4],
                        (keyBlock + 6 * keycnt)[5], 6);
            keycnt++;
        } else {
            // May be a dic file
            ...
            if ((f = fopen(filename, "r"))) {
                while (fgets(buf, sizeof(buf), f)) {
                    ...
                    PrintAndLog("chk custom key[%2d] %012" PRIx64, keycnt,
    ...
    if (SectorsCnt) {
        ...
        for (uint32_t c = 0; c < keycnt; c += max_keys) {
            ...
            res = mfCheckKeysSec(SectorsCnt, keyType, btimeout14a, true, size, &keyBlock[6 * c],
                                 e_sector); // timeout is (ms * 106)/10 or us*0.0106
        ...
        do {
            for (uint32_t c = 0; c < keycnt; c += max_keys) {
                ...
                if (res != 1) {
                    if (!res) {
                        PrintAndLog("Found valid key:[%d:%c]%012" PRIx64, blockNo,
                                    (keyAB & 0x01) ? 'B' : 'A', key64);
                        foundAKey = true;
                    }
                } else {
                    PrintAndLog("Command execute timeout");
    ...
    if (transferToEml) {
        ...
        for (uint16_t sectorNo = 0; sectorNo < SectorsCnt; sectorNo++) {
            if (e_sector[sectorNo].foundKey[0] || e_sector[sectorNo].foundKey[1]) {
                mfEmlGetMem(block, FirstBlockOfSector(sectorNo) + NumBlocksPerSector(sectorNo) - 1,
                            1);
                ...
                mfEmlSetMem(block, FirstBlockOfSector(sectorNo) + NumBlocksPerSector(sectorNo) - 1,
                            1);
            }
        }
    ...
    if (createDumpFile) {
        FILE *fkeys = fopen("/sdcard/NfcTools/keyFile/dumpkeys.bin", "wb");
        if (fkeys == NULL) {
            PrintAndLog("Could not create file dumpkeys.bin");
            free(e_sector);
            free(keyBlock);
    ...
    for (uint8_t i = 0; i < ATTACK_KEY_COUNT; i++) {
        if (ar_resp[i].ar2 > 0) {
            ...
            if (doStandardAttack && mfkey32(ar_resp[i], &key)) {
                PrintAndLog("  Found Key%s for sector %02d: [%04x%08x]",
                            (ar_resp[i].keytype) ? "B" : "A", ar_resp[i].sector,
                ...
                PrintAndLog("M-Found Key%s for sector %02d: [%012" PRIx64 "]", keyType ? "B" : "A",
                            sectorNum, key
                );
    ...
    if (setEmulatorMem) {
        for (uint8_t i = 0; i < ATTACK_KEY_COUNT; i++) {
            if (key_cnt[i] > 0) {
                ...
                snprintf(cmd1, sizeof(cmd1), "%04x%08xFF078069%04x%08x",
                         (uint32_t) (sector_trailer[i].keyA >> 32),
                         (uint32_t) (sector_trailer[i].keyA & 0xFFFFFFFF),
                         (uint32_t) (sector_trailer[i].keyB >> 32),
                         (uint32_t) (sector_trailer[i].keyB & 0xFFFFFFFF));
...
int usage_hf14_mf1ksim(void) {
    ...
    PrintAndLog(
            "      e    (Optional) set keys found from 'reader attack' to emulator memory (implies x and i)");
    PrintAndLog(
            "      f    (Optional) get UIDs to use for 'reader attack' from file 'f <filename.txt>' (implies x and i)");
    PrintAndLog(
            "      r    (Optional) Generate random nonces instead of sequential nonces. Standard reader attack won't work with this option, only moebius attack works.");
    PrintAndLog("samples:");
        ...
        while (!feof(f) && !ukbhit()) {
            ...
            switch (uidlen) {
                ...
                case 8:
                    ...
                    break;
                default:
                    PrintAndLog("uid in file wrong length at %d (length: %d) [%s]", count, uidlen,
                                buf);
                    fclose(f);
                    return 2;
            }
...
int CmdHF14AMfCWipe(const char *Cmd) {
    ...
    PrintAndLog("--blocks count:%2d wipe:%c fill:%c", numBlocks, (wipeCard) ? 'y' : 'n',
                (fillCard) ? 'y' : 'n');
...
int CmdHF14AMfCSetBlk(const char *Cmd) {
    ...
    if (gen == 2) {
        /* generation 1b magic card */
        res = mfCSetBlock(blockNo, memBlock, NULL, wipeCard,
                          CSETBLOCK_SINGLE_OPER | CSETBLOCK_MAGIC_1B);
    } else {
        /* generation 1a magic card by default */
...
int CmdHF14AMfCLoad(const char *Cmd) {
    ...
    if (fillFromEmulator) {
        for (blockNum = 0; blockNum < numblock; blockNum += 1) {
            ...
            if (blockNum == 0)
                flags = CSETBLOCK_INIT_FIELD +
                        CSETBLOCK_WUPC;                // switch on field and write magic sequence
            if (blockNum == 1)
                flags = 0;                                                    // just write
            if (blockNum == numblock - 1)
                flags = CSETBLOCK_HALT +
                        CSETBLOCK_RESET_FIELD;        // Done. Magic Halt and switch off field.
        ...
        while (!feof(f)) {
            ...
            if (blockNum == 1)
                flags = 0;                                                    // just write
            if (blockNum == numblock - 1)
                flags = CSETBLOCK_HALT + CSETBLOCK_RESET_FIELD;        // Done. Switch off field.
...
int CmdHF14AMfCGetBlk(const char *Cmd) {
    ...
    if (mfIsSectorTrailer(blockNo)) {
        ...
        int blinc = (mfNumBlocksPerSector(mfSectorNum(blockNo)) > 4) ? 5 : 1;
        for (int i = 0; i < 4; i++) {
            PrintAndLogEx(NORMAL, "Access block %d%s: %s", bln, ((blinc > 1) && (i < 3) ? "+" : ""),
                          mfGetAccessConditionsDesc(i, &memBlock[6]));
            bln += blinc;
        }
...
int CmdHF14AMfCGetSc(const char *Cmd) {
    ...
    if (sectorNo > 39) {
        PrintAndLog(
                "Sector number must be in [0..15] in MIFARE classic 1k and [0..39] in MIFARE classic 4k.");
        return 1;
    }
...
int CmdHF14AMfCSave(const char *Cmd) {
    ...
    if (param_getchar(Cmd, 0) == 'h') {
        PrintAndLog(
                "It saves `magic Chinese` card dump into the file `filename.eml` or `cardID.eml`");
    ...
    if (fillFromEmulator) {
        ...
        PrintAndLog("*** Start dump to file ***");
        // put hex
        flags = CSETBLOCK_INIT_FIELD + CSETBLOCK_WUPC;
        for (i = 0; i < numblock; i++) {
            if (i == 1) flags = 0;
...
int CmdHF14AMfSniff(const char *Cmd) {
    ...
    if (ctmp == 'h' || ctmp == 'H') {
        PrintAndLog(
                "It continuously gets data from the field and saves it to: log, emulator, emulator file.");
        ...
        PrintAndLog(
                " n/a   e - decrypt sequence, collect read and write commands and save the result of the sequence to emulator memory");
        PrintAndLog(
                "    f - decrypt sequence, collect read and write commands and save the result of the sequence to emulator dump file `uid.eml`");
    ...
    printf("Press the key on the proxmark3 devices to abort both proxmark3 and client.\n");
...
int CmdDecryptTraceCmds(const char *Cmd) {
    ...
    return tryDecryptWord(param_get32ex(Cmd, 0, 0, 16), param_get32ex(Cmd, 1, 0, 16),
                          param_get32ex(Cmd, 2, 0, 16), data, len / 2);
}
