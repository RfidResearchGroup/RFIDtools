	...
	*dataoutlen = 0;
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
		...
		*dataoutlen = iLen - 2;
		...
		if (recv[0] != header[0]) {
			PrintAndLog("14aRAW ERROR: iso14443-4 framing error. Card write %2x must be %2x", dataout[0], header[0]);
			return 2;
		}
...
int CmdHF14AAPDU(const char *cmd) {
	...
	void* argtable[] = {
		...
		arg_lit0("kK",  "keep",    "leave the signal field ON after read response"),
...
int CmdHF14ACmdRaw(const char *cmd) {
	...
	void* argtable[] = {
		...
		arg_lit0("pP",  "power",   "leave the signal field ON after read"),
		arg_lit0("aA",  "active",  "active signal field ON without select"),
		arg_lit0("sS",  "actives", "active signal field ON with select"),
		arg_int0("bB",  "bits",    NULL, "number of bits to write. Useful for write partial byte"),
		arg_int0("t",   "timeout", NULL, "timeout in ms"),
		arg_lit0("T",   "topaz",   "use Topaz protocol to write command"),
		arg_lit0("3",   NULL,      "ISO14443-3 select only (skip RATS)"),
