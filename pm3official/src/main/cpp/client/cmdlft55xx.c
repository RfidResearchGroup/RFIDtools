...
int usage_t55xx_wakup(){
	...
	PrintAndLog("This commands write the Answer-On-Request command and leaves the readerfield ON afterwards.");
	PrintAndLog("Options:");
	PrintAndLog("     h             - this help");
	PrintAndLog("     <password>  - [required] password 4bytes (8 hex symbols)");
	PrintAndLog("");
	PrintAndLog("Examples:");
		PrintAndLog("      lf t55xx wakeup 11223344  - write wakeup password");
	return 0;
}
...
int CmdT55xxWriteBlock(const char *Cmd) {
	...
	if (!WaitForResponseTimeout(CMD_ACK, &resp, 1000)){
		PrintAndLog("Error occurred, devices did not ACK write operation. (May be due to old firmware)");
		return 0;
	}
	return 1;
}
	...
	if (cmdp == 'i' || cmdp == 'I') {
		...
		for (uint16_t c = 0; c < keycnt; ++c ) {
			...
			if ( !AquireData(T55x7_PAGE0, T55x7_CONFIGURATION_BLOCK, true, testpwd)) {
				PrintAndLog("Aquireing data from devices failed. Quitting");
				free(keyBlock);
				return 0;
			}
