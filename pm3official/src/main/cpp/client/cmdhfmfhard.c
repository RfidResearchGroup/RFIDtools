...
#define NONCESBIN "/sdcard/NfcTools/pm3/nonces.bin"

#define DEBUG_KEY_ELIMINATION
// #define DEBUG_REDUCTION

static uint16_t sums[NUM_SUMS] = {0, 32, 56, 64, 80, 96, 104, 112, 120, 128, 136, 144, 152, 160,
                                  176, 192, 200, 224, 256}; // possible sum property values
...
void hardnested_print_progress(uint32_t nonces, char *activity, float brute_force,
    ...
    if (msclock() - last_print_time > min_diff_print_time) {
        ...
        PrintAndLog(" %7.0f | %7d | %-55s | %15.0f | %5s", (float) total_time / 1000.0, nonces,
                    activity, brute_force, brute_force_time_string);
    }
}
...
//----------------------------------------------------------------------------
static void init_inflate(z_streamp compressed_stream, uint8_t *input_buffer, uint32_t insize,
                         uint8_t *output_buffer, uint32_t outsize) {
...
static void init_bitflip_bitarrays(void) {
    ...
    //TODO 此次出现了执行时目录获取异常的问题！
    char runtime_path[] = "/sdcard/NfcTools/pm3/";
    char state_files_path[
            strlen(runtime_path) + strlen(STATE_FILES_DIRECTORY) + strlen(STATE_FILE_TEMPLATE) + 1];
    ...
    for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
        ...
        for (uint16_t bitflip = 0x001; bitflip < 0x400; bitflip++) {
            ...
            //进行可执行文件运行时目录的添加,我们直接添加runtime_path!!!
            strcpy(state_files_path, runtime_path);
            ...
            if (statesfile == NULL) {
                ...
                fclose(statesfile);
                uint32_t count = 0;
                init_inflate(&compressed_stream, input_buffer, filesize, (uint8_t *) &count,
                             sizeof(count));
    ...
    qsort(all_effective_bitflip, num_1st_byte_effective_bitflips, sizeof(uint16_t),
...
#endif
    char progress_text[80];
    sprintf(progress_text, "Using %d precalculated bitflip state tables",
...
static void init_part_sum_bitarrays(void) {
    for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
        for (uint16_t part_sum_a0 = 0; part_sum_a0 < NUM_PART_SUMS; part_sum_a0++) {
            part_sum_a0_bitarrays[odd_even][part_sum_a0] = (uint32_t *) malloc_bitarray(
    ...
    for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
        for (uint16_t part_sum_a8 = 0; part_sum_a8 < NUM_PART_SUMS; part_sum_a8++) {
            part_sum_a8_bitarrays[odd_even][part_sum_a8] = (uint32_t *) malloc_bitarray(
                    sizeof(uint32_t) * (1 << 19));
...
static void init_sum_bitarrays(void) {
    for (uint16_t sum_a0 = 0; sum_a0 < NUM_SUMS; sum_a0++) {
        for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
            sum_a0_bitarrays[odd_even][sum_a0] = (uint32_t *) malloc_bitarray(
                    sizeof(uint32_t) * (1 << 19));
    ...
    for (uint8_t p = 0; p < NUM_PART_SUMS; p++) {
        for (uint8_t q = 0; q < NUM_PART_SUMS; q++) {
            ...
            bitarray_OR(sum_a0_bitarrays[EVEN_STATE][sum_a0_idx],
                        part_sum_a0_bitarrays[EVEN_STATE][q]);
            bitarray_OR(sum_a0_bitarrays[ODD_STATE][sum_a0_idx],
                        part_sum_a0_bitarrays[ODD_STATE][p]);
        }
    }
    // for (uint16_t sum_a0 = 0; sum_a0 < NUM_SUMS; sum_a0++) {
    // for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
...
static int add_nonce(uint32_t nonce_enc, uint8_t par_enc) {
    ...
    if (p1 ==
        NULL) {                                                                    // need to add at the end of the list
    ...
    } else if ((p1->nonce_enc & 0x00ff0000) != (nonce_enc &
...
static void init_nonce_memory(void) {
    for (uint16_t i = 0; i < 256; i++) {
        ...
        nonces[i].states_bitarray[EVEN_STATE] = (uint32_t *) malloc_bitarray(
                sizeof(uint32_t) * (1 << 19));
        ...
        nonces[i].states_bitarray[ODD_STATE] = (uint32_t *) malloc_bitarray(
                sizeof(uint32_t) * (1 << 19));
...
static double p_hypergeometric(uint16_t i_K, uint16_t n, uint16_t k) {
    ...
    if (k == 0) {
        ...
        if (n - k == N -
                     K) {    // special case. The published recursion below would fail with a divide by zero exception
            double log_result = 0.0;
            ...
            return (p_hypergeometric(i_K, n, k - 1) * (K - k + 1) * (n - k + 1) /
                    (k * (N - K - n + k)));
        }
    }
}
...
static void init_allbitflips_array(void) {
    for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
        uint32_t *bitset = all_bitflips_bitarray[odd_even] = (uint32_t *) malloc_bitarray(
                sizeof(uint32_t) * (1 << 19));
        if (bitset == NULL) {
        ...
        for (uint16_t i = 0; i < 256; i++) {
            for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
                if (nonces[i].all_bitflips_dirty[odd_even]) {
                    ...
                    num_all_bitflips_bitarray[odd_even] = count_bitarray_low20_AND(
                            all_bitflips_bitarray[odd_even], nonces[i].states_bitarray[odd_even]);
...
static uint32_t
estimated_num_states_part_sum_coarse(uint16_t part_sum_a0_idx, uint16_t part_sum_a8_idx,
                                     odd_even_t odd_even) {
    return part_sum_count[odd_even][part_sum_a0_idx][part_sum_a8_idx];
}


static uint32_t estimated_num_states_part_sum(uint8_t first_byte, uint16_t part_sum_a0_idx,
                                              uint16_t part_sum_a8_idx, odd_even_t odd_even) {
    if (odd_even == ODD_STATE) {
                ...
                for (uint8_t r = 0; r < NUM_PART_SUMS; r++) {
                    for (uint8_t s = 0; s < NUM_PART_SUMS; s++) {
                        if (2 * r * (16 - 2 * s) + (16 - 2 * r) * 2 * s == sum_a8) {
                            num_states += (uint64_t) estimated_num_states_part_sum(first_byte, p, r,
                                                                                   ODD_STATE)
                                          * estimated_num_states_part_sum(first_byte, q, s,
                                                                          EVEN_STATE);
                        }
                    }
                }
            ...
            if (2 * p * (16 - 2 * q) + (16 - 2 * p) * 2 * q == sum_a0) {
                for (uint8_t r = 0; r < NUM_PART_SUMS; r++) {
                    for (uint8_t s = 0; s < NUM_PART_SUMS; s++) {
                        if (2 * r * (16 - 2 * s) + (16 - 2 * r) * 2 * s == sum_a8) {
                            num_states +=
                                    (uint64_t) estimated_num_states_part_sum_coarse(p, r, ODD_STATE)
...
static void update_sum_bitarrays(odd_even_t odd_even) {
    if (all_bitflips_bitarray_dirty[odd_even]) {
        for (uint8_t part_sum = 0; part_sum < NUM_PART_SUMS; part_sum++) {
            bitarray_AND(part_sum_a0_bitarrays[odd_even][part_sum],
                         all_bitflips_bitarray[odd_even]);
            bitarray_AND(part_sum_a8_bitarrays[odd_even][part_sum],
                         all_bitflips_bitarray[odd_even]);
        }
        for (uint16_t i = 0; i < 256; i++) {
            nonces[i].num_states_bitarray[odd_even] = count_bitarray_AND(
                    nonces[i].states_bitarray[odd_even], all_bitflips_bitarray[odd_even]);
        }
        for (uint8_t part_sum_a0 = 0; part_sum_a0 < NUM_PART_SUMS; part_sum_a0++) {
            for (uint8_t part_sum_a8 = 0; part_sum_a8 < NUM_PART_SUMS; part_sum_a8++) {
                part_sum_count[odd_even][part_sum_a0][part_sum_a8]
                        += count_bitarray_AND2(part_sum_a0_bitarrays[odd_even][part_sum_a0],
                                               part_sum_a8_bitarrays[odd_even][part_sum_a8]);
            }
        }
        all_bitflips_bitarray_dirty[odd_even] = false;
    }
...
static void update_expected_brute_force(uint8_t best_byte) {
    ...
    for (uint8_t i = 0; i < NUM_SUMS; i++) {
        nonces[best_byte].expected_num_brute_force += nonces[best_byte].sum_a8_guess[i].prob *
                                                      (float) nonces[best_byte].sum_a8_guess[i].num_states /
                                                      2.0;
    ...
    for (uint16_t i = 0; i < 256; i++) {
        ...
        for (uint8_t j = 0; j < NUM_SUMS; j++) {
            ...
            nonces[i].expected_num_brute_force +=
                    nonces[i].sum_a8_guess[j].prob * (float) nonces[i].sum_a8_guess[j].num_states /
                    2.0;
            prob_all_failed -= nonces[i].sum_a8_guess[j].prob;
            nonces[i].expected_num_brute_force +=
                    prob_all_failed * (float) nonces[i].sum_a8_guess[j].num_states / 2.0;
        }
    }
...
#define NUM_REFINES    1
    ...
    for (uint16_t i = 0; i < NUM_REFINES; i++) {
        ...
        for (uint8_t j = 0; j < NUM_SUMS && nonces[first_byte].sum_a8_guess[j].prob > 0.05; j++) {
            nonces[first_byte].sum_a8_guess[j].num_states = estimated_num_states(first_byte,
                                                                                 sums[first_byte_Sum],
        ...
        for (uint8_t j = 0; j < NUM_SUMS; j++) {
            nonces[first_byte].expected_num_brute_force += nonces[first_byte].sum_a8_guess[j].prob *
                                                           (float) nonces[first_byte].sum_a8_guess[j].num_states /
                                                           2.0;
...
static bool shrink_key_space(float *brute_forces) {
    ...
    *brute_forces = MIN(brute_forces1, brute_forces2);
    ...
    return ((hardnested_stage & CHECK_2ND_BYTES)
            && reduction_rate >= 0.0 &&
            reduction_rate < brute_force_per_second * sample_period / 1000.0);
}
        ...
        for (uint16_t i = 0; i < 256; i++) {
            if (nonces[i].sum_a8_guess_dirty) {
                for (uint16_t j = 0; j < NUM_SUMS; j++) {
                    ...
                    nonces[i].sum_a8_guess[j].prob = sum_probability(sum_a8_idx, nonces[i].num,
                                                                     nonces[i].Sum);
                }
                qsort(nonces[i].sum_a8_guess, NUM_SUMS, sizeof(guess_sum_a8_t),
...
static int read_nonce_file(void) {
    ...
    if ((fnonces = fopen(NONCESBIN, "rb")) == NULL) {
    ...
    sprintf(progress_string, "Target Block=%d, Keytype=%c", trgBlockNo,
            trgKeyType == 0 ? 'A' : 'B');
    hardnested_print_progress(num_acquired_nonces, progress_string, (float) (1LL << 47), 0);
...
*check_for_BitFlipProperties_thread(void *args) {
    ...
    if (hardnested_stage & CHECK_1ST_BYTES) {
        ...
        for (uint16_t bitflip_idx = 0;
             bitflip_idx < num_1st_byte_effective_bitflips; bitflip_idx++) {
            ...
            for (uint16_t i = first_byte; i <= last_byte; i++) {
                if (nonces[i].BitFlips[bitflip] == 0 && nonces[i].BitFlips[bitflip ^ 0x100] == 0
                    ...
                    uint8_t parity1 = (nonces[i].first->par_enc)
                            >> 3;                    // parity of first byte
                    uint8_t parity2 = (nonces[i ^ (bitflip & 0xff)].first->par_enc)
            ...
            ((uint8_t *) args)[1] = num_1st_byte_effective_bitflips - bitflip_idx -
                ...
                if (nonces[i].BitFlips[bitflip] == 0) {
                    for (uint16_t j = 0; j < 256; j++) {    // for each 2nd Byte
                        ...
                        if (byte1 != NULL && byte2 != NULL) {
                            ...
                            uint8_t parity2 = byte2->par_enc >> 2 &
                                              0x01;    // parity of 2nd byte with bits flipped
                            if ((parity1 == parity2 && !(bitflip & 0x100))        // bitflip
                                || (parity1 != parity2 && (bitflip & 0x100))) { // not bitflip
                                nonces[i].BitFlips[bitflip] = 1;
                                for (odd_even_t odd_even = EVEN_STATE;
                                     odd_even <= ODD_STATE; odd_even++) {
                                    if (bitflip_bitarrays[odd_even][bitflip] != NULL) {
                                        uint32_t old_count = nonces[i].num_states_bitarray[odd_even];
                                        nonces[i].num_states_bitarray[odd_even] = count_bitarray_AND(
                                                nonces[i].states_bitarray[odd_even],
...
static void check_for_BitFlipProperties(bool time_budget) {
    ...
    uint16_t bytes_per_thread =
            (256 + (NUM_CHECK_BITFLIPS_THREADS / 2)) / NUM_CHECK_BITFLIPS_THREADS;
...
static void
simulate_MFplus_RNG(uint32_t test_cuid, uint64_t test_key, uint32_t *nt_enc, uint8_t *par_enc) {
    ...
    *par_enc = 0;
    ...
    for (int8_t byte_pos = 3; byte_pos >= 0; byte_pos--) {
        ...
        uint8_t nt_byte_enc =
                crypto1_byte(&sim_cs, nt_byte_dec ^ (test_cuid >> (8 * byte_pos)), false) ^
                nt_byte_dec;    // encode the nonce byte
        *nt_enc = (*nt_enc << 8) | nt_byte_enc;
        uint8_t ks_par = filter(
                sim_cs.odd);                                            // the keystream bit to encode/decode the parity bit
        uint8_t nt_byte_par_enc = ks_par ^oddparity8(
                nt_byte_dec);                        // determine the nt byte's parity and encode it
...
static void simulate_acquire_nonces() {
    ...
    if (known_target_key == -1) {
        known_target_key = ((uint64_t) rand() & 0xfff) << 36 | ((uint64_t) rand() & 0xfff) << 24 |
                           ((uint64_t) rand() & 0xfff) << 12 | ((uint64_t) rand() & 0xfff);
    }

    char progress_text[80];
    sprintf(progress_text, "Simulating key %012" PRIx64 ", cuid %08" PRIx32 " ...",
    ...
    do {
        ...
        if (first_byte_num == 256) {
            ...
            if (!reported_suma8) {
                ...
                hardnested_print_progress(num_acquired_nonces, "Apply bit flip properties",
                                          brute_force, 0);
            }
        } else {
            update_nonce_data(true);
            acquisition_completed = shrink_key_space(&brute_force);
            hardnested_print_progress(num_acquired_nonces, "Apply bit flip properties", brute_force,
                                      0);
        }
    ...
    fprintf(fstats, "%" PRId32 ";%" PRId32 ";%1.0f;", total_num_nonces, num_acquired_nonces,
            difftime(end_time, time1));

}


static int acquire_nonces(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo,
                          uint8_t trgKeyType, bool nonce_file_write, bool slow) {
    last_sample_clock = msclock();
    ...
    do {
        ...
        UsbCommand c = {CMD_MIFARE_ACQUIRE_ENCRYPTED_NONCES,
                        {blockNo + keyType * 0x100, trgBlockNo + trgKeyType * 0x100, flags}};
        ...
        if (initialize) {
            ...
            if (nonce_file_write && fnonces == NULL) {
                if ((fnonces = fopen(NONCESBIN, "wb")) == NULL) {
                    PrintAndLog("Could not create file "NONCESBIN);
                    return 3;
                }
                hardnested_print_progress(0, "Writing acquired nonces to binary file "NONCESBIN,
                                          (float) (1LL << 47), 0);
        ...
        if (!initialize) {
            ...
            if (first_byte_num == 256) {
                ...
                if (!reported_suma8) {
                    char progress_string[80];
                    sprintf(progress_string, "Apply Sum property. Sum(a0) = %d",
                            sums[first_byte_Sum]);
                    hardnested_print_progress(num_acquired_nonces, progress_string, brute_force, 0);
                    reported_suma8 = true;
                } else {
...
static inline bool
invariant_holds(uint_fast8_t byte_diff, uint_fast32_t state1, uint_fast32_t state2,
                uint_fast8_t bit, uint_fast8_t state_bit) {
    uint_fast8_t j_1_bit_mask = 0x01 << (bit - 1);
    uint_fast8_t bit_diff = byte_diff &
                            j_1_bit_mask;                                        // difference of (j-1)th bit
    uint_fast8_t filter_diff = filter(state1 >> (4 - state_bit)) ^filter(state2 >> (4 -
                                                                                    state_bit));    // difference in filter function
    uint_fast8_t mask_y12_y13 = 0xc0 >> state_bit;
    uint_fast8_t state_bits_diff = (state1 ^ state2) &
                                   mask_y12_y13;                        // difference in state bits 12 and 13
    uint_fast8_t all_diff = evenparity8(bit_diff ^ state_bits_diff ^
                                        filter_diff);            // use parity function to XOR all bits
    return !all_diff;
}


static inline bool
invalid_state(uint_fast8_t byte_diff, uint_fast32_t state1, uint_fast32_t state2, uint_fast8_t bit,
              uint_fast8_t state_bit) {
    uint_fast8_t j_bit_mask = 0x01 << bit;
    uint_fast8_t bit_diff = byte_diff &
                            j_bit_mask;                                            // difference of jth bit
    uint_fast8_t mask_y13_y16 = 0x48 >> state_bit;
    uint_fast8_t state_bits_diff = (state1 ^ state2) &
                                   mask_y13_y16;                        // difference in state bits 13 and 16
    uint_fast8_t all_diff = evenparity8(bit_diff ^
                                        state_bits_diff);                        // use parity function to XOR all bits
    return all_diff;
}


static inline bool
remaining_bits_match(uint_fast8_t num_common_bits, uint_fast8_t byte_diff, uint_fast32_t state1,
...
#endif
{
    ...
    if (!possible) {
        ...
        if (!quiet && known_target_key != -1 && state == test_state[odd_even]) {
            ...
            sprintf(failstr, "Initial %s Byte Bitflip property",
                    odd_even == EVEN_STATE ? "even" : "odd");
        }
#endif
        return false;
...
static bool all_bitflips_match(uint8_t byte, uint32_t state, odd_even_t odd_even) {
    ...
    for (uint16_t i = 1; i < 256; i++) {
        ...
        for (uint8_t remaining_bits = 0; remaining_bits <= (~mask & 0xff); remaining_bits++) {
            if (remaining_bits_match(num_common, bytes_diff, state, (state & mask) | remaining_bits,
                                     odd_even)) {
#ifdef DEBUG_KEY_ELIMINATION
        ...
        if (!found_match) {
            ...
            if (known_target_key != -1 && state == test_state[odd_even]) {
                ...
                if (failstr[0] == '\0') {
                    sprintf(failstr, "Other 1st Byte %s, all_bitflips_match(), no match",
                            odd_even ? "odd" : "even");
                }
            }
#endif
...
static void bitarray_to_list(uint8_t byte, uint32_t *bitarray, uint32_t *state_list, uint32_t *len,
                             odd_even_t odd_even) {
    uint32_t *p = state_list;
    for (uint32_t state = next_state(bitarray, -1L);
         state < (1 << 24); state = next_state(bitarray, state)) {
...
static void add_cached_states(statelist_t *candidates, uint16_t part_sum_a0, uint16_t part_sum_a8,
                              odd_even_t odd_even) {
...
static void add_matching_states(statelist_t *candidates, uint8_t part_sum_a0, uint8_t part_sum_a8,
                                odd_even_t odd_even) {
    ...
    bitarray_to_list(best_first_bytes[0], candidates_bitarray, candidates->states[odd_even],
                     &(candidates->len[odd_even]), odd_even);
    if (candidates->len[odd_even] == 0) {
        free(candidates->states[odd_even]);
        candidates->states[odd_even] = NULL;
    } else if (candidates->len[odd_even] + 1 < worstcase_size) {
        candidates->states[odd_even] = realloc(candidates->states[odd_even],
                                               sizeof(uint32_t) * (candidates->len[odd_even] + 1));
    }
...
static void add_bitflip_candidates(uint8_t byte) {
    ...
    for (odd_even_t odd_even = EVEN_STATE; odd_even <= ODD_STATE; odd_even++) {
        ...
        bitarray_to_list(byte, nonces[byte].states_bitarray[odd_even], candidates->states[odd_even],
                         &(candidates->len[odd_even]), odd_even);

        if (candidates->len[odd_even] + 1 < worstcase_size) {
            candidates->states[odd_even] = realloc(candidates->states[odd_even], sizeof(uint32_t) *
                                                                                 (candidates->len[odd_even] +
                                                                                  1));
        }
    }
    return;
...
static bool TestIfKeyExists(uint64_t key) {
    ...
    for (statelist_t *p = candidates; p != NULL; p = p->next) {
        ...
        if (p_odd != NULL && p_even != NULL) {
            ...
            count += (uint64_t) (p_odd - p->states[ODD_STATE]) *
                     (uint64_t) (p_even - p->states[EVEN_STATE]);
        }
                            ...
                            if (2 * r * (16 - 2 * s) + (16 - 2 * r) * 2 * s == sum_a8) {
                                ...
                                if (!work_required) {
                                    ...
                                    if (even_completed) { // we had one cache hit with non-zero even states
                                        ...
                                        pthread_mutex_unlock(&statelist_cache_mutex);
                                        pthread_mutex_unlock(&book_of_work_mutex);
                                        add_matching_states(current_candidates, 2 * p, 2 * r,
                                                            ODD_STATE);
                                        work_required = false;
                                        ...
                                        pthread_mutex_unlock(&statelist_cache_mutex);
                                        pthread_mutex_unlock(&book_of_work_mutex);
                                        add_matching_states(current_candidates, 2 * q, 2 * s,
                                                            EVEN_STATE);
                                        work_required = false;
                                    }
                                }
...
static bool brute_force(void) {
    ...
    return brute_force_bs(NULL, candidates, cuid, num_acquired_nonces, maximum_states, nonces,
                          best_first_bytes);
}
...
int
mfnestedhard(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType,
             uint8_t *trgkey, bool nonce_file_read, bool nonce_file_write, bool slow, int tests) {
    char progress_text[80];
    ...
    if (tests) {
        ...
        for (uint32_t i = 0; i < tests; i++) {
            ...
            sprintf(progress_text, "Brute force benchmark: %1.0f million (2^%1.1f) keys/s",
            ...
            fprintf(fstats, "%" PRIu16 ";%1.1f;", sums[first_byte_Sum],
                    log(p_K0[first_byte_Sum]) / log(2.0));
            fprintf(fstats, "%" PRIu16 ";%1.1f;",
                    sums[nonces[best_first_bytes[0]].sum_a8_guess[0].sum_a8_idx],
                    log(p_K[nonces[best_first_bytes[0]].sum_a8_guess[0].sum_a8_idx]) / log(2.0));
            ...
            fprintf(fstats, "%1.1f;%1.1f;", log(expected_brute_force1) / log(2.0),
                    log(expected_brute_force2) / log(2.0));
            if (expected_brute_force1 < expected_brute_force2) {
                hardnested_print_progress(num_acquired_nonces, "(Ignoring Sum(a8) properties)",
                                          expected_brute_force1, 0);
                ...
                hardnested_print_progress(num_acquired_nonces, "Starting brute force...",
                                          expected_brute_force1, 0);
                ...
                for (uint8_t j = 0; j < NUM_SUMS && !key_found; j++) {
                    ...
                    sprintf(progress_text, "(%d. guess: Sum(a8) = %" PRIu16 ")", j + 1,
                            sums[nonces[best_first_bytes[0]].sum_a8_guess[j].sum_a8_idx]);
                    hardnested_print_progress(num_acquired_nonces, progress_text,
                                              expected_brute_force, 0);
                    if (sums[nonces[best_first_bytes[0]].sum_a8_guess[j].sum_a8_idx] !=
                        real_sum_a8) {
                        sprintf(progress_text,
                    ...
                    generate_candidates(first_byte_Sum,
                                        nonces[best_first_bytes[0]].sum_a8_guess[j].sum_a8_idx);
            ...
            fprintf(fstats, "%1.1f;%1.0f;%d;%s\n", log(num_keys_tested) / log(2.0),
                    (float) num_keys_tested / brute_force_per_second, key_found, failstr);
#else
        ...
        if (expected_brute_force1 < expected_brute_force2) {
            ...
            for (uint8_t j = 0; j < NUM_SUMS && !key_found; j++) {
                ...
                sprintf(progress_text, "(%d. guess: Sum(a8) = %" PRIu16 ")", j + 1,
                        sums[nonces[best_first_bytes[0]].sum_a8_guess[j].sum_a8_idx]);
                hardnested_print_progress(num_acquired_nonces, progress_text, expected_brute_force,
                                          0);
                if (trgkey != NULL &&
                    sums[nonces[best_first_bytes[0]].sum_a8_guess[j].sum_a8_idx] != real_sum_a8) {
                    sprintf(progress_text,
                            "(Estimated Sum(a8) is WRONG! Correct Sum(a8) = %" PRIu16 ")",
                            real_sum_a8);
                    hardnested_print_progress(num_acquired_nonces, progress_text,
