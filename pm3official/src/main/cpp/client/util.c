...
int ukbhit(void) {
    ...
    if (0 == (error = tcsetattr(STDIN_FILENO, TCSANOW, &Ntty))) {   // set new attributes
        error += ioctl(STDIN_FILENO, FIONREAD,
                       &cnt);                 // get number of characters availabe
...
void AddLogLine(char *file, char *extData, char *c) {
    ...
    //进行路径拼接!
    sprintf(filename, "/sdcard/NfcTools/pm3/%s", filename);
    //打开文件!
...
void AddLogUint64(char *fileName, char *extData, const uint64_t data) {
    char buf[100] = {0};
    sprintf(buf, "%x%x", (unsigned int) ((data & 0xFFFFFFFF00000000) >> 32),
...
char *sprint_bin_break(const uint8_t *data, const size_t len, const uint8_t breaks) {
    ...
    if (breaks == 0)
        ...
        max_len = (len + (len / breaks) > MAX_BIN_BREAK_LENGTH) ? MAX_BIN_BREAK_LENGTH : len +
                                                                                         (len /
                                                                                          breaks);
...
int param_gethex(const char *line, int paramnum, uint8_t *data, int hexcnt) {
    ...
    for (i = 0; i < hexcnt; i += 2) {
        if (!(isxdigit((unsigned char) line[bg + i]) &&
              isxdigit((unsigned char) line[bg + i + 1])))
            return 1;
...
int
param_gethex_to_eol(const char *line, int paramnum, uint8_t *data, int maxdatalen, int *datalen) {
    int bg, en;
    uint32_t temp;
    char buf[5] = {0};
