...
static void
asn1_tag_dump_str_time(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level,
                       bool longyear, bool *needdump) {
    int len = tlv->len;
    *needdump = false;
...
static void
asn1_tag_dump_string(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level) {
    fprintf(f, "\tvalue: '");
...
static void
asn1_tag_dump_octet_string(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level,
                           bool *needdump) {
    *needdump = false;
...
static void
asn1_tag_dump_boolean(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level) {
    PRINT_INDENT(level);
    if (tlv->len > 0) {
...
static void
asn1_tag_dump_integer(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level) {
    PRINT_INDENT(level);
    if (tlv->len == 4) {
        int32_t val = 0;
        for (int i = 0; i < tlv->len; i++)
...
static void
asn1_tag_dump_object_id(const struct tlv *tlv, const struct asn1_tag *tag, FILE *f, int level) {
    PRINT_INDENT(level);
