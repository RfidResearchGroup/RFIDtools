...
void printEM410x(uint32_t hi, uint64_t id) {
    if (id || hi) {
        ...
        if (hi) {
            ...
            PrintAndLog("DEZ 3.5B       : %03lld.%05" PRIu64, (id & 0xFF000000) >> 24,
                        (id & 0xFFFF));
            ...
            PrintAndLog("Pattern Sebury : %d %d %d  [0x%X 0x%X 0x%X]", sebury1, sebury2, sebury3,
                        sebury1, sebury2, sebury3);
        }
    }
    return;
}
...
int CmdAskEM410xDemod(const char *Cmd) {
    ...
    if (strlen(Cmd) > 10 || cmdp == 'h' || cmdp == 'H') {
        ...
        PrintAndLog(
                "          : lf em 410xdemod 32     = demod an EM410x Tag ID from GraphBuffer using a clock of RF/32");
        PrintAndLog(
                "          : lf em 410xdemod 32 1   = demod an EM410x Tag ID from GraphBuffer using a clock of RF/32 and inverting data");
        PrintAndLog(
                "          : lf em 410xdemod 1      = demod an EM410x Tag ID from GraphBuffer while inverting data");
        PrintAndLog(
                "          : lf em 410xdemod 64 1 0 = demod an EM410x Tag ID from GraphBuffer using a clock of RF/64 and inverting data and allowing 0 demod errors");
        return 0;
    }
    uint64_t lo = 0;
    uint32_t hi = 0;
...
int CmdEM410xSim(const char *Cmd) {
    ...
    PrintAndLog("Starting simulating UID %02X%02X%02X%02X%02X  clock: %d", uid[0], uid[1], uid[2],
                uid[3], uid[4], clock);
...
int CmdEM410xBrute(const char *Cmd) {
    ...
    for (uint32_t c = 0; c < uidcnt; ++c) {
        ...
        PrintAndLog("Bruteforce %d / %d: simulating UID  %s, clock %d", c + 1, uidcnt, testuid,
                    clock);
...
int CmdEM410xWrite(const char *Cmd) {
    ...
    if (card == 1) {
        PrintAndLog("Writing %s tag with UID 0x%010" PRIx64 " (clock rate: %d)", "T55x7", id,
                    clock);
...
// download samples from devices and copy them to the Graphbuffer
...
int
EM4x05WriteWord(uint8_t addr, uint32_t data, uint32_t pwd, bool usePwd, bool swap, bool invert) {
    ...
    if (!WaitForResponseTimeout(CMD_ACK, &resp, 2000)) {
        PrintAndLog("Error occurred, devices did not respond during write operation.");
        return -1;
    }
    if (!downloadSamplesEM()) {
        return -1;
...
void printEM4x05config(uint32_t wordData) {
    ...
    PrintAndLog(
            ...
            LWR, numblks);
    PrintAndLog(" ReadLogin:   %u | Read Login is %s", readLogin,
                readLogin ? "Required" : "Not Required");
    PrintAndLog("   ReadHKL:   %u | Read Housekeeping Words Login is %s", readHKL,
                readHKL ? "Required" : "Not Required");
    PrintAndLog("WriteLogin:   %u | Write Login is %s", writeLogin,
                writeLogin ? "Required" : "Not Required");
    PrintAndLog("  WriteHKL:   %u | Write Housekeeping Words Login is %s", writeHKL,
                writeHKL ? "Required" : "Not Required");
    PrintAndLog("    R.A.W.:   %u | Read After Write is %s", raw, raw ? "On" : "Off");
    PrintAndLog("   Disable:   %u | Disable Command is %s", disable,
...
void printEM4x05info(uint8_t chipType, uint8_t cap, uint16_t custCode, uint32_t serial) {
    switch (chipType) {
        case 9:
            ...
            break;
        case 4:
            PrintAndLog(" Chip Type:   %u | Unknown", chipType);
            break;
        case 2:
            PrintAndLog(" Chip Type:   %u | EM4469", chipType);
            break;
    ...
    switch (cap) {
        case 3:
            PrintAndLog("  Cap Type:   %u | 330pF", cap);
            break;
        case 2:
            PrintAndLog("  Cap Type:   %u | %spF", cap, (chipType == 2) ? "75" : "210");
            break;
        case 1:
            PrintAndLog("  Cap Type:   %u | 250pF", cap);
            break;
        case 0:
            PrintAndLog("  Cap Type:   %u | no resonant capacitor", cap);
            break;
        default:
            PrintAndLog("  Cap Type:   %u | unknown", cap);
            break;
    }
...
void printEM4x05ProtectionBits(uint32_t wordData) {
    for (uint8_t i = 0; i < 15; i++) {
        PrintAndLog("      Word:  %02u | %s", i,
                    (((1 << i) & wordData) || i < 2) ? "Is Write Locked" : "Is Not Write Locked");
        if (i == 14) {
            PrintAndLog("      Word:  %02u | %s", i + 1,
                        (((1 << i) & wordData) || i < 2) ? "Is Write Locked"
                                                         : "Is Not Write Locked");
        }
    }
}
