...
static uint32_t
nonce2key(uint32_t uid, uint32_t nt, uint32_t nr, uint32_t ar, uint64_t par_info, uint64_t ks_info,
          uint64_t **keys) {
...
int mfDarkside(uint64_t *key) {
    ...
    printf("Press button on the proxmark3 devices to abort both proxmark3 and client.\n");
    printf("-------------------------------------------------------------------------\n");
...
int
mfCheckKeys(uint8_t blockNo, uint8_t keyType, bool clear_trace, uint8_t keycnt, uint8_t *keyBlock,
            uint64_t *key) {

    *key = -1;

    UsbCommand c = {CMD_MIFARE_CHKKEYS,
                    {((blockNo & 0xff) | ((keyType & 0xff) << 8)), clear_trace, keycnt}};
...
int mfCheckKeysSec(uint8_t sectorCnt, uint8_t keyType, uint8_t timeout14a, bool clear_trace,
                   uint8_t keycnt, uint8_t *keyBlock, sector_t *e_sector) {
    ...
    UsbCommand c = {CMD_MIFARE_CHKKEYS, {((sectorCnt & 0xff) | ((keyType & 0xff) << 8)),
                                         (clear_trace | 0x02) | ((timeout14a & 0xff) << 8),
                                         keycnt}};
    memcpy(c.d.asBytes, keyBlock, 6 * keycnt);
    SendCommand(&c);

    UsbCommand resp;
    if (!WaitForResponseTimeoutW(CMD_ACK, &resp,
                                 MAX(3000, 1000 + 13 * sectorCnt * keycnt * (keyType == 2 ? 2 : 1)),
                                 false))
        return 1; // timeout: 13 ms / fail auth
    if ((resp.arg[0] & 0xff) != 0x01) return 2;
...
int Compare16Bits(const void *a, const void *b) {
    ...
    else if ((*(uint64_t *) b & 0x00ff000000ff0000) > (*(uint64_t *) a & 0x00ff000000ff0000))
        return 1;
    else return -1;
}
...
int mfnested(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType,
             uint8_t *resultKey, bool calibrate) {
    uint16_t i;
    uint32_t uid;
    UsbCommand resp;
    ...
    UsbCommand c = {CMD_MIFARE_NESTED,
                    {blockNo + keyType * 0x100, trgBlockNo + trgKeyType * 0x100, calibrate}};
    ...
    PrintAndLog("uid:%08x trgbl=%d trgkey=%x", uid, (uint16_t) resp.arg[2] & 0xff,
                (uint16_t) resp.arg[2] >> 8);
    ...
    *(uint64_t *) p4 = -1;
    ...
    for (i = 0; i < statelists[0].len; i++) {
        ...
        key64 = 0;
        if (!mfCheckKeys(statelists[0].blockNo, statelists[0].keyType, false, 1, keyBlock,
                         &key64)) {
...
int mfCSetBlock(uint8_t blockNo, uint8_t *data, uint8_t *uid, bool wantWipe, uint8_t params) {
    ...
    uint8_t isOK = 0;
    UsbCommand c = {CMD_MIFARE_CSETBLOCK,
                    {wantWipe, params & (0xFE | (uid == NULL ? 0 : 1)), blockNo}};
...
int mfCIdentify() {
    ...
    switch (isGeneration) {
        case 1:
            PrintAndLog("Chinese magic backdoor commands (GEN 1a) detected");
            break;
        case 2:
            PrintAndLog("Chinese magic backdoor command (GEN 1b) detected");
            break;
        default:
            PrintAndLog("No chinese magic backdoor command detected");
            break;
    }
...
int isTraceCardEmpty(void) {
    return ((traceCard[0] == 0) && (traceCard[1] == 0) && (traceCard[2] == 0) &&
            (traceCard[3] == 0));
}
...
bool NTParityCheck(uint32_t ntx) {
    if (
            (oddparity8(ntx >> 8 & 0xff) ^ (ntx & 0x01) ^ ((nt_enc_par >> 5) & 0x01) ^
             (nt_enc & 0x01)) ||
            (oddparity8(ntx >> 16 & 0xff) ^ (ntx >> 8 & 0x01) ^ ((nt_enc_par >> 6) & 0x01) ^
             (nt_enc >> 8 & 0x01)) ||
            (oddparity8(ntx >> 24 & 0xff) ^ (ntx >> 16 & 0x01) ^ ((nt_enc_par >> 7) & 0x01) ^
             (nt_enc >> 16 & 0x01))
            )
        return false;
    ...
    if (
            (oddparity8(ar >> 8 & 0xff) ^ (ar & 0x01) ^ ((ar_enc_par >> 5) & 0x01) ^
             (ar_enc & 0x01)) ||
            (oddparity8(ar >> 16 & 0xff) ^ (ar >> 8 & 0x01) ^ ((ar_enc_par >> 6) & 0x01) ^
             (ar_enc >> 8 & 0x01)) ||
            (oddparity8(ar >> 24 & 0xff) ^ (ar >> 16 & 0x01) ^ ((ar_enc_par >> 7) & 0x01) ^
             (ar_enc >> 16 & 0x01))
            )
        return false;
    ...
    if (
            (oddparity8(ar & 0xff) ^ (at >> 24 & 0x01) ^ ((ar_enc_par >> 4) & 0x01) ^
             (at_enc >> 24 & 0x01)) ||
            (oddparity8(at >> 8 & 0xff) ^ (at & 0x01) ^ ((at_enc_par >> 5) & 0x01) ^
             (at_enc & 0x01)) ||
            (oddparity8(at >> 16 & 0xff) ^ (at >> 8 & 0x01) ^ ((at_enc_par >> 6) & 0x01) ^
             (at_enc >> 8 & 0x01)) ||
            (oddparity8(at >> 24 & 0xff) ^ (at >> 16 & 0x01) ^ ((at_enc_par >> 7) & 0x01) ^
             (at_enc >> 16 & 0x01))
            )
        return false;
                    ...
                    if (validate_prng_nonce(nt)) {
                        ...
                        if (NTParityCheck(nt1))
                            printf("key> the same key test OK. key=%x%x\n",
                                   (unsigned int) ((ui64Key & 0xFFFFFFFF00000000) >> 32),
                                   (unsigned int) (ui64Key & 0xFFFFFFFF));
                        else
