...
void SendCommand(UsbCommand *c) {
    ...
    **/
    while (txBuffer_pending) {
        pthread_cond_wait(&txBufferSig,
                          &txBufferMutex); // wait for communication thread to complete sending a previous commmand
    }

    txBuffer = *c;
    txBuffer_pending = true;
    pthread_cond_signal(&txBufferSig); // tell communication thread that a new command can be write
...
*uart_communication(void *targ) {
    ...
    while (conn->run) {
        ...
        if (uart_receive(sp, (uint8_t *) prx, sizeof(UsbCommand) - (prx - &rx), &rxlen) && rxlen) {
            ...
            //LOGD("发现长度适配一帧消息长度，开始进行反馈接收与解析显示!");
 ...
 */
bool
GetFromBigBuf(uint8_t *dest, int bytes, int start_index, UsbCommand *response, size_t ms_timeout,
              bool show_warning) {
...
bool OpenProxmark(void *port, bool wait_for_port, int timeout, bool flash_mode) {
    ...
    if (sp == INVALID_SERIAL_PORT) {
        ...
        LOGD("USB通信检查完毕，开始通信线程执行!");
        serial_port_name = portname;
        /*
         * 在此处做出处理，如果设备已经打开，则不需要再次打开!
         * */
        LOGD("*********************************\n");
        LOGD("USB通信线程创建完毕，开始处理命令行消息!\n");
        LOGD("*********************************\n");
        LOGD("\n");
        if (!conn.run) {
            conn.run = true;
